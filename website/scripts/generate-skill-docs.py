...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/centurion-fleet/Centurion-AI-OS/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    centurion_meta = (fm.get("metadata") or {}).get("centurion") or {}
    tags = centurion_meta.get("tags") or []
    related = centurion_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `centurion skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Centurion AI OS"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Centurion ships with a large built-in skill library copied into `~/.centurion/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Centurion also syncs bundled skills on `centurion update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.centurion/skills/` tree, it is still shipped with Centurion; restore it with `centurion skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with centurion-os — install via centurion skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with centurion-os under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
