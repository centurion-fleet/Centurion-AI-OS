...
description: "Frequently asked questions and solutions to common issues with Centurion AI OS"
---
...
### What LLM providers work with Centurion?

Centurion AI OS works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Centurion Portal** — Centurion Fleet's own inference endpoint
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `centurion login anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `centurion model` or by editing `~/.centurion/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Centurion AI OS requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Centurion from inside it. The standard install command works perfectly in WSL2:

```bash
curl -fsSL https://raw.githubusercontent.com/centurion-fleet/Centurion-AI-OS/main/scripts/install.sh | bash
```

### I run Centurion in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Centurion inside WSL2
...
- let Centurion use the resulting MCP browser tools

This is more reliable than trying to force Centurion core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Centurion](../guides/use-mcp-with-centurion.md#wsl2-bridge-centurion-in-wsl-to-windows-chrome)
...
Yes — Centurion now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Centurion AI OS does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.centurion/`.

### Can I use it offline / with local models?

Yes. Run `centurion model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Centurion minimum; set this to match your server's actual context window
```
...
Centurion persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Centurion — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::
...
Centurion AI OS itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Centurion AI OS instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Centurion programmatically:
...
#### `centurion: command not found` after installation
...
**Cause:** Centurion requires Python 3.11 or newer.
...
**Cause:** Centurion builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Centurion runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Centurion auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.centurion/config.yaml`:
...
sudo rm /usr/local/bin/centurion
...
**Solution:** Exit your session and use `centurion model` from your terminal to add new providers:

```bash
# Exit the Centurion chat session first (Ctrl+C or /quit)
...
After adding a new provider via `centurion model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `centurion model` (from terminal) |
| Enter/change API keys | `centurion model` (from terminal) |
...
centurion config show

# Re-configure your provider
centurion model

# Or set directly
centurion config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.centurion/.env` for conflicting entries.
:::
...
- Using `centurion chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Centurion detected the wrong context length for your model.
...
centurion chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Centurion may have the wrong context length for your model. Check what it detected:
...
**Cause:** Centurion detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Centurion never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `centurion chat`
...
cat ~/.centurion/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `centurion gateway setup`
- Check gateway logs: `cat ~/.centurion/logs/gateway.log | tail -50`
...
Configure in `~/.centurion/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
#### WSL: Gateway keeps disconnecting or `centurion gateway start` fails
...
tmux new -s centurion 'centurion gateway run'
# Reattach later: tmux attach -t centurion

# Option 3: Background via nohup
nohup centurion gateway run > ~/.centurion/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'centurion gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `centurion gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
centurion gateway install    # Re-snapshots your current PATH
centurion gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.centurion.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `centurion chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `centurion chat -t "terminal"`
...
cd ~/.centurion/centurion-ai-os && uv pip install -e ".[mcp]"
...
Verify your `~/.centurion/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
centurion config show | grep -A 12 mcp_servers

# Restart Centurion or reload MCP after config changes
centurion chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with Centurion](/guides/use-mcp-with-centurion)
...
:::warning
If an MCP server crashes mid-request, Centurion will report a timeout. Check the server's own logs (not just Centurion logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `centurion profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `centurion update`?

`centurion update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `centurion update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.centurion/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Centurion can route subagents to a different model automatically. Set this in `~/.centurion/config.yaml`:
...
Now when you tell Centurion "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Centurion do this?

**Current limitation:** Centurion profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Centurion reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `centurion skills config` settings don't seem to take effect.

**Solution:** Use `centurion skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`centurion gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Centurion creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Centurion to another machine
...
1. Install Centurion AI OS on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/centurion-fleet/Centurion-AI-OS/main/scripts/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   centurion backup
   ```
   This creates a zip of your entire `~/.centurion/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/centurion-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/centurion-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   centurion import ~/centurion-backup-<timestamp>.zip
   ```

4. On the new machine, run `centurion setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
centurion profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
centurion profile import ./work-backup.tar.gz work
```
...
### `centurion backup` vs `centurion profile export`

| Feature | `centurion backup` | `centurion profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.centurion` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='centurion-ai-os' ~/.centurion/ newmachine:~/.centurion/
```

:::tip
`centurion backup` produces a consistent snapshot even while Centurion is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Centurion installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Centurion-specific issue — it's a shell config permissions problem.
...
centurion config show | head -20

# Re-run model selection
centurion model

# Or test with a known-good model
centurion chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/centurion-fleet/Centurion-AI-OS/issues)
2. **Ask the community:** [Centurion Fleet Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Centurion version (`centurion --version`), and the full error message
