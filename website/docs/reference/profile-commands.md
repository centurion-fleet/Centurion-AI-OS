...
This page covers all commands related to [Centurion profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `centurion profile` without a subcommand shows help.
...
## `centurion profile use`

```bash
centurion profile use <name>
```

Sets `<name>` as the active profile. All subsequent `centurion` commands (without `-p`) will use this profile.
...
centurion profile use default
```

## `centurion profile create`

```bash
centurion profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `centurion profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-skills` marker into the profile so future `centurion update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. |
...
centurion profile create mybot

# Clone config only from current profile
centurion profile create work --clone

# Clone everything from current profile
centurion profile create backup --clone-all

# Clone config from a specific profile
centurion profile create work2 --clone --clone-from work
```

## `centurion profile describe`

```bash
centurion profile describe [<name>] [options]
```
...
centurion profile describe researcher

# Set it explicitly
centurion profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
centurion profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
centurion profile describe --all --auto
```

## `centurion profile delete`

```bash
centurion profile delete <name> [options]
```
...
```bash
centurion profile delete mybot
centurion profile delete mybot --yes
```
...
## `centurion profile show`

```bash
centurion profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Centurion home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ centurion profile show work
Profile: work
Path:    ~/.centurion/profiles/work
...
## `centurion profile alias`

```bash
centurion profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Centurion installation.
...
```bash
centurion profile alias work
# Creates/updates ~/.local/bin/work

centurion profile alias work --name mywork
# Creates ~/.local/bin/mywork

centurion profile alias work --remove
# Removes the wrapper script
```

## `centurion profile rename`

```bash
centurion profile rename <old-name> <new-name>
```
...
```bash
centurion profile rename mybot assistant
# ~/.centurion/profiles/mybot → ~/.centurion/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `centurion profile export`

```bash
centurion profile export <name> [options]
```
...
```bash
centurion profile export work
# Creates work.tar.gz in the current directory

centurion profile export work -o ./work-2026-03-29.tar.gz
```

## `centurion profile import`

```bash
centurion profile import <archive> [options]
```
...
```bash
centurion profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

centurion profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`centurion profile export` / `import` are still the right commands for
...
### `centurion profile install`

```bash
centurion profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `centurion -p telemetry`). |
...
centurion profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
centurion profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
centurion profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
centurion profile install ./telemetry/
```

### `centurion profile update`

```bash
centurion profile update <name> [--force-config] [--yes]
```
...
### `centurion profile info`

```bash
centurion profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Centurion version, author, env var requirements, the source URL/path, and
...
`centurion profile list` also shows the distribution name and version in a
`Distribution` column, and `centurion profile show <name>` / `delete <name>`
...
centurion profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
centurion profile install https://github.com/your-org/internal-assistant.git
```
...
`centurion_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Centurion version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Centurion can clone from.
3. Tell recipients to run `centurion profile install <your-repo-url>`.
...
## `centurion -p` / `centurion --profile`

```bash
centurion -p <name> <command> [options]
centurion --profile <name> <command> [options]
```

Global flag to run any Centurion command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
centurion -p work chat -q "Check the server status"
centurion --profile dev gateway start
centurion -p personal skills list
centurion -p work config edit
```

## `centurion completion`

```bash
centurion completion <shell>
```
...
- `centurion profile <TAB>` — subcommands (list, use, create, etc.)
- `centurion profile use <TAB>` — profile names
- `centurion -p <TAB>` — profile names
