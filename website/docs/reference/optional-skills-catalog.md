...
description: "Official optional skills shipped with centurion-ai-os — install via centurion skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Optional skills ship with centurion-ai-os under `optional-skills/` but are **not active by default**. Install them explicitly:

```bash
centurion skills install official/<category>/<skill>
```

For example:

```bash
centurion skills install official/blockchain/solana
centurion skills install official/mlops/flash-attention
```
...
```bash
centurion skills uninstall <skill-name>
```
...
| [**honcho**](/user-guide/skills/optional/autonomous-ai-agents/autonomous-ai-agents-honcho) | Configure and use Honcho memory with Centurion -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshoo... |
...
| Skill | Description |
|-------|-------------|
| [**blender-mcp**](/user-guide/skills/optional/creative/creative-blender-mcp) | Control Blender directly from Centurion via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. Use when user wants to create or modify anything in Blender. |
...
| [**kanban-video-orchestrator**](/user-guide/skills/optional/creative/creative-kanban-video-orchestrator) | Plan, set up, and monitor a multi-agent video production pipeline backed by Centurion Kanban. Use when the user wants to make ANY video — narrative film, product/marketing, music video, explainer, ASCII/terminal art, abstract/generative loo... |
...
| Skill | Description |
|-------|-------------|
| [**agentmail**](/user-guide/skills/optional/email/email-agentmail) | Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. centurion-ai-os@agentmail.to). |
...
| [**telephony**](/user-guide/skills/optional/productivity/productivity-telephony) | Give Centurion phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
