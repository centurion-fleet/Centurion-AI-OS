...
description: "Run Centurion AI OS directly on an Android phone with Termux"
---

# Centurion on Android with Termux

This is the tested path for running Centurion AI OS directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Centurion from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Centurion now ships a Termux-aware installer path:
...
- links `centurion` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `centurion` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/centurion" "$PREFIX/bin/centurion"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `centurion` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.centurion/.env`.
...
### `centurion doctor` says ripgrep or Node is missing
