...
description: "Early BETA: run Centurion AI OS natively on Windows 10 / 11 — install, feature matrix, UTF-8 console, Git Bash, gateway as a Scheduled Task, editor handling, PATH, uninstall, and common pitfalls"
...
:::warning Early BETA
Native Windows support is **early beta**. It installs, runs, and passes our Windows-footgun lint, but it hasn't been road-tested at the scale our Linux/macOS/WSL2 paths have. Expect rough edges — especially around subprocess handling, path quirks, and non-ASCII console output. Please [file issues](https://github.com/centurion-fleet/Centurion-AI-OS/issues) with repro steps when you hit something. If you want a battle-tested setup today, use the [Linux/macOS installer under WSL2](./windows-wsl-quickstart.md) instead.
:::

Centurion runs natively on Windows 10 and Windows 11 — no WSL, no Cygwin, no Docker. This page is the deep dive: what works natively, what's WSL-only, what the installer actually does, and the Windows-specific knobs you might need to touch.
...
If you prefer a real POSIX environment (for the dashboard's embedded terminal, `fork` semantics, Linux-style file watchers, etc.), see the **[Windows (WSL2) Guide](./windows-wsl-quickstart.md)**. Both coexist cleanly: native data lives under `%LOCALAPPDATA%\centurion`, WSL data lives under `~/.centurion`.
:::
...
No admin rights required. The installer goes to `%LOCALAPPDATA%\centurion\` and adds `centurion` to your **User PATH** — open a new terminal after it finishes.

**Installer options** (requires the scriptblock form to pass parameters):

```powershell
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/centurion-fleet/Centurion-AI-OS/main/scripts/install.ps1))) -NoVenv -SkipSetup -Branch main
```
...
| `-SkipSetup` | off | Skip the post-install `centurion setup` wizard |
| `-CenturionHome` | `%LOCALAPPDATA%\centurion` | Override data directory |
| `-InstallDir` | `%LOCALAPPDATA%\centurion\centurion-ai-os` | Override code location |
...
A thin GUI installer is also available — useful if you'd rather double-click an `.exe` than open PowerShell. Download Centurion Desktop, run the installer, and on first launch the GUI calls `install.ps1` under the hood to provision Python (via `uv`), Node, PortableGit, and the rest of the dependency bootstrap described below. After the first run, the desktop app and the PowerShell-installed `centurion` CLI share the same `%LOCALAPPDATA%\centurion\centurion-ai-os` install and `%USERPROFILE%\.centurion` data directory — switch between the GUI and the CLI freely.

Use the desktop installer when you want a familiar Windows install experience or you're handing Centurion to a non-developer; use the PowerShell one-liner when you're already in a terminal.

### Dependency bootstrap (`dep_ensure`)

On first launch (and on demand when a missing tool is detected), Centurion runs a small Python bootstrapper — `centurion_cli/dep_ensure.py` — that checks for and lazily installs the non-Python dependencies it needs. On Windows, the relevant ones are:

| Dependency | Why Centurion needs it |
|---|---|
...
3. **Installs Node.js 22** (winget if available, else a portable Node tarball unpacked under `%LOCALAPPDATA%\centurion\node`). Used for the browser tool and the WhatsApp bridge.
4. **Installs portable Git** — if `git` is already on PATH the installer uses it; otherwise it downloads a trimmed, self-contained **PortableGit** (~45 MB, from the official `git-for-windows` release) to `%LOCALAPPDATA%\centurion\git`. No admin, no Windows installer registry, no interference with anything else on the box.
...
Native Windows is still early beta, and per-tool API key setup (Firecrawl, FAL, Browser Use, OpenAI TTS) is the highest-friction part of getting a useful agent. A [Centurion Portal](/user-guide/features/tool-gateway) subscription covers the model **and** all of those tools through one OAuth login. After the installer finishes, run `centurion setup --portal` to wire everything up.
:::
...
|---|---|---|
| CLI (`centurion chat`, `centurion setup`, `centurion gateway`, …) | ✓ | ✓ |
| Interactive TUI (`centurion --tui`) | ✓ | ✓ |
...
## How Centurion runs shell commands on Windows

Centurion's terminal tool runs commands through **Git Bash**, same strategy Claude Code uses. This sidesteps the POSIX-vs-Windows gap without rewriting every tool.
...
Python's default stdio on Windows uses the console's active code page (usually cp1252 or cp437). Centurion's banner, slash-command list, tool feed, Rich panels, and skill descriptions all contain Unicode. Without intervention, any of that crashes with `UnicodeEncodeError: 'charmap' codec can't encode character…`.

The fix is in `centurion_cli/stdio.py::configure_windows_stdio()`, called early in every entry point (`cli.py::main`, `centurion_cli/main.py::main`, `gateway/run.py::main`). It:
...
Centurion's Windows stdio shim now sets `EDITOR=notepad` as a default. Notepad ships with every Windows install and works as a blocking editor — `subprocess.call(["notepad", file])` blocks until the window closes.
...
The `--wait` flag on VS Code is critical — without it the editor returns immediately and Centurion gets a blank buffer back.
...
`centurion gateway install` on Windows uses **Scheduled Tasks** with a Startup-folder fallback — no admin required.

### Install

```powershell
centurion gateway install
```

What happens under the hood:

1. `schtasks /Create /SC ONLOGON /RL LIMITED /TN CenturionGateway` — registers a task that runs at your login with standard (non-elevated) permissions. No UAC prompt.
...
```powershell
centurion gateway status      # Merged view: schtasks + Startup folder + running PID
centurion gateway start       # Starts the scheduled task now
centurion gateway stop        # Graceful SIGTERM equivalent (TerminateProcess via psutil)
centurion gateway restart
centurion gateway uninstall   # Removes schtasks entry, Startup shortcut, pid file
```

`centurion gateway status` is idempotent — call it a thousand times in a row and it will never accidentally kill the gateway. (Pre-PR #21561 it silently did, via `os.kill(pid, 0)` colliding with `CTRL_C_EVENT` at the C level — see "process management internals" below if you care about the story.)

### Why not a Windows Service?

Services require admin rights to install and tie the gateway's lifecycle to machine boot, not user login. The typical Centurion user wants: log in → gateway available, log out → gateway gone. Scheduled Tasks do exactly that without elevation. If you genuinely want a service, use `nssm` or `sc create` manually — but you probably don't.

## Data layout

| Path | Contents |
|---|---|
| `%LOCALAPPDATA%\centurion\centurion-ai-os\` | Git checkout + venv. Safe to `Remove-Item -Recurse` and reinstall. |
| `%LOCALAPPDATA%\centurion\git\` | PortableGit (only if the installer provisioned it). |
| `%LOCALAPPDATA%\centurion\node\` | Portable Node.js (only if the installer provisioned it). |
| `%LOCALAPPDATA%\centurion\bin\` | `centurion.cmd` shim, added to User PATH. |
| `%USERPROFILE%\.centurion\` | Your config, auth, skills, sessions, logs. **Survives reinstalls.** |

The split is deliberate: `%LOCALAPPDATA%\centurion` is disposable infrastructure (you can blow it away and the one-liner restores it). `%USERPROFILE%\.centurion` is your data — config, memory, skills, session history — and is identical in shape to a Linux install. Mirror it between machines and your Centurion moves with you.

**Override `CENTURION_HOME`:** set the environment variable to point at a different data dir. Works the same as on Linux.
...
- `shutil.which("agent-browser", path=...)` picks up the `.cmd` shim automatically — `CreateProcessW` can't execute an extensionless shebang, so Centurion always resolves to the `.CMD` wrapper. Don't manually invoke the shebang script; always go through the `.cmd`.
- Playwright Chromium is auto-installed on first run (`npx playwright install chromium`). If installation fails, `centurion doctor` surfaces it with a fix-it hint.

## Running Centurion on Windows — practical notes

### PATH after install

The installer adds `%LOCALAPPDATA%\centurion\bin` to your **User PATH** via `[Environment]::SetEnvironmentVariable`. Existing terminals don't pick this up — open a new PowerShell window (or Windows Terminal tab) after installation. Close-and-reopen, don't `$env:PATH += …` by hand unless you know what you're doing.

Verify:

```powershell
Get-Command centurion        # should print C:\Users\<you>\AppData\Local\centurion\bin\centurion.cmd
centurion --version
```

### Environment variables

Centurion honors both `$env:X` (process-scope) and User environment variables (permanent, set in System Properties → Environment Variables). Setting API keys in `%USERPROFILE%\.centurion\.env` is the normal path — same as Linux:
...
That's the clean path — removes the schtasks entry, Startup folder shortcut, `centurion.cmd` shim, deletes `%LOCALAPPDATA%\centurion\centurion-ai-os\`, and trims the User PATH. It leaves `%USERPROFILE%\.centurion\` alone (your config, auth, skills, sessions, logs) in case you're reinstalling.

To nuke everything:

```powershell
centurion uninstall
Remove-Item -Recurse -Force "$env:USERPROFILE\.centurion"
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\centurion"
```

The `centurion uninstall` CLI subcommand also handles the case where the schtasks entry was registered under a different task name (older installs) — it searches by install path rather than by hardcoded task name.
...
Consequence: any codepath that said "check if this PID is alive" via `os.kill(pid, 0)` on Windows was silently killing the target. Centurion migrated every such site (14 across 11 files) to `gateway.status._pid_exists()`, which uses `psutil.pid_exists()` (which in turn uses `OpenProcess + GetExitCodeProcess` on Windows — no signals). If you're writing a plugin or patch, use `psutil.pid_exists()` directly or `gateway.status._pid_exists()` — never `os.kill(pid, 0)`.
...
**`centurion: command not found` right after install.**
Open a new PowerShell window. The installer added `%LOCALAPPDATA%\centurion\bin` to User PATH, but existing shells need to be restarted to pick it up. In the meantime you can run `& "$env:LOCALAPPDATA\centurion\bin\centurion.cmd"`.

**`WinError 193: %1 is not a valid Win32 application` when running a tool.**
You hit a shebang-script invocation that bypassed the `.cmd` shim. Centurion resolves commands through `shutil.which(cmd, path=local_bin)` so PATHEXT picks up `.CMD` — if you're invoking the tool via a hardcoded path instead, switch to the `.cmd` variant (e.g., `npx.cmd`, not `npx`).
...
Chromium is auto-installed on first run. If the install failed (rate-limited GitHub, Playwright CDN hiccup), run `centurion doctor` — it will surface the missing Chromium and print the exact `npx playwright install chromium` command to fix it.
...
If you edited Centurion config or a skill on Windows using a non-UTF-8 editor (Notepad on older Windows versions, some Chinese IMEs), the file may have been saved with a BOM. Centurion tolerates `utf-8-sig` on most config reads, but a BOM inside a folded YAML scalar (`description: >`) silently breaks YAML parsing. Re-save the file as plain UTF-8 without BOM.
...
- **[CLI Reference](../reference/cli-commands.md)** — every `centurion` subcommand.
