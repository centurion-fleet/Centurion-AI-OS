...
title: "Git Worktrees"
description: "Run multiple Centurion agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Centurion AI OS is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Centurion so each session has a clean, isolated working directory.

## Why Use Worktrees with Centurion?

Centurion treats the **current working directory** as the project root:

- CLI: the directory where you run `centurion` or `centurion chat`
...
git worktree add ../repo-feature feature/centurion-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/centurion-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Centurion there:

```bash
cd ../repo-feature

# Start Centurion in the worktree
centurion
```
...
git worktree add ../repo-experiment-a feature/centurion-a
git worktree add ../repo-experiment-b feature/centurion-b
```
...
Each Centurion process:

- Works on its own branch (`feature/centurion-a` vs `feature/centurion-b`).
...
- Centurion checkpoint data under `~/.centurion/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Centurion experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/centurion-checkpoints-docs`, `feature/centurion-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Centurion from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `centurion -w` (Automatic Worktree Mode)

Centurion has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `centurion/centurion-<hash>`).
...
```bash
centurion -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `centurion -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Centurion session its own clean checkout.
