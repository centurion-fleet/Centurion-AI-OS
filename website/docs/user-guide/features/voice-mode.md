...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Centurion AI OS — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Centurion AI OS supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Centurion](/guides/use-voice-mode-with-centurion).

## Prerequisites

Before using voice features, make sure you have:

1. **Centurion AI OS installed** — `pip install centurion-ai-os` (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `centurion model` or set your preferred provider credentials in `~/.centurion/.env`
3. **A working base setup** — run `centurion` to verify the agent responds to text before enabling voice

:::tip
The `~/.centurion/` directory and default `config.yaml` are created automatically the first time you run `centurion`. You only need to create `~/.centurion/.env` manually for API keys.
:::

:::tip Centurion Portal covers both
A paid [Centurion Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `centurion setup --portal` wires both up at once.
:::
...
Voice mode is available in both the **classic CLI** (`centurion chat`) and the **TUI** (`centurion --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.centurion/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
centurion                # Start the interactive CLI
```
...
1. Start the CLI with `centurion` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.centurion/config.yaml` (default: `ctrl+b`).
:::
...
```bash
centurion gateway        # Start the gateway (connects to configured platforms)
centurion gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@centurionbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.centurion/.env`:
```bash
...
```bash
centurion gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.centurion/.env`
...
- Review gateway logs: `tail -f ~/.centurion/logs/gateway.log`
