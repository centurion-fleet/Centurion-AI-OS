...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Centurion's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Centurion instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
centurion memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.centurion/.env
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$CENTURION_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `centurion` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Centurion syncs them back at session init.
...
The `centurion honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `centurion memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
centurion honcho status          # Connection status, config, and key settings
centurion honcho setup           # Redirects to `centurion memory setup`
centurion honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
centurion honcho peer            # Show or update peer names + dialectic reasoning level
centurion honcho mode            # Show or set recall mode (hybrid/context/tools)
centurion honcho tokens          # Show or set token budget for context and dialectic
centurion honcho identity        # Seed or show the AI peer's Honcho identity
centurion honcho sync            # Sync Honcho config to all existing profiles
centurion honcho peers           # Show peer identities across all profiles
centurion honcho sessions        # List known Honcho session mappings
centurion honcho map             # Map current directory to a Honcho session name
centurion honcho enable          # Enable Honcho for the active profile
centurion honcho disable         # Disable Honcho for the active profile
centurion honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `centurion honcho`

If you previously used the standalone `centurion honcho setup`:
...
No re-login or re-setup needed. Run `centurion memory setup` and select "honcho" — the wizard detects your existing config.
