...
If no `HERMES_SPOTIFY_CLIENT_ID` is set, Centurion walks you through the app registration inline:
...
Removes tokens from `~/.centurion/auth.json`. To also clear the app config, delete `HERMES_SPOTIFY_CLIENT_ID` (and `HERMES_SPOTIFY_REDIRECT_URI` if you set it) from `~/.centurion/.env`, or run the wizard again.
...
| `~/.centurion/.env` | `HERMES_SPOTIFY_CLIENT_ID`, optional `HERMES_SPOTIFY_REDIRECT_URI` |
