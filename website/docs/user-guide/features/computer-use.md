...
Centurion AI OS can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `centurion computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `centurion tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.centurion/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Centurion refreshes the binary in two
places so you don't get stuck on a stale release:

- **`centurion update`** — when you update Centurion itself, if `cua-driver` is
...
- **`centurion computer-use install --upgrade`** — manual force-refresh.
...
`centurion computer-use status` shows the installed version next to the
binary path.
...
Centurion applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.centurion/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Centurion applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `CENTURION_CUA_DRIVER_VERSION`
...
`centurion computer-use install` to fetch the cua-driver binary, or run
`centurion tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/centurion-fleet/Centurion-AI-OS/blob/main/skills/apple/macos-computer-use/SKILL.md)
