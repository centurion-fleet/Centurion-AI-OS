...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, Centurion AI OS supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.centurion/config.yaml`:
...
- **CLI mode** — configured in `~/.centurion/config.yaml`, loaded at startup
