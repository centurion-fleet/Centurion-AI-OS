...
```bash
pip install 'centurionai-os[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install centurionai-os[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- `ptyprocess` — installed by the `pty` extra (`pip install 'centurionai-os[web,pty]'`, or `[all]` covers both)
