...
The web dashboard is a browser-based UI for managing your Centurion AI OS installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
```bash
# Custom port
centurion dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
centurion dashboard --host 0.0.0.0

# Start without opening browser
centurion dashboard --no-open
...
The default `centurion-ai-os` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'centurion-ai-os[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install centurion-ai-os[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.
...
- Node.js (same requirement as `centurion --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'centurion-ai-os[web,pty]'`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `centurion config set` and the gateway read from.
:::
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.centurion/skills/` and grouped by category.
...
This re-reads `~/.centurion/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `centurion_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `centurion update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `centurion dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Centurion Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Centurion Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
