...
Type `@` followed by a reference to inject content directly into your message. Centurion expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Centurion env: `$CENTURION_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CENTURION_HOME/skills/.hub/`
