...
description: "Use your Centurion Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Centurion-managed provider subscription as their
...
| Use case | "Use Centurion as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Centurion Portal OAuth flow. Centurion stores
the refresh token in `~/.centurion/auth.json` — the same place all Centurion
...
Model:      Centurion-4-70B    # or Centurion-4.3-36B, Centurion-4-405B
```
...
Currently shipped: `nous` (Centurion Portal). More OAuth providers can be
added by implementing the `UpstreamAdapter` interface in
`centurion_cli/proxy/adapters/`.
...
If you see `not logged in`, run `centurion login nous`. If you see
...
supports; check `portal.personal-centurion.com/models`.
...
[portal.personal-centurion.com](https://portal.personal-centurion.com).
...
`centurion_cli/proxy/adapters/<provider>.py` and registering it in
