...
description: "Plugins shipped with Centurion AI OS that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Centurion ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.centurion/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Centurion Plugin](/guides/build-a-centurion-plugin) to write your own.
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `centurion memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `centurion plugins list` and the interactive `centurion plugins` UI), but none load until you explicitly enable them:

```bash
centurion plugins enable disk-cleanup
```

Or via `~/.centurion/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Centurion. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
centurion plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `centurion plugins enable <name>`.
...
| `centurion-achievements` | dashboard tab | Steam-style collectible badges generated from your real Centurion session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `centurion memory` and `centurion plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CENTURION_HOME` or `/tmp/centurion-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under CENTURION_HOME | always | Never |
...
**State** — everything lives at `$CENTURION_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `CENTURION_HOME` or `/tmp/centurion-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `centurion plugins enable disk-cleanup` (or check the box in `centurion plugins`).

**Disabling again:** `centurion plugins disable disk-cleanup`.

### observability/langfuse

Traces Centurion turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Centurion' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `centurion logs`.
...
centurion chat -q "hello"              # check the Langfuse UI for a "Centurion turn" trace
```
...
**Disabling:** `centurion plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.centurion/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
centurion plugins enable google_meet
...
**Disabling:** `centurion plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.centurion/cache/google_meet/` until you remove them.

### centurion-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Centurion session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Centurion feature changes.

**How it works:**

- Scans your entire `~/.centurion/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$CENTURION_HOME/plugins/centurion-achievements/state.json`
...
| Secret | Hidden until Centurion detects the first related signal in your history |

**API** — routes mount under `/api/plugins/centurion-achievements/`:
...
**State files** — live under `$CENTURION_HOME/plugins/centurion-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Centurion updates. |
...
**Enabling:** Nothing to enable — `centurion-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `centurion dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/centurion-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.centurion/plugins/centurion-achievements/` that ships no dashboard. The plugin's state files under `$CENTURION_HOME/plugins/centurion-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Centurion plugin — see [Build a Centurion Plugin](/guides/build-a-centurion-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.centurion/plugins/<name>/`
- Manifest source is reported as `bundled` in `centurion plugins list`
