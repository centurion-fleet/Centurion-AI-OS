...
Connect Centurion to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Centurion supports two WeCom integration modes:
...
3. Centurion decrypts the message, queues it for the agent
...
(Use `centurion gateway start` only after `centurion gateway install` has registered the systemd/launchd service.)
