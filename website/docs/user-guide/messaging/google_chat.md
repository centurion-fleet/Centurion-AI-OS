...
sidebar_position: 12
title: "Google Chat"
description: "Set up Centurion AI OS as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Centurion AI OS to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Centurion
...
download the file. Save it somewhere only Centurion can read (e.g.,
`~/.centurion/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a centurion restart)
...
- Principal: `centurion-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Centurion calls
...
- **App name**: whatever you want users to see ("Centurion" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/centurion-chat-events`.
...
`ADDED_TO_SPACE` event that Centurion uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Centurion

Add the Google Chat section to `~/.centurion/.env`:
...
Install the dependencies the Google Chat adapter needs (no Centurion extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Centurion is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Centurion detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Centurion session.
...
3. Download the JSON. Move it onto the host that runs Centurion.
4. On the host, register the client with Centurion:
...
That writes `~/.centurion/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.centurion/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.centurion/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Centurion isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `centurion gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Centurion will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.centurion/google_chat_user_tokens/<sanitized_email>.json` (filesystem
