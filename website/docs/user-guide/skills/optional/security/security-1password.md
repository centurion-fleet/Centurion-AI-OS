...
| | |
|---|---|
| Source | Optional — install with `centurion skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Centurion AI OS |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Centurion terminal calls (desktop app flow only)
...
### Service Account (recommended for Centurion)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.centurion/.env` (the skill will prompt for this on first load).
...
## Centurion Execution Pattern (desktop app flow)

Centurion terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/centurion-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/centurion-op.sock"
