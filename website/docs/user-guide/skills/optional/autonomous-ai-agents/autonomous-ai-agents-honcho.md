...
description: "Configure and use Honcho memory with Centurion -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Centurion -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `centurion skills install official/autonomous-ai-agents/honcho` |
| Path | `optional-skills/autonomous-ai-agents/honcho` |
| Version | `2.0.0` |
| Author | Centurion AI OS |
| License | MIT |
...
# Honcho Memory for Centurion

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Centurion profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/centurion#running-honcho-locally-with-centurion

### Verify

```bash
centurion honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Centurion profile's AI peer
...
Honcho models conversations as interactions between **peers**. Centurion creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Centurion instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Centurion run |
| `global` | Single session across all directories |

Manual override: `centurion honcho map my-project-name`
...
Each Centurion profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block centurion.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `centurion.coder` host block in `honcho.json`
...
```bash
centurion honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Centurion when Honcho memory is active.
...
Config file: `$CENTURION_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `centurion honcho setup`. Ensure `memory.provider: honcho` is in `~/.centurion/config.yaml`.

### Memory not persisting across sessions
Check `centurion honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `centurion profile create <name> --clone`. For existing profiles: `centurion honcho sync`.
...
| Command | Description |
|---------|-------------|
| `centurion honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `centurion honcho status` | Show resolved config, connection test, peer info for active profile |
| `centurion honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `centurion honcho disable` | Disable Honcho for the active profile |
| `centurion honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `centurion honcho peers` | Show peer identities across all profiles |
| `centurion honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `centurion honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `centurion honcho sessions` | List known directory-to-session-name mappings |
| `centurion honcho map <name>` | Map current working directory to a Honcho session name |
| `centurion honcho identity` | Seed AI peer identity or show both peer representations |
| `centurion honcho sync` | Create host blocks for all Centurion profiles that don't have one yet |
| `centurion honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Centurion + Honcho |
| `centurion memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `centurion memory status` | Show active memory provider and config |
| `centurion memory off` | Disable external memory provider |
