...
| | |
|---|---|
| Source | Optional — install with `centurion skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Centurion or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Centurion. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.centurion/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.centurion/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Centurion integration, either:

- configure the server in `~/.centurion/config.yaml` using the `native-mcp` skill, or
...
### Centurion cannot see the deployed server

The server-building part may be correct while the Centurion config is not. Load the `native-mcp` skill and configure the server in `~/.centurion/config.yaml`, then restart Centurion.
