...
| | |
|---|---|
| Source | Optional — install with `centurion skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${CENTURION_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${CENTURION_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${CENTURION_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client centurion
```
...
```bash
PUBLISH="${CENTURION_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client centurion
```
...
```bash
DRIVE="${CENTURION_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `centurion`)    |
