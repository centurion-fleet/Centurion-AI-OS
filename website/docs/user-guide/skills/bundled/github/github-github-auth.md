...
- Give it a name like "centurion-ai-os"
- Select scopes:
...
- Give it a title like "centurion-ai-os-&lt;machine-name>"
...
elif [ -f ~/.centurion/.env ] && grep -q "^GITHUB_TOKEN=" ~/.centurion/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.centurion/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
