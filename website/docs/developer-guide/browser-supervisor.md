...
[PR #12550](https://github.com/centurion-fleet/Centurion-AI-OS/pull/12550) proposed a
...
(`centurion-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Centurion `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `centurion-acp`, `centurion-api-server`, core toolsets (gated on CDP reachability)
...
- `centurion_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
