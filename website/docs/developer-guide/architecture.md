...
sidebar_position: 1
title: "Architecture"
description: "Centurion AI OS internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Centurion AI OS internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ centurion_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # CenturionCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── centurion_state.py           # SQLite session/state database with FTS5
├── centurion_constants.py       # CENTURION_HOME, profile-aware paths
...
│
├── centurion_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `centurion` subcommands (large file)
...
│   ├── skills_config.py      # centurion skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # centurion tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # centurion gateway start/stop
│
...
```text
User input → CenturionCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .centurion.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.centurion/plugins/` (user), `.centurion/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `centurion plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-centurion-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Centurion as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`centurion -p <name>`) gets its own CENTURION_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
