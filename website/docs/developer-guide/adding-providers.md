...
description: "How to add a new inference provider to Centurion AI OS — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Centurion can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `centurion model` menu entries
...
1. `centurion_cli/auth.py` decides how credentials are found.
2. `centurion_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `centurion_cli/models.py` and `centurion_cli/main.py` make the provider show up in the CLI. (`centurion_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `centurion_cli/auth.py`
2. `centurion_cli/models.py`
3. `centurion_cli/runtime_provider.py`
4. `centurion_cli/main.py`
...
:::tip
`centurion_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `centurion setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$CENTURION_HOME/plugins/model-providers/` both get picked up.
...
7. `centurion model` menu includes the provider
8. `centurion setup` wizard delegates to `main.py` automatically
...
User plugins at `$CENTURION_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- OAuth or token refresh (Centurion Portal, Codex, Google Gemini, Qwen Portal, Copilot)
...
- Provider-specific `centurion model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `centurion_cli/auth.py`
- `_PROVIDER_LABELS` in `centurion_cli/models.py`
- `_PROVIDER_ALIASES` in both `centurion_cli/auth.py` and `centurion_cli/models.py`
- CLI `--provider` choices in `centurion_cli/main.py`
...
## Step 2: Add auth metadata in `centurion_cli/auth.py`
...
- What env vars should Centurion check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `centurion_cli/models.py`
...
## Step 4: Resolve runtime data in `centurion_cli/runtime_provider.py`
...
Be careful with API-key precedence. Centurion already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `centurion_cli/main.py`

A provider is not discoverable until it shows up in the interactive `centurion model` flow.

Update these in `centurion_cli/main.py`:
...
:::tip
`centurion_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `centurion model` and `centurion setup` automatically.
:::
...
When you add a native provider, double-check that Centurion is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m centurion_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m centurion_cli.main model
...
- [ ] `ProviderConfig` added in `centurion_cli/auth.py`
- [ ] aliases added in `centurion_cli/auth.py` and `centurion_cli/models.py`
- [ ] model catalog added in `centurion_cli/models.py`
- [ ] runtime branch added in `centurion_cli/runtime_provider.py`
- [ ] CLI wiring added in `centurion_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `centurion model` but not `centurion setup`
