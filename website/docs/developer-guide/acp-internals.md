...
The ACP adapter wraps Centurion' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
centurion acp / centurion-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.centurion/.env
  -> configure stderr logging
  -> construct CenturionACPAgent
...
- `allow_once` -> Centurion `once`
- `allow_always` -> Centurion `always`
- reject options -> Centurion `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Centurion tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["centurion-acp"])
...
Instead it reuses Centurion' runtime resolver:

- `acp_adapter/auth.py`
- `centurion_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Centurion provider/credentials. It also always advertises a terminal setup auth method (`centurion-setup`, args `--setup`) so first-run registry clients can open Centurion' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.centurion/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `centurion-acp` toolset definition
- `centurion_cli/main.py` — `centurion acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `centurion-acp` script
