...
description: "How Centurion builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Centurion deliberately separates:
...
1. agent identity — `SOUL.md` from `CENTURION_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.centurion/SOUL.md)
You are Centurion, an AI assistant created by Centurion Fleet.
...
`SOUL.md` lives at `~/.centurion/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:

```python
# From agent/prompt_builder.py (simplified)
def load_soul_md() -> Optional[str]:
    soul_path = get_centurion_home() / "SOUL.md"
...
```
You are Centurion AI OS, an intelligent AI assistant created by Centurion Fleet.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_centurion_md(cwd_path)       # 1. .centurion.md / CENTURION.md (walks to git root)
    ...
    # SOUL.md from CENTURION_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.centurion.md`, `CENTURION.md` | CWD up to git root | Centurion-native project config |
...
- **YAML frontmatter stripped** — `.centurion.md` frontmatter is removed (reserved for future config overrides)
...
1. `.centurion.md` / `CENTURION.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Centurion already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.centurion/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.centurion/MEMORY.md` and `~/.centurion/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.centurion.md`, `CENTURION.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Centurion.
- Ephemeral overlays such as `CENTURION_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Centurion assembles prompts for everyone, change Python and treat it as a code contribution
