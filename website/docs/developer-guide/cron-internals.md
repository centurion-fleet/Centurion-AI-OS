...
description: "How Centurion stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `centurion_cli/cron.py` | CLI `centurion cron` subcommands |
...
Jobs are stored in `~/.centurion/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `centurion cron` commands are run or during active CLI sessions.
...
```python
# ~/.centurion/scripts/check_competitors.py
...
2. **Environment variable** — `CENTURION_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.centurion/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `centurion cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `centurion cron` CLI provides direct job management:

```bash
centurion cron list                    # Show all jobs
centurion cron create                  # Interactive job creation (alias: add)
centurion cron edit <job_id>           # Edit job configuration
centurion cron pause <job_id>          # Pause a running job
centurion cron resume <job_id>         # Resume a paused job
centurion cron run <job_id>            # Trigger immediate execution
centurion cron remove <job_id>         # Delete a job
```
