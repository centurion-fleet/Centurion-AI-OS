...
Centurion AI OS uses a SQLite database (`~/.centurion/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `centurion_state.py`


## Architecture Overview

```
~/.centurion/state.db (SQLite, WAL mode)
...
Multiple centurion processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.centurion/state.db
...
Default path: `~/.centurion/state.db`

This is derived from `centurion_constants.get_centurion_home()` which resolves to
`~/.centurion/` by default, or the value of `CENTURION_HOME` environment variable.
