...
Copy-paste recipes for common automation patterns. Each template uses Centurion's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`centurion webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `centurion webhook subscribe`) |
...
```bash
centurion cron create "0 2 * * *" \
  "You are a project manager triaging the centurion-fleet/Centurion-AI-OS GitHub repo.

1. Run: gh issue list --repo centurion-fleet/Centurion-AI-OS --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
centurion webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
centurion cron create "0 9 * * 1" \
  "Scan the centurion-fleet/Centurion-AI-OS repo for documentation drift.

1. Run: gh pr list --repo centurion-fleet/Centurion-AI-OS --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (centurion_cli/commands.py, centurion_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (centurion_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
centurion cron create "0 6 * * *" \
  "Run a dependency security audit on the centurion-ai-os project.

1. cd ~/.centurion/centurion-ai-os && source .venv/bin/activate
...
```bash
centurion webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
centurion webhook subscribe alert-triage \
...
```python title="~/.centurion/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Centurion-Monitor/1.0"})
...
```bash
centurion cron create "every 30m" \
  ...
  --script ~/.centurion/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
centurion cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Centurion AI OS development." \
...
```bash
centurion webhook subscribe github-issues \
  --events "issues" \
...
```bash
centurion webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
centurion webhook subscribe stripe-payments \
...
```bash
centurion cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the centurion-ai-os codebase.
