---
sidebar_position: 6
title: "Use MCP with Centurion"
description: "A practical guide to connecting MCP servers to Centurion AI OS, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Centurion

This guide shows how to actually use MCP with Centurion AI OS in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Centurion tool
- you want Centurion to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Centurion to internal APIs, databases, or company systems without modifying Centurion core

Do not use MCP when:
- a built-in Centurion tool already solves the job well
...
- Centurion remains the agent
- MCP servers contribute tools
- Centurion discovers those tools at startup or reload time
...
If you installed Centurion with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Centurion banner/status should show MCP integration when configured
- ask Centurion what tools it has available
...
## WSL2: bridge Centurion in WSL to Windows Chrome

This is the practical setup when:

- Centurion runs inside WSL2
...
In this setup, Centurion does **not** connect to Chrome directly. Instead:

- Centurion runs in WSL
- Centurion starts a local stdio MCP server
...
```text
Centurion (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Centurion stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Centurion core browser transport
...
```bash
centurion mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
centurion mcp test chrome-devtools-win
```

Then start a fresh Centurion session or run:
...
Once loaded, Centurion can use the MCP-prefixed browser tools directly. For example:
...
If Centurion runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Centurion from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Centurion from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Centurion:
...
2. Centurion-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Centurion will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Centurion to reason over a bounded workspace.
...
Now Centurion can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Centurion core.
...
Because Centurion now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
