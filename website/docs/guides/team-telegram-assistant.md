...
This tutorial walks you through setting up a Telegram bot powered by Centurion AI OS that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Centurion AI OS installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.centurion/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Centurion itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Centurion Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_centurion_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Centurion AI OS. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.centurion/.env`:
...
```
[Gateway] Starting Centurion Gateway...
...
# View live logs
journalctl --user -u centurion-gateway -f
...
centurion gateway start
centurion gateway stop
tail -f ~/.centurion/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `centurion gateway install` to update the plist.
:::
...
```bash
centurion gateway stop && centurion gateway start
```
...
centurion pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.centurion/.env`:
...
Control how much detail the bot shows when using tools. In `~/.centurion/config.yaml`:
...
Customize how the bot communicates by editing `~/.centurion/SOUL.md`:

For a full guide, see [Use SOUL.md with Centurion](/guides/use-soul-with-centurion).
...
```bash
# From the CLI
centurion cron list          # View all scheduled jobs
centurion cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u centurion-gateway` (Linux) or `~/.centurion/logs/gateway.log` (macOS) |
| Cron job output | `~/.centurion/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.centurion/cron/jobs.json` |
| Pairing data | `~/.centurion/pairing/` |
| Session history | `~/.centurion/sessions/` |
