...
```bash
centurion teams-pipeline validate
```
...
```bash
centurion teams-pipeline token-health
centurion teams-pipeline token-health --force-refresh
```
...
```bash
centurion teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
centurion teams-pipeline maintain-subscriptions
centurion teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Centurion cron (recommended if you already run the Centurion gateway)

Centurion ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.centurion/scripts/`. First create the script:

```bash
mkdir -p ~/.centurion/scripts
cat > ~/.centurion/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec centurion teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.centurion/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
centurion cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/centurion-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Centurion Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=centurion
EnvironmentFile=/etc/centurion/env
ExecStart=/usr/local/bin/centurion teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/centurion-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Centurion Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now centurion-teams-pipeline-maintain.timer
systemctl list-timers centurion-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/centurion teams-pipeline maintain-subscriptions >> /var/log/centurion/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.centurion/.env` at the top of a wrapper script that crontab calls.
...
```bash
centurion teams-pipeline subscriptions   # should show expirationDateTime advanced
centurion teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
centurion teams-pipeline list
centurion teams-pipeline list --status failed
centurion teams-pipeline show <job-id>
```

### Replay a stored job

```bash
centurion teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
centurion teams-pipeline fetch --meeting-id <meeting-id>
centurion teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `centurion teams-pipeline maintain-subscriptions --dry-run`
- inspect `centurion teams-pipeline list --status failed`
...
- run `centurion teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `centurion teams-pipeline run`
...
- [ ] `centurion teams-pipeline validate` returns an OK snapshot
- [ ] `centurion teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Centurion cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
