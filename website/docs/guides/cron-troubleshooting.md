...
description: "Diagnose and fix common Centurion cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`centurion gateway` for foreground, or `centurion gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `centurion cron tick`.
...
```bash
date
centurion cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.centurion/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.centurion/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.centurion/.env` |
...
| `local` | Write access to `~/.centurion/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `centurion cron list` for updated `last_error` field (if available).
...
```bash
centurion skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `centurion skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `centurion skills list`.
...
2. `~/.centurion/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `centurion cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Centurion config directory). Verify:
```bash
ls ~/.centurion/scripts/your-script.py   # Must exist
centurion cron edit <job_id> --script ~/.centurion/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `centurion skills install <skill-name>`.
...
```bash
ps aux | grep centurion
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.centurion/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.centurion/cron/jobs.json
chmod 600 ~/.centurion/cron/jobs.json   # Your user should own it
```
...
```bash
centurion cron list                    # Show all jobs, states, next_run times
centurion cron run <job_id>            # Schedule for next tick (for testing)
centurion cron edit <job_id>           # Fix configuration issues
centurion logs                         # View recent Centurion logs
centurion skills list                  # Verify installed skills
```
...
1. Run the job with `centurion cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.centurion/logs/agent.log` for scheduler messages and `~/.centurion/logs/errors.log` for warnings
3. Open an issue at [github.com/centurion-fleet/Centurion-AI-OS](https://github.com/centurion-fleet/Centurion-AI-OS) with:
