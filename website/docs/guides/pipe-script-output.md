...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `centurion send`."
---

# Pipe Script Output to Messaging Platforms

`centurion send` is a small, scriptable CLI that pushes a message to any
messaging platform Centurion is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | centurion send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `centurion
...
centurion send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | centurion send --to telegram:-1001234567890

# Send a file
centurion send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
centurion send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
centurion send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
centurion send --list

# Filter by platform
centurion send --list telegram
```
...
Any platform Centurion ships adapters for works as a target:
...
`centurion send` resolves the message body in this order:

1. **Positional argument** — `centurion send --to telegram "hi"`
2. **`--file PATH`** — `centurion send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | centurion send --to telegram`

When stdin is a TTY (no pipe), Centurion does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  centurion send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `centurion send` reuses your Centurion config, the same script works on
any host where Centurion is installed — no need to export bot tokens into
...
`centurion send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  centurion send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | centurion send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.centurion/bin/centurion send \
...
```bash
./train.py --epochs 200 && \
  centurion send --to telegram "training done" || \
  centurion send --to telegram "training failed (exit $?)"
```
...
centurion send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(centurion send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `centurion send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `centurion send` calls
the platform's REST endpoint directly using credentials from
`~/.centurion/.env` and `~/.centurion/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `centurion gateway start` and retry.
...
# Machine-readable
centurion send --list --json
```

The listing is built from `~/.centurion/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`centurion gateway
...
| Approach | Multi-platform | Reuses Centurion creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `centurion send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`centurion send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `centurion send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `centurion send` shares with cron delivery.
