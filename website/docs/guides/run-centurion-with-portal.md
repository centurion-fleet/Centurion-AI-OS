---
sidebar_position: 1
title: "Run Centurion AI OS with Centurion Portal"
description: "Start-to-finish walkthrough: subscribe, set up, switch models, enable gateway tools, and verify routing"
---

# Run Centurion AI OS with Centurion Portal

This guide walks you through running Centurion AI OS on a [Centurion Portal](https://portal.personal-centurion.com) subscription end to end — from signing up to verifying that every tool routes correctly. If you just want the overview of what the Portal is and what's in the subscription, see the [Centurion Portal integration page](/integrations/nous-portal). This page is the task script.

## Prerequisites

- Centurion AI OS installed ([Quickstart](/getting-started/quickstart))
...
Open [portal.personal-centurion.com/manage-subscription](https://portal.personal-centurion.com/manage-subscription), sign up, and pick a plan.
...
3. Sets `model.provider: nous` in `~/.centurion/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Centurion is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
centurion setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
centurion auth add nous --type oauth --manual-paste
# Then re-run `centurion setup --portal` to wire the provider + gateway
```
...
If any line shows something other than "via Centurion Portal" or the auth line says "not logged in", jump to [Troubleshooting](#troubleshooting) below.
...
```
Hey, search the web for "Centurion AI OS release notes" and summarize the top 3 hits.
```

You should see Centurion call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

The default after `centurion setup --portal` is a sensible general-purpose model, but the whole point of the subscription is access to the full catalog. Switch with `/model` mid-session:
...
centurion config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Centurion-4 for agent work

Centurion-4-70B and Centurion-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them via [Nous Chat](https://chat.nousresearch.com) for conversation/research work, or through the [subscription proxy](/user-guide/features/subscription-proxy) from non-agent tools. For Centurion AI OS itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.personal-centurion.com/info) carries this warning too — it's the official Nous guidance, not just a Centurion-side opinion.
...
```bash
centurion portal tools
```

You'll see per-tool routing — `via Centurion Portal` for the ones routed through the subscription, and the partner name (`browserbase`, `firecrawl`, etc.) for the ones using your own keys.
...
```bash
centurion setup voice
# → pick "Nous Subscription" for TTS
# → pick a speech-to-text backend (local faster-whisper is free, no setup)
```

Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Centurion will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
centurion cron add "Daily AI news summary" "every day at 9am" \
...
If you use [Centurion profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.centurion/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `centurion portal status` shows "not logged in" after `centurion setup --portal`
...
```bash
centurion config set model.provider nous
```
...
```bash
centurion model
# pick Centurion Portal
```

Re-verify with `centurion portal status`.

### Tool Gateway tools showing partner names instead of "via Centurion Portal"
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Centurion doesn't replay it endlessly. Just log in again:

```bash
centurion auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/centurion-fleet/Centurion-AI-OS/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`centurion portal status` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Centurion profiles where you're using the wrong one (check `centurion profile current`)

### Want to revoke and start clean

```bash
centurion auth remove nous       # wipes the local refresh token
...
- **[Centurion Portal integration page](/integrations/nous-portal)** — Overview of what's in the subscription
- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway-routed tool
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Centurion tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Centurion configurations
