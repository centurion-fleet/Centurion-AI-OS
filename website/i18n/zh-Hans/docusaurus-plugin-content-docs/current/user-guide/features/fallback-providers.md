...
| Google Gemini（OAuth） | `google-gemini-cli` | `centurion model`（Google OAuth；可选：`HERMES_GEMINI_PROJECT_ID`） |
...
| Qwen Portal（OAuth） | `qwen-oauth` | `centurion model`（Qwen Portal OAuth；可选：`HERMES_QWEN_BASE_URL`） |
