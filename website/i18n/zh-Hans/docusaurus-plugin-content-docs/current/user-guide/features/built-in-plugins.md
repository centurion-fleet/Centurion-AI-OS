...
sidebar_label: "内置插件"
title: "内置插件"
description: "随 Centurion AI OS 附带并通过生命周期 hook 自动运行的插件——disk-cleanup 等"
---

# 内置插件

Centurion 随仓库附带了一小组插件。它们位于 `<repo>/plugins/<name>/`，与用户安装在 `~/.centurion/plugins/` 中的插件一同自动加载。它们使用与第三方插件相同的插件接口——hook、工具、斜杠命令——只是在仓库内维护。

请参阅 [插件](/user-guide/features/plugins) 页面了解通用插件系统，以及 [构建 Centurion 插件](/guides/build-a-centurion-plugin) 了解如何编写自己的插件。
...
`plugins/memory/` 和 `plugins/context_engine/` 被刻意排除在内置扫描之外。这两个目录使用各自的发现路径，因为内存提供者和上下文引擎是通过 `centurion memory setup` / 配置中的 `context.engine` 进行单选配置的提供者。

## 内置插件默认不启用

内置插件随附时处于禁用状态。发现机制会找到它们（它们会出现在 `centurion plugins list` 和交互式 `centurion plugins` UI 中），但在你明确启用之前不会加载：

```bash
centurion plugins enable disk-cleanup
```

或通过 `~/.centurion/config.yaml`：
...
这与用户安装的插件使用的机制相同。内置插件永远不会自动启用——无论是全新安装，还是现有用户升级到更新版本的 Centurion，都需要你明确选择启用。
...
仓库在 `plugins/` 下附带了以下内置插件。所有插件均需手动启用——通过 `centurion plugins enable <name>` 启用。
...
| `centurion-achievements` | 仪表盘标签页 | Steam 风格的可收集徽章，根据你真实的 Centurion 会话历史生成 |
| `kanban/dashboard` | 仪表盘标签页 | 多智能体调度器的看板（Kanban）UI——任务、评论、扇出、切换看板。参见 [Kanban 多智能体](./kanban.md)。 |

内存提供者（`plugins/memory/*`）和上下文引擎（`plugins/context_engine/*`）在 [内存提供者](./memory-providers.md) 中单独列出——它们分别通过 `centurion memory` 和 `centurion plugins` 管理。以下是两个长期运行的基于 hook 的插件的详细说明。
...
| Hook | 行为 |
|---|---|
| `post_tool_call` | 当 `write_file` / `terminal` / `patch` 在 `CENTURION_HOME` 或 `/tmp/centurion-*` 内创建匹配 `test_*`、`tmp_*` 或 `*.test.*` 的文件时，静默追踪为 `test` / `temp` / `cron-output`。 |
...
**状态** — 所有内容存储在 `$CENTURION_HOME/disk-cleanup/`：
...
**安全性** — 清理操作仅涉及 `CENTURION_HOME` 或 `/tmp/centurion-*` 下的路径。Windows 挂载点（`/mnt/c/...`）会被拒绝。已知的顶级状态目录（`logs/`、`memories/`、`sessions/`、`cron/`、`cache/`、`skills/`、`plugins/`、`disk-cleanup/` 本身）即使为空也不会被删除——全新安装不会在第一次会话结束时被清空。

**启用：** `centurion plugins enable disk-cleanup`（或在 `centurion plugins` 中勾选复选框）。

**再次禁用：** `centurion plugins disable disk-cleanup`。

### observability/langfuse

将 Centurion 的轮次、LLM 调用和工具调用追踪到 [Langfuse](https://langfuse.com)——一个开源 LLM 可观测性平台。每轮一个 span，每次 API 调用一个 generation，每次工具调用一个 tool observation。用量总计、各类型 token 数量和成本估算来自 Centurion 的标准 `agent.usage_pricing` 数据，因此 Langfuse 仪表盘看到的分类（input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`）与 `centurion logs` 中显示的一致。
...
```bash
centurion plugins list                 # observability/langfuse 应显示 "enabled"
centurion chat -q "hello"              # 在 Langfuse UI 中检查是否有 "Centurion turn" trace
```
...
**禁用：** `centurion plugins disable observability/langfuse`。插件模块仍会被发现，但在你重新启用之前不会运行任何模块代码。
...
- 会后产物（转录、带发言人归属的笔记、行动项）保存在 `~/.centurion/cache/google_meet/<meeting_id>/`
...
**禁用：** `centurion plugins disable google_meet`。已缓存的转录和录音保留在 `~/.centurion/cache/google_meet/`，直到你手动删除。

### centurion-achievements

在仪表盘中添加一个 **Steam 风格的成就标签页**——60 多个可收集的分级徽章，根据你真实的 Centurion 会话历史生成。工具链成就、调试模式、vibe-coding 连击、技能/内存使用、模型/提供者多样性、生活方式特征（周末和夜间会话）。最初由 [@PCinkusz](https://github.com/PCinkusz) 作为外部插件编写；已并入仓库，以便与 Centurion 功能变更保持同步。

**工作原理：**

- 在仪表盘后端扫描你的整个 `~/.centurion/state.db` 会话历史
- 每个会话的统计数据按 `(started_at, last_active)` 指纹缓存，因此后续扫描只重新分析新增或变更的会话
- 首次扫描在后台线程中运行——即使数据库有数千个会话，仪表盘也不会阻塞等待
- 解锁状态持久化到 `$CENTURION_HOME/plugins/centurion-achievements/state.json`
...
| 已解锁 | 至少达到一个等级 |
| 已发现 | 已知成就，进度可见，尚未获得 |
| 隐藏 | 在 Centurion 检测到你历史中的第一个相关信号之前保持隐藏 |

**API** — 路由挂载在 `/api/plugins/centurion-achievements/` 下：
...
**状态文件** — 位于 `$CENTURION_HOME/plugins/centurion-achievements/`：

| 文件 | 内容 |
|---|---|
| `state.json` | 解锁历史：你获得了哪些徽章以及获得时间。在 Centurion 更新间保持稳定。 |
...
**启用：** 无需启用——`centurion-achievements` 是一个仅限仪表盘的插件（无生命周期 hook，无模型可见工具）。它在 `centurion dashboard` 首次启动时自动注册为标签页。`plugins.enabled` 配置仅控制生命周期/工具插件；仪表盘插件完全通过其 `dashboard/manifest.json` 发现。

**退出：** 删除或重命名 `plugins/centurion-achievements/dashboard/manifest.json`，或在 `~/.centurion/plugins/centurion-achievements/` 中用同名用户插件覆盖它（该插件不包含仪表盘）。`$CENTURION_HOME/plugins/centurion-achievements/` 下的插件状态文件会保留——重新安装后你的解锁历史依然存在。

## 添加内置插件

内置插件的编写方式与其他 Centurion 插件完全相同——参见 [构建 Centurion 插件](/guides/build-a-centurion-plugin)。唯一的区别是：

- 目录位于 `<repo>/plugins/<name>/`，而非 `~/.centurion/plugins/<name>/`
- 在 `centurion plugins list` 中，manifest 来源显示为 `bundled`
- 同名用户插件会覆盖内置版本
