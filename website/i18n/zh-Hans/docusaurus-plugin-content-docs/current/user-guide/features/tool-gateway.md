...
:::tip 快速开始
Tool Gateway 包含在付费 Centurion Portal 订阅中。**[管理订阅 →](https://portal.personal-centurion.com/manage-subscription)**
:::

**Tool Gateway** 让已付费的 [Centurion Portal](https://portal.personal-centurion.com) 用户通过同一份订阅，直接使用网页搜索、文生图、语音合成（TTS）与浏览器自动化，而**不必**再分别注册 Firecrawl、FAL、OpenAI、Browser Use 等服务的 API Key。
...
Tool Gateway 仅对 **[付费](https://portal.personal-centurion.com/manage-subscription)** Centurion Portal 订阅开放；免费档不可用——请 [升级订阅](https://portal.personal-centurion.com/manage-subscription) 后解锁。
...
运行 `centurion model` 并选择 Centurion Portal 作为提供商时，Centurion 会主动询问是否启用 Tool Gateway：
...
在 `~/.centurion/config.yaml` 中直接设置 `use_gateway`：
...
网关使用 Centurion Portal 凭据认证（在 `centurion model` 完成后写入 `~/.centurion/auth.json`）。
...
在 `centurion tools` 中选择非网关提供商时，`use_gateway` 会自动设为 `false`，避免配置自相矛盾。
...
若使用自建或自定义网关，可在 `~/.centurion/.env` 中用环境变量覆盖端点：
...
经网关路由的工具会停止工作，直到你 [续订](https://portal.personal-centurion.com/manage-subscription) 或通过 `centurion tools` 改回直连 Key。
...
Modal（无服务器终端后端）可作为 Nous 订阅的可选附加能力，但**不会**由 Tool Gateway 安装向导一并打开——请单独通过 `centurion setup terminal` 或在 `config.yaml` 中配置。
