...
| `--tui` | 关闭 | 启用浏览器内 Chat 标签页（通过 PTY/WebSocket 嵌入 `centurion --tui`）。也可设置 `HERMES_DASHBOARD_TUI=1`。 |
...
Chat 标签页在普通 `centurion dashboard` 启动时默认关闭。如需嵌入式浏览器聊天面板，请使用 `centurion dashboard --tui` 启动，或设置 `HERMES_DASHBOARD_TUI=1`。
