#!/usr/bin/env python3
"""
SQLite State Store for Centurion AI OS.
...
DEFAULT_DB_PATH = get_centurion_home() / "state.db"
...
class SessionDB:
    ...
    # With multiple centurion processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``centurion_state._set_last_init_error(None)`` explicitly.
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Centurion, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Centurion session.

        A Centurion session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Centurion session is already bound to any Telegram DM topic.
