...
own pages inject it via ``window.__CENTURION_SESSION_TOKEN__`` so logged-in
...
from centurion_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from centurion_cli import web_server as _ws
    except Exception:
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from centurion_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``CENTURION_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from centurion_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``centurion kanban
...
def list_diagnostics(
    ...
    AND for ``centurion kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``centurion kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``centurion kanban specify <task_id>``.
    ...
    try:
        os.environ["CENTURION_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        # Import lazily so a missing auxiliary client at import time
        # doesn't break plugin load.
        from centurion_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``centurion kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def _active_profile_name() -> str:
    """Return the current Centurion profile name for notify-sub ownership."""
    try:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``centurion profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from centurion_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``centurion kanban decompose <task_id>``.
    ...
    try:
        os.environ["CENTURION_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        from centurion_cli import kanban_decompose  # noqa: WPS433 (intentional)
