/**
 * Centurion Kanban — Dashboard Plugin
 *
...
(function () {
  ...
  const COLUMN_DOT = {
    triage: "centurion-kanban-dot-triage",
    todo: "centurion-kanban-dot-todo",
    ready: "centurion-kanban-dot-ready",
    running: "centurion-kanban-dot-running",
    blocked: "centurion-kanban-dot-blocked",
    done: "centurion-kanban-dot-done",
    archived: "centurion-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-centurion-task";
  ...
  const DOCS_URL = "https://centurion-agent.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://centurion-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "centurion.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="centurion-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "centurion-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "centurion-kanban-md",
  ...
  // standard `drop` event and our `centurion-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("centurion-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("centurion-kanban-column--drop");
          if (target) target.classList.add("centurion-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("centurion-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("centurion-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("centurion-kanban:drop", {
  ...
  function KanbanPage() {
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "centurion-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "centurion-kanban-attention",
        "centurion-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "centurion-kanban-attention-bar" },
        h("span", { className: "centurion-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "centurion-kanban-attention-text" },
          diagTasks.length === 1
        ...
        h("button", {
          className: "centurion-kanban-attention-toggle",
          onClick: function () { setExpanded(function (x) { return !x; }); },
          type: "button",
        }, expanded ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
        h("button", {
          className: "centurion-kanban-attention-dismiss",
      ...
      expanded
        ? h("div", { className: "centurion-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "centurion-kanban-attention-row",
                  "centurion-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "centurion-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "centurion-kanban-attention-row-id" }, task.id),
                h("span", { className: "centurion-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "centurion-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "centurion-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "centurion-kanban-diag-action-btn",
      action.suggested ? "centurion-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " centurion-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".centurion-kanban-drawer-comment-row input, .centurion-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "centurion-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("centurion-kanban-diag", sevClass) },
      h("div", { className: "centurion-kanban-diag-header" },
        h("span", { className: "centurion-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "centurion-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "centurion-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "centurion-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "centurion-kanban-diag-data-row" },
                  h("span", { className: "centurion-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "centurion-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "centurion-kanban-diag-data-row" },
                h("span", { className: "centurion-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "centurion-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "centurion-kanban-diag-reassign-row" },
            h("span", { className: "centurion-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "centurion-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "centurion-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "centurion-kanban-diag-msg",
              msg.ok ? "centurion-kanban-diag-msg--ok" : "centurion-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "centurion-kanban-section" },
      h("div", { className: "centurion-kanban-section-head-row" },
        h("span", { className: "centurion-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "centurion-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "centurion-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "centurion-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "centurion-kanban-docs-link",
      title: "Open Centurion Kanban docs in a new tab",
      "aria-label": "Centurion Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `centurion kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "centurion-kanban-msg-ok" : "centurion-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "centurion-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "centurion-kanban-boardswitcher" },
      h("div", { className: "centurion-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "centurion-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "centurion-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "centurion-kanban-dialog-title" },
        ...
        h("div", { className: "centurion-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Centurion profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "centurion-kanban-bulk" },
      h("span", { className: "centurion-kanban-bulk-count" },
      ...
      h("div", { className: "centurion-kanban-bulk-priority",
      ...
      h("div", { className: "centurion-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Centurion profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "centurion-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("centurion-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("centurion-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "centurion-kanban-trash",
        dragOver ? "centurion-kanban-trash--drop" : "",
        props.draggingTaskId ? "centurion-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "centurion-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "centurion-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".centurion-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "centurion-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("centurion-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("centurion-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "centurion-kanban-column",
        dragOver ? "centurion-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "centurion-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "centurion-kanban-col-check",
        ...
        h("span", { className: cn("centurion-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "centurion-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "centurion-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "centurion-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "centurion-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "centurion-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "centurion-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "centurion-kanban-lane" },
                  h("div", { className: "centurion-kanban-lane-head" },
                    h("span", { className: "centurion-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "centurion-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "centurion-kanban-card--stale-red";
    if (age >= tier.amber) return "centurion-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".centurion-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "centurion-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "centurion-kanban-card",
        props.selected ? "centurion-kanban-card--selected" : "",
        props.failed ? "centurion-kanban-card--failed" : "",
        props.draggingSource ? "centurion-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "centurion-kanban-card-content" },
          h("div", { className: "centurion-kanban-card-row" },
            h("label", {
              className: "centurion-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "centurion-kanban-card-check",
            ...
            h("span", { className: "centurion-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or centurion kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "centurion-kanban-warning-badge",
                    "centurion-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
              ...
              : null,
            t.priority > 0
              ? h(Badge, { className: "centurion-kanban-priority",
                           title: `Priority ${t.priority}. Higher-priority tasks are claimed first by the dispatcher.` }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "centurion-kanban-tag",
                           title: `Tenant: ${t.tenant}. Free-form tag for grouping tasks (customer, project, team).` }, t.tenant)
              : null,
            progress
              ? h("span", {
                  className: cn(
                    "centurion-kanban-progress",
                    progress.done === progress.total ? "centurion-kanban-progress--full" : "",
                  ),
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "centurion-kanban-needs-assignee",
          ...
          h("div", { className: "centurion-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "centurion-kanban-card-row centurion-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "centurion-kanban-assignee",
                            title: `Assigned to Centurion profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "centurion-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "centurion-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "centurion-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "centurion-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Centurion profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Centurion profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "centurion-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "centurion-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "centurion-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "centurion-kanban-drawer-close",
        ...
        data ? h("div", { className: "centurion-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "centurion-kanban-drawer-body" },
      h("div", { className: "centurion-kanban-drawer-title" },
        h("span", { className: cn("centurion-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "centurion-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "centurion-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "centurion-kanban-section" },
        h("div", { className: "centurion-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "centurion-kanban-section" },
        h("div", { className: "centurion-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "centurion-kanban-comment" },
                h("div", { className: "centurion-kanban-comment-head" },
                  h("span", { className: "centurion-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "centurion-kanban-comment-ago" },
      ...
      h("div", { className: "centurion-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "centurion-kanban-event",
              isDiag ? "centurion-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "centurion-kanban-event-header" },
                  h("span", { className: "centurion-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "centurion-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "centurion-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "centurion-kanban-event-header-plain" },
                  h("span", { className: "centurion-kanban-event-kind" }, e.kind),
                  h("span", { className: "centurion-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "centurion-kanban-event-phantom-row" },
                  h("span", { className: "centurion-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "centurion-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "centurion-kanban-event-payload" },
    ...
    return h("div", { className: "centurion-kanban-section" },
      h("div", { className: "centurion-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "centurion-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `centurion-kanban-run--${r.outcome || r.status || "ended"}`
          : "centurion-kanban-run--active";
        return h("div", { key: r.id, className: cn("centurion-kanban-run", outcomeClass) },
          h("div", { className: "centurion-kanban-run-head" },
            h("span", { className: "centurion-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "centurion-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "centurion-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "centurion-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "centurion-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "centurion-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "centurion-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "centurion-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "centurion-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "centurion-kanban-pre centurion-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "centurion-kanban-meta-row" },
      h("span", { className: "centurion-kanban-meta-label" }, props.label),
      h("span", { className: "centurion-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "centurion-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "centurion-kanban-meta-row" },
        h("span", { className: "centurion-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "centurion-kanban-meta-value centurion-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "centurion-kanban-meta-row" },
        h("span", { className: "centurion-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "centurion-kanban-meta-value centurion-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "centurion-kanban-section" },
      h("div", { className: "centurion-kanban-section-head-row" },
        h("span", { className: "centurion-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "centurion-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "centurion-kanban-section" },
      h("div", { className: "centurion-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "centurion-kanban-deps-row" },
        h("span", { className: "centurion-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "centurion-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "centurion-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "centurion-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "centurion-kanban-dep-chip-x",
      ...
      h("div", { className: "centurion-kanban-deps-row" },
        h("span", { className: "centurion-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "centurion-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "centurion-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "centurion-kanban-msg-ok"
          : "centurion-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "centurion-kanban-section" },
      ...
      h("div", { className: "centurion-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "centurion-kanban-home-sub centurion-kanban-home-sub--on"
              : "centurion-kanban-home-sub",
          }, label);
        })
      )
    );
