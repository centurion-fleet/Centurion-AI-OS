...
Config (env vars or centurion config.yaml under retaindb:):
...
class _Client:
    ...
    def _headers(self, path: str) -> dict:
        ...
        h = {
            ...
            "x-sdk-runtime": "centurion-plugin",
        }
    ...
    def upload_file(self, data: bytes, filename: str, remote_path: str, mime_type: str, scope: str, project_id: str | None) -> dict:
        ...
        token = self.api_key.replace("Bearer ", "").strip()
        headers = {"Authorization": f"Bearer {token}", "x-sdk-runtime": "centurion-plugin"}
    ...
    def read_file_content(self, file_id: str) -> bytes:
        ...
        resp = requests.get(url, headers={"Authorization": f"Bearer {token}", "x-sdk-runtime": "centurion-plugin"}, timeout=30, allow_redirects=True)
        resp.raise_for_status()
        return resp.content
...
class RetainDBMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Project resolution: RETAINDB_PROJECT > centurion-<profile> > "default"
        ...
        if explicit:
            ...
            project = f"centurion-{profile_name}" if (profile_name and profile_name not in {"", ".centurion"}) else "default"
