...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/centurion>
...
```bash
centurion honcho setup    # full interactive wizard (cloud or local)
centurion memory setup    # generic picker, also works
```

Or manually:
```bash
centurion config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CENTURION_HOME/honcho.json` | Profile-local (isolated Centurion instances) |
| 2 | `~/.centurion/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Centurion profile: `centurion` (default) or `centurion.<profile>`.
...
| 4 | `per-session` strategy | Centurion session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`centurion-ai-os`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`centurion`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-centurion-ai-os`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening centurion in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Centurion session ID (timestamp + hex). Every `centurion` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Centurion profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`centurion`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Centurion profile: `centurion` (default) or `centurion.<profile>` (e.g. `centurion -p coder` → host key `centurion.coder`).
...
| `CENTURION_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `centurion honcho setup` | Full interactive setup wizard |
| `centurion honcho status` | Show resolved config for active profile |
| `centurion honcho enable` / `disable` | Toggle Honcho for active profile |
| `centurion honcho mode <mode>` | Change recall or observation mode |
| `centurion honcho peer --user <name>` | Update user peer name |
| `centurion honcho peer --ai <name>` | Update AI peer name |
| `centurion honcho tokens --context <N>` | Set context token budget |
| `centurion honcho tokens --dialectic <N>` | Set dialectic max chars |
| `centurion honcho map <name>` | Map current directory to a session name |
| `centurion honcho sync` | Create host blocks for all Centurion profiles |
