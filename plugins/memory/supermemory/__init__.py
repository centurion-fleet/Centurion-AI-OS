...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `centurion memory setup`.
        # All other options are documented for $CENTURION_HOME/supermemory.json
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "centurion_memory", "target": target, "type": "explicit_memory"},
    ...
    def _tool_store(self, args: dict) -> str:
        ...
        metadata["source"] = "centurion_tool"
        try:
