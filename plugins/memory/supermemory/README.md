...
Config file: `$CENTURION_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `centurion` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `centurion-{identity}` → `centurion-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Centurion profile:

```json
{
  "container_tag": "centurion-{identity}"
}
```

For a profile named `coder`, this resolves to `centurion-coder`. The default profile resolves to `centurion-default`. Without `{identity}`, all profiles share the same container.
