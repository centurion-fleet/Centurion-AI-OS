"""`centurion meet node ...` subcommand tree.

Wired into the existing ``centurion meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``centurion meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="centurion-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``centurion meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             centurion meet node approve <name> ws://<host>:{args.port} {token}")
        try:
