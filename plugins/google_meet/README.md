...
Let the centurion agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where centurion runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `centurion meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `centurion meet` CLI |
...
| `cli.py` | `centurion meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CENTURION_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `centurion meet node {run,list,approve,remove,status,ping}` |
...
centurion meet install                                      # pip + Chromium
centurion meet setup                                        # preflight
centurion meet auth                                         # optional
centurion meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
centurion meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.centurion/.env
centurion meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
centurion meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
centurion meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.centurion/.env
centurion meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, centurion will **not** switch your system audio input automatically — the
...
centurion plugins enable google_meet
centurion meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
centurion meet node ping my-mac
