...
Auto-tracks and cleans up ephemeral files created during Centurion AI OS
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CENTURION_HOME` and `/tmp/centurion-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CENTURION_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CENTURION_HOME` or `/tmp/centurion-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CENTURION_HOME/disk-cleanup/` is itself excluded
- `$CENTURION_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
