"""
IRC Platform Adapter for Centurion AI OS.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Centurion agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "centurion-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Centurion AI OS")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Centurion AI OS shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: centurion_, centurion_1, centurion_2...
...
def check_requirements() -> bool:
    ...
    # centurion setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `centurion gateway setup` flow for the IRC platform.

    Lazy-imports ``centurion_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Centurion to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    nickname = prompt(
        "Bot nickname (e.g. centurion-bot)",
    ...
    channel = prompt(
        "Channel to join (e.g. #centurion — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.centurion/.env")
    print_info("Restart the gateway for changes to take effect: centurion gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "centurion-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "centurion-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Centurion AI OS (cron)")
