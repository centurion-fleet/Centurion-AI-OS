...
# ``model_tools`` import time, so ``centurion status``, ``centurion chat``, etc.
...
# ``centurion_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Centurion is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a centurion MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CENTURION_HOME}/google_chat_thread_counts.json so
        ...
        # they don't sit in the chat forever as "Centurion is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("CENTURION_HOME", str(_Path.home() / ".centurion"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CENTURION_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a centurion MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Centurion is thinking…' marker message.
        ...
        "Centurion is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Centurion is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Centurion is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Centurion is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Centurion is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``centurion setup``.
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. centurion-chat-events) and a Pull subscription.")
    ...
    print_info("Restart the gateway: centurion gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``centurion cron`` running as a
    separate process from ``centurion gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Centurion plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'centurionai-os[google_chat]'",
        ...
        # the "Centurion is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Centurion is thinking…' marker message "
