"""
Microsoft Teams platform adapter for Centurion AI OS.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Centurion"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        centurion_action = data.get("centurion_action", "")
        session_key = data.get("session_key", "")

        if not centurion_action or not session_key:
        ...
        choice = choice_map.get(centurion_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Centurion\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.centurion/.env")
