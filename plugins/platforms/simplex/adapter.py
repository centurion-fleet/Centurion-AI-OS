"""SimpleX Chat platform adapter (Centurion plugin).
...
This adapter ships as a Centurion platform plugin under
``plugins/platforms/simplex/``. The Centurion plugin loader scans the
...
discoverable and `centurion setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Centurion — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"centurion-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``centurion setup gateway`` → SimpleX.
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `centurion gateway status` without
