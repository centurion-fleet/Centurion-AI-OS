...
from centurion_cli.timeouts import (
...
_loaded_env_paths = load_centurion_dotenv(centurion_home=_centurion_home, project_env=_project_env)
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[centurion-os: tool call arguments were corrupted in this session and "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                ...
                source=self.platform or os.environ.get("CENTURION_SESSION_SOURCE", "cli"),
                model=self.model,
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Centurion' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from centurion_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``centurion chat -q``. In that mode,
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``CENTURION_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``CENTURION_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("CENTURION_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``CENTURION_API_CALL_STALE_TIMEOUT`` env var
        ...
        env_timeout = os.getenv("CENTURION_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        centurion-os #21444 for the symptom history.  The upstream backend
        ...
        return (
            ...
            "See centurion-os#21444 for symptom history."
        )
    ...
    def _redact_message_content(content):
        ...
        Respects ``CENTURION_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.centurion/sessions/session_{sid}.json``
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects CENTURION_REDACT_SECRETS via
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``CENTURION_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        as a method so tests can patch a single seam without reaching into
        the private ``_turn_failed_file_mutations`` state dict.
        """
        try:
            import os as _os
            env = _os.environ.get("CENTURION_FILE_MUTATION_VERIFIER")
            if env is not None:
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``centurion auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from centurion_cli.copilot_auth import resolve_copilot_token
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct Centurion Portal.
        """
        ...
        if (
            ...
            try:
                from centurion_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from centurion_cli.models import lmstudio_model_reasoning_options
