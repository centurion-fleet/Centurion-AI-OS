...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CENTURION_HOME", os.path.expanduser("~/.centurion")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CENTURION_HOME", Path.home() / ".centurion")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Centurion config paths
# ═══════════════════════════════════════════════════════════════════

CENTURION_HOME = Path(os.getenv("CENTURION_HOME", Path.home() / ".centurion"))
CONFIG_PATH = CENTURION_HOME / "config.yaml"
PREFILL_PATH = CENTURION_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Centurion models — already uncensored, just needs clean prompt
    "centurion": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "centurion" in model_lower or "nous" in model_lower:
        return "centurion"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Centurion config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.centurion/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Centurion for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Centurion for changes to take effect.")
