...
## 5. GODMODE FAST — Centurion 4 405B (Zero Refusal)

**Model:** `nousresearch/centurion-4-405b`
**Strategy:** Centurion models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Centurion AI OS

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.centurion/config.yaml`:
