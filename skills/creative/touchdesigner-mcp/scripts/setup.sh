...
CENTURION_HOME_DIR="${CENTURION_HOME:-$HOME/.centurion}"
CENTURION_CFG="${CENTURION_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Centurion config has twozero_td MCP entry ──
if [[ ! -f "$CENTURION_CFG" ]]; then
    echo -e " ${FAIL} Centurion config not found at ${CENTURION_CFG}"
    manual_steps+=("Create ${CENTURION_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$CENTURION_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Centurion config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Centurion config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$CENTURION_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${CENTURION_CFG} manually"); }
    manual_steps+=("Restart Centurion session to pick up config change")
fi
