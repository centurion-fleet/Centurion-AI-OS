...
## 5. Centurion Configuration

### Config location

`$CENTURION_HOME/config.yaml` (defaults to `~/.centurion/config.yaml` when `CENTURION_HOME` is unset)
...
Restart the Centurion session for changes to take effect. The MCP connection is
