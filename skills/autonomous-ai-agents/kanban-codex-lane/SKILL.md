...
description: Use when a Centurion Kanban worker wants to run Codex CLI as an isolated implementation lane while Centurion keeps ownership of task lifecycle, reconciliation, testing, and handoff.
version: 1.0.0
...
metadata:
  centurion:
    ...
    related_skills: [kanban-worker, codex, centurion-ai-os]
---
...
- A bounded diff can be evaluated by Centurion in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- Centurion can run the relevant tests itself after Codex exits.
...
1. Centurion owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Centurion board CLI as a substitute for the worker.
2. Centurion owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Centurion owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Centurion with the repo's canonical wrapper.
4. Centurion owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Centurion owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${CENTURION_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional Centurion edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Centurion can do the task directly.
...
Centurion owns the Kanban lifecycle; do not call Centurion kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Centurion reconciliation.
...
- Explicit statement: Centurion owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Centurion will run afterward.
...
Centurion must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Centurion.
...
- [ ] Centurion ran the canonical tests itself, using `scripts/run_tests.sh` for Centurion AI OS or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from Centurion-run tests.
- [ ] Accepted commits/diffs were applied to the Centurion-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "centurion"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct Centurion edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Centurion.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but Centurion writes board state.
...
- [ ] Centurion reviewed `git diff` and safety-sensitive files.
- [ ] Centurion ran canonical tests independently.
