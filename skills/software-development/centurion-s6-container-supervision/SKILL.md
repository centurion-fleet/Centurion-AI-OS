---
name: centurion-s6-container-supervision
description: Modify, debug, or extend the s6-overlay supervision tree inside the Centurion AI OS Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.
version: 1.0.0
author: Centurion Fleet
license: MIT
metadata:
  centurion:
    tags: [docker, s6, supervision, gateway, profiles]
    related_skills: [centurion-ai-os, centurion-agent-dev]
---

# Centurion s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the Centurion Docker image (something that should be supervised at every container start, like the dashboard)
- Diagnosing why a per-profile gateway isn't starting, restarting, or surviving `docker restart`
- Understanding why the container's CMD is `/opt/centurion/docker/main-wrapper.sh` and how leading-dash args reach the user's program
...
If you're just running the Centurion AI OS and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   ├── 01-centurion-setup                ← docker/stage2-hook.sh
│   │   ├── UID/GID remap
│   │   ├── chown /opt/data
│   │   ├── chown /opt/data/profiles (every boot)
│   │   ├── seed .env / config.yaml / SOUL.md
│   │   └── skills_sync.py
│   └── 02-reconcile-profiles          ← centurion_cli.container_boot
│       ├── chown /run/service (centurion-writable for runtime register)
│       └── walk $CENTURION_HOME/profiles/<name>/gateway_state.json
...
│   │   ├── type        ("longrun")
│   │   ├── run         ("#!/command/with-contenv sh ... exec s6-setuidgid centurion centurion -p coder gateway run")
│   │   ├── down        (marker — present means "registered but don't auto-start")
│   │   └── log/run     (s6-log → $CENTURION_HOME/logs/gateways/coder/current)
│   └── ...
│
└── CMD ("main program")               ← /opt/centurion/docker/main-wrapper.sh
    └── routes user args: bare exec | centurion subcommand | centurion (no args)
...
| Path | Role |
|---|---|
| `Dockerfile` | s6-overlay install + cont-init.d wiring + `ENTRYPOINT ["/init", "/opt/centurion/docker/main-wrapper.sh"]` |
| `docker/stage2-hook.sh` | The "old entrypoint logic" — UID remap, chown, seed, skills sync. Runs as cont-init.d/01-centurion-setup. |
...
| `centurion_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `centurion gateway start/stop/restart` and routes to s6 when running in a container. |
...
Trade-off: main centurion is unsupervised under s6. That exactly matches its behavior under tini (the pre-s6 image). Dashboard supervision is the only **new** guarantee — and per-profile gateways under `/run/service/` get full supervision.
...
2. Drop to centurion via `s6-setuidgid centurion` at the top of run (unless you specifically need root).
...
Edit `S6ServiceManager._render_run_script` in `centurion_cli/service_manager.py`. The function is also called by `centurion_cli/container_boot.py::_register_service` during boot reconciliation, so it's the single source of truth. Update the corresponding assertion in `tests/centurion_cli/test_service_manager.py::test_s6_register_creates_service_dir_and_triggers_scan`.
...
`/command/` (where s6-overlay puts its binaries) is on PATH only for processes spawned by the supervision tree — services, cont-init.d, main-wrapper.sh. `docker exec <c> s6-svstat …` will fail with "command not found"; always use the absolute path `/command/s6-svstat`. The `centurion` binary works because the Dockerfile adds `/opt/centurion/.venv/bin` to the runtime `ENV PATH`.

### Profile directory ownership

The cont-init reconciler runs as centurion (`s6-setuidgid centurion` in `02-reconcile-profiles`). If a profile dir ends up root-owned (e.g. because `docker exec <c> centurion profile create …` ran as root by default), the reconciler can't read SOUL.md and fails with `PermissionError`. Mitigation: `stage2-hook.sh` chowns `$CENTURION_HOME/profiles` to centurion on **every** boot, idempotently. Don't remove that block.

### Files written by `docker exec` are root-owned

`docker exec` defaults to root. Either pass `--user centurion` or rely on the stage2 chown sweep next reboot. Don't write files under `$CENTURION_HOME/profiles/<name>/` as root manually — the next reconcile pass will sweep them but in-flight operations may hit perm errors.
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `centurion -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `centurion profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
...
- `centurion-agent-dev`: General centurion-ai-os codebase navigation
- `centurion-tool-quirks`: Specific Centurion-tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with centurion built-in tools.
