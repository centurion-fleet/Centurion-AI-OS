#!/usr/bin/env python3
"""Google Workspace API CLI for Centurion AI OS.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Centurion-facing JSON contract and falls back to the Python client
...
from _centurion_home import get_centurion_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CENTURION_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Centurion AI OS")
