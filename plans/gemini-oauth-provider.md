...
## Client ID
- Need to register a "Desktop app" OAuth client on a Centurion Fleet GCP project
...
## Token Lifecycle
- Store at `~/.centurion/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `centurion_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `centurion_cli/models.py` — Add Gemini model catalog
4. `centurion_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `centurion_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `centurion_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- centurion-ai-os Copilot OAuth: `centurion_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
