"""Persistent slash-command worker — one CenturionCLI per TUI session.
...
def _run(cli: CenturionCLI, command: str) -> str:
...
def main():
    ...
    os.environ["CENTURION_SESSION_KEY"] = args.session_key
    os.environ["CENTURION_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = CenturionCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
