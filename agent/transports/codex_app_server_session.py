...
Owns one Codex thread per Centurion session. Drives `turn/start`, consumes
...
# Centurion' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_CENTURION_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Centurion/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Centurion session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _CENTURION_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("CENTURION_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            ...
            client_title="Centurion AI OS",
            client_version=_get_centurion_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Centurion' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Centurion' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own centurion-tools callback we
            # auto-accept — the user already approved Centurion' tools
            ...
            if server_name == "centurion-tools":
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Centurion approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Centurion returns 'once', 'session', 'always', or 'deny'.
...
def _get_centurion_version() -> str:
    """Best-effort Centurion version string for codex's userAgent line."""
    try:
