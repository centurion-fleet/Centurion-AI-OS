...
projection into Centurion' display, approval bridging, transcript projection into
...
"codex_app_server"`. Centurion' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        if spawn_env.get("CENTURION_KANBAN_TASK"):
            kanban_db = spawn_env.get("CENTURION_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "CENTURION_KANBAN_ROOT",
                    os.path.join(
    ...
    def initialize(
        ...
        client_title: str = "Centurion AI OS",
