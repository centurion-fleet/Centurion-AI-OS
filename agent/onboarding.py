...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Centurion starts and finds ``~/.openclaw/``.

    Points users at ``centurion claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``centurion claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Centurion, run "
        "`centurion claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `centurion claw cleanup` (renames it to ~/.openclaw.pre-migration — "
