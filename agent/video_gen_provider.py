...
as ``kind: backend``) or ``~/.centurion/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``centurion tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CENTURION_HOME/cache/videos/``, creating parents as needed."""
    from centurion_constants import get_centurion_home

    path = get_centurion_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CENTURION_HOME/cache/videos/``.
