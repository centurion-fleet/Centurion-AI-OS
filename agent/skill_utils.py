...
from centurion_constants import get_config_path, get_skills_dir, is_termux
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``CENTURION_PLATFORM`` or
            ``CENTURION_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("CENTURION_PLATFORM")
...
# which becomes the dominant cost of ``centurion`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.centurion/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``centurion`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against CENTURION_HOME, not cwd
        if not p.is_absolute():
            p = (centurion_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.centurion/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    centurion = metadata.get("centurion") or {}
    if not isinstance(centurion, dict):
        centurion = {}
    return {
        "fallback_for_toolsets": centurion.get("fallback_for_toolsets", []),
        "requires_toolsets": centurion.get("requires_toolsets", []),
        "fallback_for_tools": centurion.get("fallback_for_tools", []),
        "requires_tools": centurion.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    centurion = metadata.get("centurion")
    if not isinstance(centurion, dict):
        return []
    raw = centurion.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Centurion metadata, VCS, virtualenv/dependency, and cache
