...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Centurion with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.centurion/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Centurion
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``centurion tools``.
