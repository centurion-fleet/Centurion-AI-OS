...
Storage
-------
Bundles live in ``~/.centurion/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``CENTURION_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``centurion bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under CENTURION_HOME.

    Honors ``CENTURION_BUNDLES_DIR`` for tests; falls back to
    ``<CENTURION_HOME>/skill-bundles``.
    """
    override = os.environ.get("CENTURION_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_centurion_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `centurion bundles` CLI subcommand.
# ---------------------------------------------------------------------------
