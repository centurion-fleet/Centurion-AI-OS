"""Anthropic Messages API adapter for Centurion AI OS.

Translates between Centurion's internal OpenAI-style message format and
...
# Centurion effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Centurion historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Centurion)
    ...
    # 1. Centurion-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Centurion configs.
...
# ── Centurion-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.centurion/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_CENTURION_OAUTH_FILE = get_centurion_home() / ".anthropic_oauth.json"
...
def run_centurion_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Centurion-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Centurion with your Claude Pro/Max subscription.")
    print()
...
def read_centurion_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Centurion-managed OAuth credentials from ~/.centurion/.anthropic_oauth.json."""
    if _CENTURION_OAUTH_FILE.exists():
        try:
            data = json.loads(_CENTURION_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Centurion OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See centurion-os#13848.
    #
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See centurion-os#13848 (Kimi) and
    centurion-os#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Centurion AI OS", "Claude Code")
                text = text.replace("Centurion agent", "Claude Code")
                text = text.replace("centurion-os", "claude-code")
                text = text.replace("Centurion Fleet", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Centurion surfaces in its CLI. We
