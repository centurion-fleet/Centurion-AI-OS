...
upstream prefix cache warm.  See ``centurion-os-dev``'s
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Centurion never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CENTURION_HOME persona while keeping
    ...
    # Pointer to the centurion-os skill + docs for user questions about Centurion itself.
    stable_parts.append(CENTURION_AGENT_HELP_GUIDANCE)
    ...
    # CENTURION_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Active-profile hint — names the Centurion profile the agent is running
    # under so it doesn't conflate ~/.centurion/skills/ (default profile) with
    # ~/.centurion/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Centurion profile: default. Other profiles (if any) live "
            "under ~/.centurion/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Centurion profile: {active_profile}. This session reads "
            f"and writes ~/.centurion/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.centurion/skills/, ~/.centurion/plugins/, "
            f"~/.centurion/cron/, ~/.centurion/memories/ — those belong to a "
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the centurion-os install
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Centurion never rebuilds or reinjects parts of it
