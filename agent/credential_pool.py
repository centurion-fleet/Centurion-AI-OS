...
from centurion_cli.config import get_env_value, load_env
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``centurion model`` / ``centurion auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Centurion process
    ...
    def _sync_nous_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        avoiding a "refresh token reuse" revocation on the Centurion Portal.
        """
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``centurion`` invocation that defaults to the active provider
        (e.g. setup wizard, ``centurion auth status``) would land on
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("centurion_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Centurion
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("centurion_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Centurion process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Centurion profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `centurion model` / `centurion auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `centurion model` ->
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `centurion auth remove <provider> <N>` is stable across all source types.
    try:
        from centurion_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Centurion PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `centurion setup` ("Claude
        ...
        # Claude Code CLI, centurion_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.centurion/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `centurion setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"centurion_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_centurion_oauth_credentials

        for source_name, creds in (
            ("centurion_pkce", read_centurion_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `centurion auth add nous --label <name>`).  Fall back to the
        ...
        # Centurion auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from centurion_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.centurion/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `centurion auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `centurion auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Centurion auth store.  Without this gate the removal is instantly
        ...
        # Centurion owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `centurion auth openai-codex`.
        ...
        # When the user logs in via ``centurion model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``centurion auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            from centurion_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.centurion/.env over os.environ — the user's config file is the
    # authoritative source for Centurion credentials. Stale env vars from parent
    ...
    # Honour user suppression — `centurion auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.centurion/.env.
    ...
    try:
        from centurion_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from centurion_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.centurion/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # Centurion PKCE is Centurion-owned/persistable while present, but it is
            # still a file-backed singleton and should disappear from the pool
            # when the backing OAuth file is gone.
            or entry.source == "centurion_pkce"
        )
    ]
    if len(retained) == len(entries):
        return False
