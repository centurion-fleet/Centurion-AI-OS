...
> [centurion-fleet/Centurion-AI-OS#30136](https://github.com/centurion-fleet/Centurion-AI-OS/pull/30136)
...
> (`git log --oneline a957ef083..a6f7171a5 -- 'docker/*' 'centurion_cli/service_manager.py' …`).
> Open Questions are collapsed into a single Decision Log table; full
> deliberations live in PR review comments.

**Goal:** Replace `tini` with s6-overlay as PID 1 in the Centurion Docker image so
that the main centurion process, the dashboard, and dynamically-created
...
s6-svscan as PID 1. Main centurion and the dashboard are declared as static
s6-rc services at image build time. Per-profile gateways — which users create
*after* the image is built (`centurion profile create coder` →
...
- Existing systemd/launchd/windows surface in `centurion_cli/gateway.py` and
  `centurion_cli/gateway_windows.py`.
...
- Interactive TUI must keep working:
  `docker run -it --rm centurion-fleet/centurion-ai-os:latest --tui`.
- Dynamic registration is limited to per-profile gateways — one service per
  ...
  `gateway-default` slot is always registered for the root CENTURION_HOME
  profile so `centurion gateway start` (no `-p`) has somewhere to land.
...
`ENTRYPOINT [ "/usr/bin/tini", "-g", "--", "/opt/centurion/docker/entrypoint.sh" ]`.
tini was PID 1, reaped zombies, forwarded SIGTERM to the process group. The
old `docker/entrypoint.sh`:

1. `gosu` privilege drop from root → `centurion` UID.
...
All init-system logic lives in **`centurion_cli/gateway.py`** (~5,400 LOC at
re-validation). The systemd/launchd code is ~1,500 lines of that, plus a
separate **`centurion_cli/gateway_windows.py`** (~690 LOC) for Windows
Scheduled Tasks.
...
- `centurion_cli/status.py` — gained an `s6` branch for in-container runs.
- `centurion_cli/profiles.py` — `create_profile` / `delete_profile` register and
  unregister with s6 inside the container (no-op on host).
- `centurion_cli/doctor.py` — `_check_gateway_service_linger` skips on s6, and a
  new "Service Supervisor" section reports main-centurion / dashboard /
  profile-gateway counts via the ServiceManager.
- `centurion_cli/gateway.py::gateway_command` — the
...
`centurion gateway start`, `coder gateway start` (profile alias), and
`centurion -p <profile> gateway start` all spawn a gateway process scoped to a
given profile. See
[Profiles: Running Gateways](https://github.com/centurion-fleet/Centurion-AI-OS/docs/user-guide/profiles#running-gateways).
On host, lifecycle is managed via per-profile systemd units
(`centurion-gateway-<profile>.service`); inside the container, an s6 service at
...
`centurion_cli/container_boot.py`, and auto-starts those whose last recorded
...
- **Root/non-root model:** `/init` runs as root to set up the supervision
  ...
  `s6-setuidgid centurion` in its `run` script. The per-service `s6-supervise`
  monitor stays root so it can signal its child regardless of UID. Net
  effect: centurion and all its subprocesses run as UID 10000 exactly as
...
- `/command/with-contenv` sources `/run/s6/container_environment/*` into
  service env — convenient for passing `CENTURION_HOME` etc.
...
- Zombie reaping: PID 1 (s6-svscan) reaps all zombies non-blockingly on
  SIGCHLD. Any subagent subprocess spawned by the main centurion process is
...
Container ENTRYPOINT is `/init`, PID 1 is s6-svscan. The main centurion
...
### D2. Main centurion is an s6 service with container-exit semantics

The contract "container exits when `centurion` exits" is preserved via a
...
|---|---|
| (no args) | `centurion` with no args, container exits when centurion exits |
| `chat -q "..."` | `centurion chat -q "..."`, container exits with centurion exit code |
...
- **s6-rc** (declarative, compile-then-swap): used for main centurion and the
...
(tmpfs, centurion-writable). s6-svscan picks them up on rescan.
...
(`centurion -p <profile> gateway run`, or `centurion gateway run` for the default
...
and silently failed for the unprivileged centurion user (UID 10000), making
...
Existing per-profile `centurion -p <profile> gateway start/stop/restart` CLI
...
`centurion gateway start` (no `-p`) targets a special `gateway-default` slot
that's always registered by the cont-init reconciler. Its run script omits
the `-p` flag and runs against the root `$CENTURION_HOME` profile.

`--all` lifecycle (`centurion gateway stop --all`, `... restart --all`)
...
main-centurion service so the centurion CMD becomes that main program.
...
_is_tui_invocation() {
    ...
    case "${CENTURION_TUI:-}" in 1|true|TRUE|yes) return 0 ;; esac
    if [ -t 0 ] && [ $# -eq 0 ]; then return 0; fi
    return 1
}
```

And in `docker/s6-rc.d/main-centurion/run`:
...
| Profile gateway crash loop masks a real config error | Low | Medium | s6 `finish` script `max_restarts` cap (planned follow-up); operators see crash-looping logs in `$CENTURION_HOME/logs/gateways/<profile>/` |
...
| `docker restart` silently loses per-profile gateway registrations (tmpfs scandir wiped) | High (without mitigation) | High | Cont-init reconciliation re-registers from persistent `$CENTURION_HOME/profiles/` and auto-starts those last seen `running`; outcome recorded to `$CENTURION_HOME/logs/container-boot.log` (size-bounded, rotates to `.1` at 256 KiB) |
| A `running` gateway that's actually broken auto-restarts into a crash loop after every container restart | Low | Medium | s6 `finish` script `max_restarts` cap (planned); follow-up: `centurion doctor` alerts when N consecutive container restarts ended in `startup_failed` |
| `_s6_running()` detection works as root but silently fails for unprivileged centurion user, making runtime-registration path inert | High (without mitigation) | High | **Caught in PR review.** Detection now probes `/proc/1/comm` (world-readable) + `/run/s6/basedir`. Docker integration tests refactored to `docker exec -u centurion` so the realistic runtime user is exercised |
| `s6-svscanctl` from centurion hits EACCES on the root-owned control FIFO | Medium | Medium | `02-reconcile-profiles` chowns `/run/service/.s6-svscan/{control,lock}` to centurion after stage1 creates them |
| Per-service `supervise/control` FIFO is root-owned by s6-supervise, blocking `s6-svc` from centurion | Known | Medium | Surfaced cleanly as `S6CommandError` (with rc + stderr) instead of raw `CalledProcessError`. Permission fix tracked as a follow-up (small SUID helper, polling chown loop in cont-init.d, or replace `s6-svc` with `down`-marker manipulation) |
...
| OQ8 | Log persistence | `$CENTURION_HOME/logs/gateways/<profile>/`. The log path is sourced from runtime `$CENTURION_HOME` via `with-contenv`, NOT Python-substituted at registration time |
...
- **`gateway-default` slot:** always registered by the reconciler so
  `centurion gateway start` (no `-p`) has somewhere to land.
...
- [x] `docker run -it --rm centurion-ai-os --tui` starts the Ink TUI with
      working keyboard input, cursor control, and resize (SIGWINCH)
- [x] Dashboard crashes are recovered by s6 within ~2s
- [x] `centurion profile create test` inside a container creates
      `/run/service/gateway-test/`
- [x] `centurion -p test gateway start` inside a container dispatches through s6
- [x] `centurion -p test gateway stop` inside a container cleanly stops via s6
- [x] `centurion profile delete test` inside a container removes
      `/run/service/gateway-test/`
- [x] Profile gateway logs persist at
      `$CENTURION_HOME/logs/gateways/test/current`
- [x] `centurion status` inside the container shows `Manager: s6`
- [x] `centurion gateway start` (no `-p`) inside a container targets
      `gateway-default` and runs against the root profile
- [x] `centurion gateway stop --all` / `... restart --all` iterate every
...
- [x] `centurion gateway install/start/stop` on Linux host and macOS host
