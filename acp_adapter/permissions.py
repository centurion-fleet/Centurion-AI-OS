"""ACP permission bridging for Centurion dangerous-command approvals."""
...
def _build_permission_options(*, allow_permanent: bool) -> list[PermissionOption]:
    """Return ACP options that match Centurion approval semantics."""
    options = [
        PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once"),
        PermissionOption(
            option_id="allow_session",
            # ACP has no session-scoped kind, so use the closest persistent
            # hint while keeping Centurion semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_centurion(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Centurion approval strings."""
...
def make_approval_callback(
    ...
    """
    Return a Centurion-compatible approval callback that bridges to ACP.
    ...
    def _callback(
        ...
        **_: object,
        ...
        return _map_outcome_to_centurion(
            response.outcome,
