"""ACP auth helpers — detect and advertise Centurion authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "centurion-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Centurion runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Centurion can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Centurion.
    ...
    not have Centurion provider credentials configured yet, so Centurion always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Centurion using the currently configured "
