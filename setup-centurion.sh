...
# 5. Symlinks the 'centurion' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Centurion AI OS Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/centurion-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink centurion into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up centurion command..."

CENTURION_BIN="$SCRIPT_DIR/venv/bin/centurion"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$CENTURION_BIN" "$COMMAND_LINK_DIR/centurion"
echo -e "${GREEN}✓${NC} Symlinked centurion → $COMMAND_LINK_DISPLAY_DIR/centurion"
...
echo "Other commands:"
echo "  centurion status        # Check configuration"
if is_termux; then
    echo "  centurion gateway       # Run gateway in foreground"
else
    echo "  centurion gateway install # Install gateway service (messaging + cron)"
fi
echo "  centurion cron list     # View scheduled jobs"
echo "  centurion doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m centurion_cli.main setup
fi
