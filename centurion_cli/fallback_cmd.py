"""
centurion fallback — manage the fallback provider chain.
...
Subcommands:
  centurion fallback [list]   Show the current fallback chain (default when no subcommand)
  centurion fallback add      Pick provider + model via the same picker as `centurion model`,
                           then append the selection to the chain
  centurion fallback remove   Pick an entry to delete from the chain
  centurion fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.centurion/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from centurion_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `centurion model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  centurion fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/centurion-fleet/Centurion-AI-OS/tree/main/website/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `centurion model`, then append the selection to the chain."""
    from centurion_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `centurion model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `centurion fallback list` to view, or `centurion fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``centurion fallback [subcommand]``."""
