...
Pure display functions with no CenturionCLI state dependency.
"""
...
from centurion_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE

CENTURION_LOGO = """[bold #FFD700] ██████╗███████╗███╗   ██╗████████╗██╗   ██╗██████╗ ██╗ ██████╗ ███╗   ██╗[/]
[bold #FFD700]██╔════╝██╔════╝████╗  ██║╚══██╔══╝██║   ██║██╔══██╗██║██╔═══██╗████╗  ██║[/]
[#FFBF00]██║     █████╗  ██╔██╗ ██║   ██║   ██║   ██║██████╔╝██║██║   ██║██╔██╗ ██║[/]
[#FFBF00]██║     ██╔══╝  ██║╚██╗██║   ██║   ██║   ██║██╔══██╗██║██║   ██║██║╚██╗██║[/]
[#CD7F32]╚██████╗███████╗██║ ╚████║   ██║   ╚██████╔╝██║  ██║██║╚██████╔╝██║ ╚████║[/]
...
# Back-compat aliases for skins/tests that reference the old names
CENTURION_AGENT_LOGO = CENTURION_LOGO
CENTURION_CADUCEUS = CENTURION_EAGLE
...
# (e.g. nix-built centurion — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/centurion-fleet/Centurion-AI-OS.git"
...
def check_for_updates() -> Optional[int]:
    """Check whether a Centurion update is available.

    Two paths: if ``CENTURION_REVISION`` is set (nix builds embed it), compare
    ...
    embedded_rev = os.environ.get("CENTURION_REVISION") or None
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Centurion git checkout, or None if this isn't a git install.
...
_RELEASE_URL_BASE = "https://github.com/centurion-fleet/Centurion-AI-OS/releases/tag"
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else CENTURION_EAGLE
    except Exception:
        _bskin = None
        _hero = CENTURION_EAGLE
    left_lines = ["", _hero, ""]
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Centurion AI OS[/]")

    if os.getenv("CENTURION_YOLO_MODE"):
    ...
    try:
        from centurion_cli.codex_runtime_switch import get_current_runtime
        from centurion_cli.config import load_config as _load_cfg
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from centurion_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built centurion; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else CENTURION_LOGO
        console.print(_logo)
        console.print()
