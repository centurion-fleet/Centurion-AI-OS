...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or centurion -c.",
    ...
    "Ctrl+Z suspends Centurion to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "centurion -c resumes your most recent CLI session. centurion -c \"project name\" resumes by title.",
    "centurion -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "centurion -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "centurion chat -t web,terminal enables only specific toolsets for a focused session.",
    "centurion chat -s github-pr-workflow preloads a skill at launch.",
    "centurion chat -q \"query\" runs a single non-interactive query and exits.",
    "centurion chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "centurion chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "centurion --yolo bypasses all dangerous command approval prompts for the entire session.",
    "centurion chat --source telegram tags the session for filtering in centurion sessions list.",
    "centurion -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "centurion doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "centurion dump outputs a compact setup summary — great for bug reports.",
    "centurion config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "centurion config edit opens config.yaml in your default editor.",
    "centurion config check scans for missing or stale configuration options.",
    "centurion sessions browse opens an interactive session picker with search.",
    "centurion sessions stats shows session counts by platform and database size.",
    "centurion sessions prune --older-than 30 cleans up old sessions.",
    "centurion skills search react --source skills-sh searches the skills.sh public directory.",
    "centurion skills check scans installed hub skills for upstream updates.",
    "centurion skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "centurion skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "centurion mcp add github --command npx adds MCP servers from the command line.",
    "centurion mcp serve runs Centurion itself as an MCP server for other agents.",
    "centurion auth add lets you add multiple API keys for credential pool rotation.",
    "centurion completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "centurion logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "centurion backup creates a zip backup of your entire Centurion home directory.",
    "centurion profile create coder creates an isolated profile that becomes its own command.",
    "centurion profile create work --clone copies your current config and keys to a new profile.",
    "centurion update syncs new bundled skills to ALL profiles automatically.",
    "centurion gateway install sets up Centurion as a system service (systemd/launchd).",
    "centurion memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "centurion webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: centurion tools disables unused tools, centurion skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "centurion models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Centurion tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'centurion profile create coder' creates the 'coder' command.",
    "centurion profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Centurion prints a resume command with session ID and stats.",
    "centurion sessions export backup.jsonl exports all sessions for backup or analysis.",
    "centurion -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "centurion skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .centurion/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: centurion cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Centurion runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "centurion gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Centurion via a local macOS server.",
    ...
    "Context files (.centurion.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.centurion/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Centurion loads project context from .centurion.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — centurion tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.centurion/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "centurion chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "centurion plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "centurion status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.centurion/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Centurion your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.centurion/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Centurion auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.centurion/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at CENTURION_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "CENTURION_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in CENTURION_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.centurion/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'centurion profile create ls' would be rejected.",
    "centurion profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "centurion claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "The skills quarantine at ~/.centurion/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Centurion auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Centurion config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Centurion is working.',
    ...
    '/reload reloads ~/.centurion/.env into the running session — pick up new API keys without restarting.',
    ...
    'Drop a ~/.centurion/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'centurion curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "centurion curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'centurion curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'centurion auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'CENTURION_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "CENTURION_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'centurion dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'centurion dashboard --tui embeds the full Centurion TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.centurion/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.centurion/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.centurion/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.centurion/skills/ so drop-in skills appear without restarting the session.',
    ...
    'centurion -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'centurion chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'centurion chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'centurion chat --ignore-user-config skips ~/.centurion/config.yaml — reproducible bug reports and CI runs.',
    "centurion chat --source tool tags programmatic chats so they don't clutter centurion sessions list.",
    'centurion dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'centurion sessions rename <ID> "new title" renames any past session; centurion sessions delete <ID> removes one.',
    'centurion import restores a session export or profile archive produced by sessions export or profile export.',
    'centurion fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'centurion pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'centurion setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Centurion block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from centurion sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.centurion/dashboard-plugins/.',
]
