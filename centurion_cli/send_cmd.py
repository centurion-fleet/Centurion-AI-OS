"""CLI subcommand: ``centurion send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(f"centurion send: cannot read {file_path}: {exc}", file=sys.stderr)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"centurion send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"centurion send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"centurion send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"centurion send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `centurion gateway setup`, or run the gateway once so")
...
def _load_centurion_env() -> None:
    ...
    startup that ``centurion send`` must replicate when invoked standalone:
    ...
    intentionally reimplement the minimum needed here so ``centurion send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    if not target:
        print(
            "centurion send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  centurion send --to telegram \"hello\"\n"
            "  centurion send --to discord:#ops --file report.md\n"
            "  centurion send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "centurion send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `centurion send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Centurion "
        ...
        epilog=(
            ...
            "  centurion send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  centurion send --list                  # all platforms\n"
            "  centurion send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `centurion send --list telegram`.",
    )
