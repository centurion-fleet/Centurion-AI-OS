...
Pure logic: walks a Centurion config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `centurion doctor` and a future `centurion migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Centurion config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Centurion config and return retirement issues.
