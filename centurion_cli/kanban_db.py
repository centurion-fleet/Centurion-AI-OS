...
``<root>`` is the **shared Centurion root** (the parent of any active
...
``centurion -p <profile>`` joins the same board as the dispatcher that
...
the profile's Centurion home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``CENTURION_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``CENTURION_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``centurion kanban boards
...
``/opt/centurion``), ``<root>`` is ``CENTURION_HOME``. Legacy env-var
overrides still work:

* ``CENTURION_KANBAN_DB`` — pin the database file path directly.
* ``CENTURION_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``CENTURION_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``CENTURION_KANBAN_DB``,
``CENTURION_KANBAN_WORKSPACES_ROOT``, and ``CENTURION_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/centurion-kanban-v1-spec.pdf`` for the full
...
# ``CENTURION_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``CENTURION_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("CENTURION_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def kanban_home() -> Path:
    """Return the shared Centurion root that anchors the kanban board.

    Resolution order:

    1. ``CENTURION_KANBAN_HOME`` env var when set and non-empty (explicit
    ...
    profile's ``CENTURION_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("CENTURION_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``centurion kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``CENTURION_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``centurion kanban boards
    ...
    """
    env = os.environ.get("CENTURION_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``centurion kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``CENTURION_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("CENTURION_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``CENTURION_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("CENTURION_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``centurion kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``CENTURION_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    max_retries          INTEGER,
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``CENTURION_SESSION_ID``. NULL
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Centurion treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``CENTURION_KANBAN_DB`` env → ``CENTURION_KANBAN_BOARD`` env →
    ...
    try:
        ...
        with _INIT_LOCK:
            ...
            # See centurion_state._WAL_INCOMPAT_MARKERS for detection logic.
            from centurion_state import apply_wal_with_fallback
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``centurion kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``CENTURION_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from centurion_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``centurion --skills ...`` alongside the built-in
    ...
    # `centurion --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``centurion kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `centurion kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``centurion kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``centurion kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``centurion kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``centurion kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``CENTURION_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Centurion' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Centurion-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``centurion kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Centurion
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``centurion kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Centurion profile.
    ...
    if not rows:
        return False
    try:
        from centurion_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Centurion profile.
        # `_default_spawn` invokes ``centurion -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Centurion
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `centurion kanban tail` — without
...
def _module_centurion_argv() -> list[str]:
    """Return the interpreter-bound Centurion CLI invocation."""
...
def _absolute_centurion_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Centurion shim."""
...
def _centurion_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Centurion executable path.
...
def _resolve_centurion_argv() -> list[str]:
    ...
    1. ``$CENTURION_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("centurion")`` — the console-script shim, normalized to
       ...
       ``.\\centurion.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m centurion_cli.main`` — fallback for setups where
       Centurion is launched from a venv and the ``centurion`` shim is not on
    ...
    env_bin = os.environ.get("CENTURION_BIN", "").strip()
    if env_bin:
    ...
    centurion_bin = _safe_which_no_cwd("centurion") if _IS_WINDOWS else shutil.which("centurion")
    if centurion_bin:
        return _centurion_path_argv(centurion_bin)
    return _module_centurion_argv()
...
def _kanban_worker_skill_available(centurion_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``centurion -p <name>``), its ``SKILLS_DIR``
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``centurion -p <profile> chat -q ...`` subprocess.
    ...
    ``CENTURION_KANBAN_DB`` / ``CENTURION_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject CENTURION_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `centurion -p <name>` the
    # _apply_profile_override() runs *before* centurion_constants is imported.
    # If CENTURION_HOME is absent from the child's env, get_centurion_home() falls
    # back to Path.home() / ".centurion" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from centurion_cli.profiles import resolve_profile_env
    try:
        env["CENTURION_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via CENTURION_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # CENTURION_HOME never had profiles created.
        pass
    if task.tenant:
        env["CENTURION_TENANT"] = task.tenant
    env["CENTURION_KANBAN_TASK"] = task.id
    env["CENTURION_KANBAN_WORKSPACE"] = workspace
    if task.branch_name:
        env["CENTURION_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["CENTURION_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["CENTURION_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    # that even when the worker activates a profile (`centurion -p <name>`
    ...
    env["CENTURION_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["CENTURION_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["CENTURION_KANBAN_BOARD"] = resolved_board
    # CENTURION_PROFILE is the author the kanban_comment tool defaults to.
    # `centurion -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["CENTURION_PROFILE"] = profile_arg
    ...
    if _kanban_worker_skill_available(env.get("CENTURION_HOME")):
    ...
    # `centurion kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Centurion AI OS or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``centurion kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled CENTURION_PROFILE values like "centurion-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Centurion root exists
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``centurion kanban assignees`` for the terminal.
