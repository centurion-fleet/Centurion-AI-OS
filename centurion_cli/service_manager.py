...
``centurion gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``centurion gateway start/stop/restart``).
    """
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged centurion
    ...
    detection always fails. Since every Centurion runtime call inside the
    container drops to centurion via ``s6-setuidgid``, that silent failure
...
# active (set via the ``centurion -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from centurion_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from centurion_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from centurion_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from centurion_cli.gateway import _probe_systemd_service_running
...
class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    ...
    def start(self, name: str) -> None:
        from centurion_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from centurion_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from centurion_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from centurion_cli.gateway import _probe_launchd_service_running
...
class WindowsServiceManager(_RegistrationUnsupportedMixin):
    ...
    def is_running(self, name: str) -> bool:
        from centurion_cli import gateway_windows
        from centurion_cli.gateway import find_gateway_pids
...
#
# Per-profile gateways are registered dynamically when `centurion profile create`
# runs inside the container (Phase 4). Static services (main-centurion, dashboard)
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    (the ``centurion`` user — UID 10000 — running every Centurion runtime
    ...
    is dispatched as the centurion user.
    ...
    with centurion ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           centurion:centurion, 0755 (parent must already exist)
    ``svc_dir/event/``                     centurion:centurion, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 centurion:centurion, 0755
    ``svc_dir/supervise/event/``           centurion:centurion, 03730
    ``svc_dir/supervise/control``          centurion:centurion, 0660    (FIFO)
    ...
    centurion user reads them fine.
    ...
    slot's supervise/ was centurion-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _CENTURION_UID, _CENTURION_GID)
        except PermissionError:
            # Running as the centurion user already — directory is centurion-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _CENTURION_UID, _CENTURION_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is centurion-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _CENTURION_UID, _CENTURION_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``centurion -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`centurion profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-centurion, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources CENTURION_HOME (and any extra env) via with-contenv —
             so e.g. ``-e CENTURION_HOME=/data/centurion`` is honored at run
             time, not Python-substituted at registration time (OQ8-C).
          2. Activates the bundled venv.
          3. Drops to the centurion user and exec's
             ``centurion -p <profile> gateway run`` (or just ``centurion
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``centurion gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root CENTURION_HOME profile" (the implicit profile that exists at
        ...
        if profile == "default":
            lines.append("exec s6-setuidgid centurion centurion gateway run")
        else:
            lines.append(
                f"exec s6-setuidgid centurion centurion -p {shlex.quote(profile)} gateway run"
            )
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${CENTURION_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the CENTURION_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e CENTURION_HOME=/data/centurion``
        gets its logs under /data/centurion/logs/..., not the build-time
        default.
        """
        import shlex
        ...
        return (
            ...
            f': "${{CENTURION_HOME:=/opt/data}}"\n'
            f'log_dir="$CENTURION_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            f'chown -R centurion:centurion "$log_dir" 2>/dev/null || true\n'
            f'exec s6-setuidgid centurion s6-log n10 s1000000 T "$log_dir"\n'
        )
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with centurion ownership
            ...
            # the centurion user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is centurion-owned (see ``_seed_supervise_skeleton``), and on
