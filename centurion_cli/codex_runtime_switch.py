...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Centurion'
...
def apply(
    ...
    *,
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Centurion' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Centurion for
        ...
        try:
            ...
            # Permissions + Centurion tool callback are always-on production
            ...
            if "centurion-tools" in mig_report.migrated:
                msg_lines.append(
                    "Centurion tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Centurion runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Centurion tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Centurion runtime.")
