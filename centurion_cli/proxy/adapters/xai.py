...
# xAI's public API is OpenAI-compatible for the endpoints Centurion commonly
# uses. The Responses endpoint is included because Centurion' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Centurion-managed OAuth credentials."""

    auth_hint = "centurion auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`centurion auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`centurion auth reset xai-oauth` or re-authenticate with "
                    "`centurion auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `centurion auth add xai-oauth --type oauth`."
            )
