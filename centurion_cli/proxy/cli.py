"""CLI handlers for the ``centurion proxy`` subcommand."""
...
from centurion_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"centurion login {adapter.name}")
        print(
    ...
    print(
        f"Starting Centurion proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Centurion proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: centurion proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``centurion proxy <subcommand>``."""
    ...
    print(
        "centurion proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  centurion proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
