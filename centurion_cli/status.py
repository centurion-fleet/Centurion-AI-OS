"""
Status command for centurion CLI.

Shows the status of all Centurion AI OS components.
"""
...
from centurion_cli.auth import AuthError, resolve_provider
from centurion_cli.colors import Colors, color
from centurion_cli.config import get_env_path, get_env_value, get_centurion_home, load_config
from centurion_cli.models import provider_label
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``centurion config``'s
...
def show_status(args):
    """Show status of all Centurion AI OS components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ Centurion AI OS Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: centurion auth add nous --type oauth)"
    print(
        f"  {'Centurion Portal':<12}  {check_mark(nous_logged_in)} "
        f"{nous_label}"
    )
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: centurion model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: centurion auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: centurion auth add xai-oauth)'}"
    )
    ...
    if managed_nous_tools_enabled():
        ...
        if not features.nous_auth_present:
            print("  Centurion Portal   ✗ not logged in")
        else:
            print("  Centurion Portal   ✓ managed tools available")
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: centurion model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from centurion_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from centurion_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   centurion gateway")
    ...
    jobs_file = get_centurion_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_centurion_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'centurion doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'centurion setup' to configure", Colors.DIM))
    print()
