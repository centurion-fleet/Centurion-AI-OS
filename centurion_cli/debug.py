"""``centurion debug`` debug tools for Centurion AI OS.

Currently supports:
    centurion debug share    Upload debug report (system info + logs) to a
...
_REDACTION_BANNER = (
    "[centurion debug share: log content redacted at upload time. "
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``centurion debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``centurion debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Centurion version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `centurion debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``centurion debug share`` invocation added ~20 MB of resident Python
    ...
    ``centurion debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"centurion debug delete {url}"
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----CenturionDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from centurion_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_centurion_home() / "logs" / filename) if filename else None
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``centurion debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``centurion dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``centurion dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "centurion debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  centurion debug delete <url>")

    print(f"\nShare these links with the Centurion team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: centurion debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'centurion debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``centurion debug`` call.
    ...
    # best-effort — any failure is swallowed so ``centurion debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: centurion debug <command>")
        print()
        print("Commands:")
