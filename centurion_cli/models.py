...
Add, remove, or reorder entries here — both `centurion setup` and
`centurion` provider-selection will pick up the change automatically.
"""
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `centurion model`
    flow without maintaining a separate static list.
    """
    from centurion_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $CENTURION_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $CENTURION_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Centurion refreshes the cache from
...
# ---------------------------------------------------------------------------
# Centurion Portal free-model helper
# ---------------------------------------------------------------------------
# The Centurion Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Centurion Portal account tier detection
# ---------------------------------------------------------------------------

def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    """Fetch the user's Centurion Portal account/subscription info.
    ...
    """
    base = (portal_base_url or "https://portal.personal-centurion.com").rstrip("/")
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Centurion that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Centurion that doesn't ship
...
def check_nous_free_tier() -> bool:
    """Check if the current Centurion Portal user is on a free (unpaid) tier.
    ...
    try:
        from centurion_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# ---------------------------------------------------------------------------
# Centurion Portal recommended models
#
...
def fetch_nous_recommended_models(
    ...
    *,
    ...
    """Fetch the Centurion Portal's curated recommended-models payload.
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        ...
        return "https://portal.personal-centurion.com"
...
# this list:  centurion model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `centurion model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `centurion model` TUI
...
CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ...
    ProviderEntry("nous",           "Centurion Portal",         "Centurion subscription (portal.personal-centurion.com)"),
]
...
def get_picker_providers(*, billing_enabled: bool | None = None) -> list[ProviderEntry]:
    """Return providers shown in ``centurion model`` / setup wizard.

    Hides the subscription portal provider until ``billing.enabled`` is true.
    """
    ...
    return [p for p in CANONICAL_PROVIDERS if p.slug != "nous"]
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``centurion model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``centurion auth add openai-codex`` without
    ``centurion model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    centurion-os is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Centurion Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from centurion_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — centurion-os
...
def get_curated_nous_model_ids() -> list[str]:
    """Return the curated Centurion Portal model-id list.
    ...
    """
    try:
        from centurion_cli.model_catalog import get_curated_nous_models
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Centurion Portal).
    """
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return centurion-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; centurion's picker expects
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Centurion Portal pricing.
    ...
    """
    try:
        from centurion_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``centurion model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from centurion_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:centurion-3                           →  ("nous", "centurion-3")
...
def _resolve_static_model_alias(
    ...
    try:
        from centurion_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Centurion' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``centurion_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Centurion should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``centurion auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.centurion/.env``.
    ...
    """
    try:
        from centurion_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from centurion_cli.auth import read_credential_pool
        from centurion_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Centurion release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Centurion release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        # Try live Centurion Portal /models endpoint
        try:
            from centurion_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from centurion_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "CenturionAgent/1.0",
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _CENTURION_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Centurion' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _CENTURION_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_centurion_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Centurion will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Centurion cannot verify the model name."
