"""centurion webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.centurion/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _centurion_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: centurion gateway run
"""
...
def webhook_command(args):
    """Entry point for 'centurion webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: centurion webhook {subscribe|list|remove|test}")
        print("Run 'centurion webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (centurion gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: centurion webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from centurion webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (centurion gateway run)")
