...
def _discover_plugins(centurion_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.centurion/plugins``.
...
def _discover_mcp() -> list[Component]:
    ...
    try:
        from centurion_cli.mcp_config import _get_mcp_servers
    except Exception:
        return []
...
def run_audit(
    *,
    ...
    home = centurion_home or Path(get_centurion_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, centurion_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(centurion_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, centurion_home=home
    )
    if total == 0:
