"""
Gateway subcommand for centurion CLI.

Handles: centurion gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# when centurion_constants is cached from a pre-update version during `centurion update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "centurion-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "centurion CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``centurion gateway status`` from falsely counting the ``centurion`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``centurion gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_centurion_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"CENTURION_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. CENTURION_HOME
        ...
        # CENTURION_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "CENTURION_HOME=" in command and f"CENTURION_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            ...
            Centurion profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Centurion profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from centurion_cli.profiles import list_profiles
    except Exception:
        return processes
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # with no running gateway until they re-invoke ``centurion gateway``
...
def _sync_centurion_home_from_systemd_unit(system: bool) -> None:
    ...
    :func:`get_centurion_home` falls back to ``/root/.centurion`` — the wrong
    profile. The unit file pins ``CENTURION_HOME`` for the actual gateway
    ...
    unit_home = env.get("CENTURION_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("CENTURION_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["CENTURION_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}centurion gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}centurion gateway restart{scope_flag}")
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from centurion_cli.service_manager import detect_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `centurion gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``centurion gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from centurion_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (CENTURION_HOME-scoped).
...
from centurion_constants import is_container, is_termux, is_wsl
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Centurion Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``centurion gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``CENTURION_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("CENTURION_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "centurion-gateway"
SERVICE_DESCRIPTION = "Centurion AI OS Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current CENTURION_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (CENTURION_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary CENTURION_HOME paths
...
def get_service_name() -> str:
    ...
    Default ``~/.centurion`` returns ``centurion-gateway`` (backward compatible).
    Profile ``~/.centurion/profiles/coder`` returns ``centurion-gateway-coder``.
...
# Legacy service names from older Centurion installs that predate the
# centurion-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (centurion-gateway-*.service) and unrelated third-party
# "centurion" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("centurion.service",)
...
def _find_legacy_centurion_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Centurion gateway units.

    Detects unit files installed by older Centurion versions that used a
    different service name (e.g. ``centurion.service`` before the rename to
    ``centurion-gateway.service``). When both a legacy unit and the current
    ``centurion-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``centurion-gateway-coder.service`` and unrelated third-party
      ``centurion-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``centurion.service`` running an unrelated
...
def has_legacy_centurion_units() -> bool:
    """Return True when any legacy Centurion gateway unit files exist."""
    return bool(_find_legacy_centurion_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Centurion gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy Centurion gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current centurion-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    centurion gateway migrate-legacy")
...
def remove_legacy_centurion_units(
    ...
    legacy = _find_legacy_centurion_units()
    if not legacy:
        print("No legacy Centurion gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Centurion gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: centurion gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo centurion gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    centurion gateway uninstall")
    print_info("    sudo centurion gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Centurion can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo centurion gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo centurion gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo centurion gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Centurion often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.centurion`` → ``ai.centurion.gateway.plist`` (backward compatible).
    Profile ``~/.centurion/profiles/coder`` → ``ai.centurion.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.centurion.gateway-{suffix}" if suffix else "ai.centurion.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.centurion/centurion-os  -> /home/alice/.centurion/centurion-os
      /opt/centurion                 -> /opt/centurion  (kept as-is)
...
def _centurion_home_for_target_user(target_home_dir: str) -> str:
    ...
    root's home.  This translates it to the target user's equivalent path:
      /root/.centurion                    → /home/alice/.centurion
      /root/.centurion/profiles/coder     → /home/alice/.centurion/profiles/coder
      /opt/custom-centurion               → /opt/custom-centurion  (kept as-is)
    """
    current_centurion = get_centurion_home().resolve()
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    centurion_node = centurion_home / "node" / "bin"
    if _is_dir(centurion_node):
        candidates.append(str(centurion_node))
    centurion_nm = centurion_home / "node_modules" / ".bin"
    if _is_dir(centurion_nm):
        candidates.append(str(centurion_nm))
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only CENTURION_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp CENTURION_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../centurion_test`` path into
    # ``Environment="CENTURION_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Centurion install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo centurion gateway uninstall --system")
    print_info("         centurion gateway install")
    print_info("         centurion gateway start")
...
def systemd_install(
    ...
    # Offer to remove legacy units (centurion.service from pre-rename installs)
    # before installing the new centurion-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}centurion gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}centurion gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}centurion gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `centurion gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `centurion gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}centurion gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}centurion gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}centurion gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}centurion gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.centurion.gateway-{suffix}" if suffix else "ai.centurion.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Centurion install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  centurion gateway status             # Check status")
...
def launchd_stop():
    ...
    # `centurion gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Centurion install")
    else:
        print("⚠ Service definition is stale relative to the current Centurion install")
        print("  Run: centurion gateway start")
    ...
    if deep:
        log_file = get_centurion_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _is_official_docker_checkout() -> bool:
    return (
        str(PROJECT_ROOT) == "/opt/centurion"
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("CENTURION_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Centurion gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'centurion' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/centurion/docker/entrypoint.sh before the Centurion command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$CENTURION_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set CENTURION_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `centurion gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set CENTURION_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `centurion gateway restart` which already
    ...
    # `centurion gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Centurion Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # CENTURION_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("CENTURION_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Centurion will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @centurion:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. centurion) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Centurion delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Centurion agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Centurion will use (e.g., centurion@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Centurion connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: centurion pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Centurion will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``centurion setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run centurion under
        WSL.
    """
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_centurion_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'centurion pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: centurion pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'centurion gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from centurion_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from centurion_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from centurion_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Centurion will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Centurion will store the returned account_id/token in ~/.centurion/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `centurion gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_centurion_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `centurion pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `centurion pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"CenturionAgent\"")
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: centurion gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: centurion gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: centurion gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo centurion gateway install --system")
                    print_info("  Or run in foreground:  centurion gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: centurion gateway run")
                print_info("  For persistence:   tmux new -s centurion 'centurion gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from centurion_constants import display_centurion_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: centurion gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup centurion gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: centurion gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'centurion gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    if profile is None:
        ...
        # CENTURION_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``centurion gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``centurion gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: centurion gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: centurion gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s centurion 'centurion gateway run'")
                print()
            ...
            print()
            print("  centurion gateway run                              # direct foreground")
            print("  tmux new -s centurion 'centurion gateway run'         # persistent via tmux")
            print("  nohup centurion gateway run > ~/.centurion/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  centurion profile create <name>     # creates the s6 service slot")
                print("  centurion -p <name> gateway start   # bring it up via s6")
                print("  centurion status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: centurion gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: centurion gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: centurion gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  centurion profile delete <name>     # tears down the s6 service slot")
                print("  centurion -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `centurion -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: centurion gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if supports_systemd_services():
                ...
                if linger_ok is not True:
                    ...
                    print()
                    print("  Then restart the gateway:")
                    print("    centurion gateway restart")
                    return
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: centurion gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  centurion gateway install")
                else:
                    print("To install as a service:")
                    print("  centurion gateway install")
                    print("  sudo centurion gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  centurion gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup centurion gateway run > ~/.centurion/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s centurion 'centurion gateway run'         # persistent via tmux")
                    print("  nohup centurion gateway run > ~/.centurion/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  centurion gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  centurion gateway install  # Install as user service")
                    print("  sudo centurion gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Centurion gateway unit files from
        # pre-rename installs (e.g. centurion.service). Profile units and
        # unrelated third-party services are never touched.
