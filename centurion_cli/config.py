"""
Configuration management for Centurion AI OS.

Config files are stored in ~/.centurion/ for easy access:
- ~/.centurion/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.centurion/.env         - API keys and secrets

This module provides:
- centurion config          - Show current configuration
- centurion config edit     - Open config in editor
- centurion config set      - Set a specific value
- centurion config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.centurion/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``centurion logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  centurion config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Centurion makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Centurion itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``centurion update``, the TUI build.
...
#   on every plugin install / ``centurion update``.
...
# * ``CENTURION_HOME`` / ``CENTURION_PROFILE`` / ``CENTURION_CONFIG`` /
#   ``CENTURION_ENV`` — Centurion runtime location flags. Writing these into
...
#
# IMPORTANT: ``CENTURION_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (CENTURION_GEMINI_CLIENT_ID,
# CENTURION_LANGFUSE_PUBLIC_KEY, CENTURION_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Centurion runtime location — never via dashboard env writer.
    # NOT a CENTURION_* blanket: integration credentials (CENTURION_GEMINI_*,
    # CENTURION_LANGFUSE_*, CENTURION_SPOTIFY_*, ...) ARE allowed.
    "CENTURION_HOME", "CENTURION_PROFILE", "CENTURION_CONFIG", "CENTURION_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Centurion runtime location "
            "(CENTURION_HOME, CENTURION_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.centurion/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `centurion plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "CENTURION_LANGFUSE_ENV",
    "CENTURION_LANGFUSE_RELEASE",
    "CENTURION_LANGFUSE_SAMPLE_RATE",
    "CENTURION_LANGFUSE_MAX_CHARS",
    "CENTURION_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from centurion_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CENTURION_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_centurion_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Centurion is running in package-manager-managed mode.

    Two signals: the CENTURION_MANAGED env var (set by the systemd service),
    or a .managed marker file in CENTURION_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade centurion-os"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Centurion was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.centurion/.install_method`` file (written by installers)
    2. CENTURION_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_centurion_home() / ".install_method"
    try:
    ...
    from centurion_constants import is_container
    if is_container():
        return "docker"
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.centurion/.install_method."""
    stamp = get_centurion_home() / ".install_method"
    try:
...
def format_managed_message(action: str = "modify this Centurion installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("CENTURION_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Centurion installation is managed by NixOS "
            f"(CENTURION_MANAGED={env_hint}).\n"
            "Edit services.centurion-os.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Centurion installation is managed by Homebrew "
            f"(CENTURION_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade centurion-os"
        )

    return (
        f"Cannot {action}: this Centurion installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Centurion."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CENTURION_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, centurion_bin
    or None if container mode is not active, we're already inside the
    container, or CENTURION_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("CENTURION_DEV") == "1":
        return None
    ...
    container_mode_file = get_centurion_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "centurion-os")
    exec_user = info.get("exec_user", "centurion")
    centurion_bin = info.get("centurion_bin", "/data/current-package/bin/centurion")
...
# Re-export from centurion_constants — canonical definition lives there.
from centurion_constants import get_centurion_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the centurion group can
    share state with the gateway service.

    The mode can be overridden via the CENTURION_HOME_MODE environment variable
    (e.g. CENTURION_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CENTURION_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CENTURION_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Centurion runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("CENTURION_CONTAINER") or os.environ.get("CENTURION_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set CENTURION_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CENTURION_HOME if the user doesn't have one yet."""
...
def ensure_centurion_home():
    """Ensure ~/.centurion directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_centurion_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_centurion_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CENTURION_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Centurion-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Centurion auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Centurion sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Centurion
        ...
        # bundled Centurion image, which drops to the `centurion` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Centurion sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Centurion should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``centurion chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.centurion/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: centurion sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Centurion truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``centurion kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``centurion kanban decompose`` and the kanban
        ...
        # ``centurion profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `centurion model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `centurion --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `centurion -c` muscle memory.  Default off so existing
        # users aren't surprised.  CENTURION_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/centurion`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.centurion/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.centurion/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Centurion feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Centurion auto-pauses the goal and
    ...
    # always goes to ~/.centurion/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${CENTURION_SKILL_DIR} and ${CENTURION_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `centurion curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.centurion/skills/ into
        # ~/.centurion/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `centurion curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for centurion-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Centurion AI OS*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (CENTURION_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.centurion/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or CENTURION_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `centurion doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``centurion_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Centurion to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via CENTURION_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `centurion -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `centurion` launches with
        ...
        # decomposition is manual via `centurion kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     centurion-os's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.centurion/logs/.
    ...
    # curated model lists for OpenRouter and Centurion Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a centurion-os release.
    ...
    "gateway": {
        ...
        # uploaded as native gateway attachments. Files inside the Centurion
        # cache (~/.centurion/cache/{documents,images,audio,video,screenshots})
        ...
        # to CENTURION_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        # expanded.
        ...
        # CENTURION_MEDIA_TRUST_RECENT_FILES.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to CENTURION_MEDIA_TRUST_RECENT_SECONDS.
        "trust_recent_files_seconds": 600,
    },

    # Session storage — controls automatic cleanup of ~/.centurion/state.db.
    ...
    "sessions": {
        ...
        # default of ``centurion sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.centurion/sessions/session_{sid}.json`` on every turn
    ...
    # ``centurion update`` behaviour.
    "updates": {
        # Run a full ``centurion backup``-style zip of CENTURION_HOME before every
        # ``centurion update``.  Backups land in ``<CENTURION_HOME>/backups/`` and
        # can be restored with ``centurion import <path>``.  Off by default —
        # on large CENTURION_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<CENTURION_HOME>/lsp/bin/`` on first use.
    ...
    # `centurion tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.centurion/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.centurion/.env (or your shell) and never in config.yaml.
            ...
            # ~/.centurion/bin/ on first use.  When False you must install
            ...
            # `centurion secrets bitwarden setup`.
            "server_url": "",
        },
    },
...
# selection step (Centurion Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "NOUS_BASE_URL": {
        "description": "Centurion Portal base URL override",
        "prompt": "Centurion Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "CENTURION_GEMINI_CLIENT_SECRET": {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'centurion model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Centurion auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @centurion:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CENTURION_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #centurion)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: centurion-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'centurion-os' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Centurion API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Centurion API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Centurion API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CENTURION_TOOL_PROGRESS and CENTURION_TOOL_PROGRESS_MODE are deprecated —
    ...
    "CENTURION_PREFILL_MESSAGES_FILE": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.centurion.config`` entries, then checks
    ...
    try:
        ...
        # should never break `centurion update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Centurion versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``centurion_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'centurion setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Centurion won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'centurion doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("CENTURION_HOME", "~/.centurion")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("CENTURION_TOOL_PROGRESS")
            old_mode = get_env_value("CENTURION_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from CENTURION_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from CENTURION_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("CENTURION_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from CENTURION_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$CENTURION_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_centurion_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `centurion plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `centurion curator status` has no
    ...
    #   3. Creates `~/.centurion/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_centurion_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_centurion_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: centurion config set <key> <value>")
    ...
    # metadata.centurion.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _centurion_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.centurion/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.centurion/config.yaml.
    ...
    (which change ``CENTURION_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
#   openai-codex (OAuth — centurion auth) — OpenAI Codex
#   nous         (OAuth — centurion auth) — Centurion Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.centurion/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.centurion/.env.
    ...
    interactive menu render (`centurion tools`, `centurion setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `centurion tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Centurion env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.centurion/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.centurion/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.centurion/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.centurion/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Centurion — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Centurion vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.centurion/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Centurion Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  centurion config edit     # Edit config file", Colors.DIM))
    print(color("  centurion config set <key> <value>", Colors.DIM))
    print(color("  centurion setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: centurion config set <key> <value>")
            print()
            print("Examples:")
            print("  centurion config set model anthropic/claude-sonnet-4")
            print("  centurion config set terminal.backend docker")
            print("  centurion config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'centurion config migrate' to add them")
        ...
        print("  centurion config           Show current configuration")
        print("  centurion config edit      Open config in editor")
        print("  centurion config set <key> <value>   Set a config value")
        print("  centurion config check     Check for missing/outdated config")
        print("  centurion config migrate   Update config with new options")
        print("  centurion config path      Show config file path")
        print("  centurion config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``centurion config`` UI so users
