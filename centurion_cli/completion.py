"""Shell completion script generation for centurion CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""# Centurion AI OS bash completion
# Add to ~/.bashrc:
#   eval "$(centurion completion bash)"

_centurion_profiles() {{
    local profiles_dir="$HOME/.centurion/profiles"
...
complete -F _centurion_completion centurion
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef centurion
# Centurion AI OS zsh completion
# Add to ~/.zshrc:
...
_centurion_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.centurion/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.centurion/profiles 2>/dev/null)}}")
    fi
...
{top_cmds_str}
            ...
            _describe 'centurion command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Centurion AI OS fish completion",
        "# Add to your config:",
        ...
        "    echo default",
        "    if test -d $HOME/.centurion/profiles",
        "        ls $HOME/.centurion/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c centurion -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c centurion -f -s p -l profile"
