...
on Japanese locales, etc.).  Centurion's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Centurion is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CENTURION_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CENTURION_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Centurion-managed Git install directories so
    ...
    # already-running shells don't see the change — which means centurion
    ...
    # install without Centurion-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Centurion's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Centurion's
    ``centurion config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Centurion (User env var in Windows Settings, or
    export in a PowerShell profile) and Centurion picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Centurion-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\centurion\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Centurion spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Centurion-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Centurion venv Scripts directory — host of the centurion.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes centurion, but harmless to include.
