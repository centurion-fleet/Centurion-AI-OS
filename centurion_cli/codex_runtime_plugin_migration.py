"""Migrate Centurion' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Centurion' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Centurion mcp_servers.<n>.command/args/env  → codex stdio transport
  Centurion mcp_servers.<n>.url/headers       → codex streamable_http transport
  Centurion mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Centurion mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Centurion-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Centurion' own
...
MIGRATION_MARKER = (
    "# managed by centurion-os — `centurion codex-runtime migrate` regenerates this section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Centurion config.")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Centurion' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, centurion_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Centurion MCP server config to the codex inline-table dict
    ...
    if not isinstance(codex_plugin_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(codex_plugin_cfg.get("command"))
    has_url = bool(codex_plugin_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(codex_plugin_cfg["command"])
        args = centurion_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = centurion_cfg.get("env") or {}
        if env:
            # Codex expects string values
            out["env"] = {str(k): str(v) for k, v in env.items()}
        cwd = centurion_cfg.get("cwd")
        if cwd:
            out["cwd"] = str(cwd)
    elif has_url:
        # streamable_http transport (codex covers both http and SSE here)
        out["url"] = str(codex_plugin_cfg["url"])
        headers = centurion_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Centurion' transport: sse hint is informational; codex auto-negotiates
        if centurion_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in centurion_cfg:
        try:
            out["tool_timeout_sec"] = float(codex_plugin_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in centurion_cfg:
        try:
            out["startup_timeout_sec"] = float(codex_plugin_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if centurion_cfg.get("enabled") is False:
        out["enabled"] = False

    # Detect keys we explicitly drop with warning
    for key in centurion_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_CENTURION_KEYS:
            skipped.append(f"{key} (unknown Centurion key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (CENTURION_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Centurion)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Centurion' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Centurion' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Centurion
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="centurion-migration")
...
def _build_centurion_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Centurion' own
    ...
    so a centurion installed under /opt/, /usr/local/, or a venv all work.
    CENTURION_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # CENTURION_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_centurion_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever CENTURION_HOME its launcher sets
    ...
    # — that would override the launcher's CENTURION_HOME and pin the subprocess
    ...
    # PYTHONPATH passes through so a worktree-launched centurion finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["CENTURION_QUIET"] = "1"
    env["CENTURION_REDACT_SECRETS"] = env.get("CENTURION_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    """Translate Centurion mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        centurion_config: full ~/.centurion/config.yaml dict
        ...
        expose_centurion_tools: when True (default), register Centurion' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Centurion for tools
    ...
    centurion_servers = (centurion_config or {}).get("mcp_servers") or {}
    if not isinstance(centurion_servers, dict):
        report.errors.append(
            "mcp_servers in Centurion config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in centurion_servers.items():
    ...
    # Inject Centurion' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Centurion for the tools codex
