"""centurion hooks — inspect and manage shell-script hooks.

Usage::

    centurion hooks list
    centurion hooks test <event> [--for-tool X] [--payload-file F]
    centurion hooks revoke <command>
    centurion hooks doctor

Consent records live under ``~/.centurion/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.centurion/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``centurion hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: centurion hooks {list|test|revoke|doctor}")
        print("Run 'centurion hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.centurion/config.yaml.")
        print("See `centurion hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `centurion hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `centurion hooks test` and `centurion hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Centurion wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `centurion hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `centurion hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `centurion hooks doctor`.")
