"""Helpers for loading Centurion .env files consistently across entrypoints."""
...
# load_centurion_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_centurion_dotenv().
# Used by setup / `centurion model` flows to label detected credentials so
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_centurion_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which CENTURION_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `centurion setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    already knows all valid Centurion env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from centurion_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_centurion_dotenv(
    *,
    ...
    """Load Centurion environment files with user config taking precedence.
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Centurion reads
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `centurion model`
