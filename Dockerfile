...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/centurion/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when centurion
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# centurion process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main centurion process, the dashboard,
...
# Non-root user for runtime; UID can be overridden via CENTURION_UID at runtime
RUN useradd -u 10000 -m -d /opt/data centurion

COPY --chmod=0755 --from=uv_source /usr/local/bin/uv /usr/local/bin/uvx /usr/local/bin/

WORKDIR /opt/centurion

# ---------- Layer-cached dependency install ----------
# Copy only package manifests first so npm install + Playwright are cached
# unless the lockfiles themselves change.
#
# ui-tui/packages/centurion-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/centurion-ink/ ui-tui/packages/centurion-ink/
...
# that permanently disagrees with the root lock on the @centurion/ink entry.
...
COPY --chown=centurion:centurion . .
...
# ---------- Permissions ----------
# Make install dir world-readable so any CENTURION_UID can read it at runtime.
...
USER root
RUN chmod -R a+rX /opt/centurion && \
    chown -R centurion:centurion /opt/centurion/.venv /opt/centurion/ui-tui /opt/centurion/node_modules
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the centurion user via
# `s6-setuidgid centurion` in its run script. If CENTURION_UID is unset, services
# run as the default centurion user (UID 10000).

# ---------- Link centurion-ai-os itself (editable) ----------
# Deps are already installed in the cached layer above; `--no-deps` makes
# this a fast (~1s) egg-link creation with no resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-centurion + dashboard.
...
# `exec centurion`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $CENTURION_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/bin/sh\nexec /opt/centurion/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-centurion-setup && \
    chmod +x /etc/cont-init.d/01-centurion-setup
...
# bin globally so `docker exec <container> centurion ...` and any
# subprocess that doesn't activate the venv first still find centurion.
ENV PATH="/opt/centurion/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. centurion
# subcommand vs. no-args), drops to the centurion user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/centurion/docker/main-wrapper.sh" ]
CMD [ ]
