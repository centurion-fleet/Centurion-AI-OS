"""Shared constants for Centurion AI OS.
...
_UNSET = object()
_CENTURION_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_CENTURION_HOME_OVERRIDE", default=_UNSET
)


def set_centurion_home_override(path: str | Path | None) -> Token:
    """Set a context-local Centurion home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _CENTURION_HOME_OVERRIDE.set(value)


def reset_centurion_home_override(token: Token) -> None:
    """Restore the previous context-local Centurion home override."""
    _CENTURION_HOME_OVERRIDE.reset(token)


def get_centurion_home_override() -> str | None:
    """Return the active context-local Centurion home override, if any."""
    override = _CENTURION_HOME_OVERRIDE.get()
...
def get_centurion_home() -> Path:
    ...
    Reads CENTURION_HOME env var, falls back to ~/.centurion.
    This is the single source of truth — all other copies should import this.

    When ``CENTURION_HOME`` is unset but an ``active_profile`` file indicates
    ...
    if not _profile_fallback_warned:
        try:
            # Inline the default-root resolution from get_default_centurion_root()
            # to stay import-safe (this function is called from module scope
            # in 30+ files; we cannot afford to trigger logging setup here).
            active_path = (Path.home() / ".centurion" / "active_profile")
        ...
        if active and active != "default":
            ...
            import sys
            msg = (
                f"[CENTURION_HOME fallback] CENTURION_HOME is unset but active "
                f"profile is {active!r}. Falling back to ~/.centurion, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass CENTURION_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
...
def get_default_centurion_root() -> Path:
    ...
    In Docker or custom deployments where ``CENTURION_HOME`` points outside
    ``~/.centurion`` (e.g. ``/opt/data``), returns ``CENTURION_HOME`` directly
    — that IS the root.
    ...
    """
    native_home = Path.home() / ".centurion"
    env_home = os.environ.get("CENTURION_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # CENTURION_HOME is under ~/.centurion (normal or profile mode)
        return native_home
    except ValueError:
        pass
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_centurion_home() / "optional-skills"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    if default is not None:
        return default
    return get_centurion_home() / "skills"
...
def get_centurion_dir(new_subpath: str, old_name: str) -> Path:
    ...
    Args:
        new_subpath: Preferred path relative to CENTURION_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to CENTURION_HOME (e.g. ``"image_cache"``).
...
def display_centurion_home() -> str:
    """Return a user-friendly display string for the current CENTURION_HOME.
    ...
    :func:`get_centurion_home` instead.
    """
    home = get_centurion_home()
    try:
...
def get_subprocess_home() -> str | None:
    ...
    **never** modified — only subprocess environments should inject this value.
    ...
    """
    centurion_home = get_centurion_home_override() or os.getenv("CENTURION_HOME")
    if not centurion_home:
        return None
    profile_home = os.path.join(centurion_home, "home")
...
def get_skills_dir() -> Path:
    """Return the path to the skills directory under CENTURION_HOME."""
    return get_centurion_home() / "skills"
