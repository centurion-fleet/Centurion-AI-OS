...
await build({
  ...
  // Skip the prebuilt @centurion/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@centurion/ink': resolve(root, 'packages/centurion-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The centurion_cli launcher
