import type { RunExternalProcess } from '@centurion/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`centurion ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching centurion: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`centurion ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
