...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CENTURION_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Centurion on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CENTURION_TUI_TRUECOLOR=1.
