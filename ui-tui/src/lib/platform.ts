 ...
 * centurion-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``centurion_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but centurion-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS centurion-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on centurion-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is centurion-ink's Alt signal and accepting it would
