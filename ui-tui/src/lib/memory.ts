...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.CENTURION_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.centurion', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `centurion-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
