import type { InputEvent, Key } from '@centurion/ink'
import * as Ink from '@centurion/ink'
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.CENTURION_TUI_TERMUX_FAST_ECHO ?? '').trim().toLowerCase()
