...
React + Ink terminal UI for Centurion AI OS. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CENTURION_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.centurion/.centurion_history` or under `CENTURION_HOME`.
...
```text
ui-tui/
  packages/centurion-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      centurion-ink.d.ts    type declarations for @centurion/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent CenturionCLI subprocess for slash commands
```
