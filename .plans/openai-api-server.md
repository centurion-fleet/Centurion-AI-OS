# OpenAI-Compatible API Server for Centurion AI OS
...
SSE streaming. By exposing this endpoint, centurion-ai-os becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  centurion-ai-os   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.centurion/.env`
2. Run `centurion gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with centurion-ai-os through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns centurion-ai-os as a model) |
...
Authorization: Bearer centurion-api-key-here
...
But centurion-ai-os sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "centurion-ai-os"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured centurion-ai-os model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of centurion-ai-os's core prompt
...
Once implemented, centurion-ai-os works as a drop-in backend for:
