...
from centurion_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(centurion_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <centurion_home>/plugins/<name> and opt it in."""
    plugin_dir = centurion_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    centurion_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        centurion_home, "capture_hook",
        register_body=(
