"""Tests for centurion_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Centurion entry point
(centurion, centurion-os, centurion-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Centurion entry point imports centurion_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported centurion_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("centurion_bootstrap", None)
    import centurion_bootstrap  # noqa: WPS433
    return centurion_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Centurion CLI
...
class TestEntryPointsImportBootstrap:
    """Every Centurion entry point must import centurion_bootstrap as its
    ...
    # Entry points that invoke Centurion as a process.  Each one must
    # import centurion_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "centurion_cli/main.py",   # centurion CLI (console_script)
        "run_agent.py",          # centurion-os (console_script)
        "acp_adapter/entry.py",  # centurion-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import centurion_bootstrap' and that
        ...
        half-finished ``centurion update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``centurion_bootstrap`` as a top-level module) leaves centurion
        ...
        """
        # Resolve relative to the centurion-os repo root.  Tests live
        # at tests/test_centurion_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # centurion start even when centurion_bootstrap hasn't been
        ...
        assert first_import_name == "centurion_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'centurion_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import centurion_bootstrap' line to be the first import."
        )
