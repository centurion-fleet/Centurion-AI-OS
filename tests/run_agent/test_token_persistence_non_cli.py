...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    centurion_state = ModuleType("centurion_state")
    centurion_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "centurion_state", centurion_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Centurion"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Centurion"
