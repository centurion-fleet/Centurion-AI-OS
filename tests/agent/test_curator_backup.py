...
def backup_env(monkeypatch, tmp_path):
    """Isolate CENTURION_HOME + reload modules so every test starts clean."""
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under CENTURION_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level CENTURION_DIR picks up the tmp HOME."""
    import centurion_constants
    importlib.reload(centurion_constants)
