...
def test_cache_key_is_per_config_path(tmp_path, monkeypatch):
    """Two different CENTURION_HOMEs keep separate cache entries."""
    home_a = tmp_path / "home_a" / ".centurion"
    ...
    home_b = tmp_path / "home_b" / ".centurion"
    ...
    monkeypatch.setenv("CENTURION_HOME", str(home_a))
    assert get_external_skills_dirs() == [ext_a.resolve()]

    monkeypatch.setenv("CENTURION_HOME", str(home_b))
