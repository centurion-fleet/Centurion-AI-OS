...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate CENTURION_HOME under tmp_path."""
    monkeypatch.setenv("CENTURION_HOME", str(tmp_path / ".centurion"))
    (tmp_path / ".centurion").mkdir()

    # Force the constants/image cache helpers to re-read CENTURION_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("centurion_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_centurion_home_cache(self, http_server):
        ...
        # The cache directory must be under CENTURION_HOME — gateway cleanup
