...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    ...
    config = {"platform_toolsets": {"cli": ["centurion-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[centurion-cli, spotify]`` (composite + configurable) must keep the full
    ``centurion-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``centurion-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when centurion-cli is listed"
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["centurion-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``centurion-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["centurion-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _CENTURION_CORE_TOOLS but aren't user-toggleable) are still
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/centurion-fleet/Centurion-AI-OS/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (centurion-cli, centurion-telegram, etc.) must NOT
    ...
    ``centurion tools``, but centurion-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "centurion-cli",  # <-- the culprit
    ...
    # centurion-cli must NOT survive — it's a platform default, not an MCP server
    assert "centurion-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_centurion_telegram():
    """Same bug for Telegram — centurion-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "centurion-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "centurion-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "centurion-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from centurion_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """centurion-gateway includes list should cover all messaging platforms."""
        from centurion_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["centurion-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from centurion_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from centurion_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/centurion-fleet/Centurion-AI-OS/issues/6901
    """
    config = {
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's CENTURION_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"CENTURION_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"CENTURION_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from centurion_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from centurion_cli.tools_config import TOOL_CATEGORIES
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["centurion-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "centurion-_test_platform"},
    }
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from centurion_cli.tools_config import _toolset_allowed_for_platform
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from centurion_cli.tools_config import _save_platform_tools
    config = {}
