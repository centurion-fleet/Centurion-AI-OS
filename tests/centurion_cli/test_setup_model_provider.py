...
from centurion_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from centurion_cli.config import load_config, save_config, save_env_value
from centurion_cli.nous_subscription import NousFeatureState, NousSubscriptionFeatures
from centurion_cli.setup import _print_setup_summary, setup_model_provider
...
def test_setup_model_provider_preserves_auxiliary_choices_written_by_picker(tmp_path, monkeypatch):
    """Aux choices made inside centurion setup must survive the wizard's final save."""
...
def test_setup_same_provider_rotation_strategy_saved_for_multi_credential_pool(tmp_path, monkeypatch):
    ...
    import centurion_cli.main as _main_mod
    import centurion_cli.setup as _setup_mod
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'centurion setup' to configure" not in output
