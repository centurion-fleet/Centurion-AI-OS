...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (CENTURION_OVERLAYS) — bedrock
...
class TestProviderModelIdsBedrock:
    ...
    def test_falls_back_to_static_list_when_discovery_empty(self, monkeypatch):
        ...
        from centurion_cli.models import _PROVIDER_MODELS, provider_model_ids
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via CENTURION_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in CENTURION_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from centurion_cli.providers import CENTURION_OVERLAYS
        assert "bedrock" in CENTURION_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from centurion_cli.providers import CENTURION_OVERLAYS
        assert CENTURION_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from centurion_cli.providers import CENTURION_OVERLAYS
        assert CENTURION_OVERLAYS["bedrock"].auth_type == "aws_sdk"
