...
from centurion_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("CENTURION_CA_BUNDLE", raising=False)
    ...
    def test_missing_centurion_ca_bundle_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.setenv("CENTURION_CA_BUNDLE", "/nonexistent/centurion-ca.pem")
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `centurion model` → Centurion Portal → Skip (keep current) after
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import centurion_cli.auth as auth_mod
        import centurion_cli.models as models_mod
        import centurion_cli.nous_subscription as ns
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `centurion auth add nous`
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Centurion is not logged into Centurion Portal").
    ...
    # doesn't raise "Centurion is not logged into Centurion Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `centurion auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    When the Centurion Portal server returns ``invalid_grant`` with
    ``error_description`` containing "reuse detected", Centurion must surface an
    ...
    re-authenticate" string led users to report this as a Centurion persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from centurion_cli.auth import _refresh_access_token
    ...
    assert "centurion auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect CENTURION_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("CENTURION_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without CENTURION_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    monkeypatch.delenv("CENTURION_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """CENTURION_SHARED_AUTH_DIR must redirect the path."""
    from centurion_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("CENTURION_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from centurion_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from centurion_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from centurion_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `centurion auth add nous
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from centurion_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "centurion-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different CENTURION_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("CENTURION_HOME", str(profile_a))
    ...
    # Profile B: fresh CENTURION_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("CENTURION_HOME", str(profile_b))
