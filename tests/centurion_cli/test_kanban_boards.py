"""Tests for the multi-board kanban layer (``centurion kanban boards …``).
...
* Current-board persistence via ``<root>/kanban/current`` and
  ``CENTURION_KANBAN_BOARD`` env var.
* ``connect(board=)`` isolation — writes on one board don't leak.
* ``create_board`` / ``list_boards`` / ``remove_board`` round trip.
* CLI surface: ``centurion kanban boards list/create/switch/rm``.
* ``_default_spawn`` injects ``CENTURION_KANBAN_BOARD`` into worker env.
"""
...
def fresh_home(tmp_path, monkeypatch):
    """Isolated CENTURION_HOME with no prior kanban state.
    ...
    # Also reset centurion_constants cache so get_default_centurion_root() re-reads.
    try:
        import centurion_constants
        centurion_constants._cached_default_centurion_root = None  # type: ignore[attr-defined]
    except Exception:
        pass
...
class TestCurrentBoard:
    ...
    def test_env_var_takes_precedence(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("CENTURION_KANBAN_BOARD", "envboard")
    ...
    def test_env_beats_file(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("CENTURION_KANBAN_BOARD", "b")
    ...
    def test_stale_env_falls_through_to_file_pointer(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("CENTURION_KANBAN_BOARD", "missing-board")
        assert kb.get_current_board() == "persisted"

    def test_invalid_env_falls_through(self, fresh_home, monkeypatch):
        monkeypatch.setenv("CENTURION_KANBAN_BOARD", "!!bad!!")
...
class TestConnectionIsolation:
    ...
    def test_connect_env_var_overrides_current(self, fresh_home, monkeypatch):
        ...
        monkeypatch.setenv("CENTURION_KANBAN_BOARD", "envwin")
        with kb.connect() as conn:
    ...
    def test_connect_stale_env_uses_fallback_board_without_recreating_it(
        ...
        kb.create_board("persist")
        kb.set_current_board("persist")
        monkeypatch.setenv("CENTURION_KANBAN_BOARD", "ephemeral")
...
class TestWorkerSpawnEnv:
    """Ensure the dispatcher pins ``CENTURION_KANBAN_BOARD`` / DB / workspaces on spawn.
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``centurion kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
...
class TestCLI:
    def test_boards_list_default_only(self, tmp_path):
        env = {"CENTURION_HOME": str(tmp_path)}
