...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from centurion_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # centurion model, not via upgrade prompts.
...
class TestGmiDoctor:
    ...
    def test_run_doctor_checks_gmi_models_endpoint(self, monkeypatch, tmp_path):
        from centurion_cli import doctor as doctor_mod
...
class TestGmiAuxiliary:
    ...
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a CenturionAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("CenturionAgent/")

    def test_gmi_profile_declares_centurion_user_agent(self):
        """The GMI plugin sets a CenturionAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("CenturionAgent/"), (
            f"expected GMI profile User-Agent to start with 'CenturionAgent/', got {ua!r}"
        )
