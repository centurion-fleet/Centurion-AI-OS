...
Before the fix, ``centurion whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``centurion gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    centurion = home / ".centurion"
    centurion.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("CENTURION_HOME", str(centurion))
...
def _env_value(centurion_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    monkeypatch.setattr("centurion_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``centurion whatsapp`` with an existing paired session and
