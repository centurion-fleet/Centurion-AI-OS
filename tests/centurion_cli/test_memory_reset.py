"""Tests for the `centurion memory reset` CLI command.
...
- Profile-scoped reset (uses CENTURION_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    ...
    (memories / "MEMORY.md").write_text(
        "§\nCenturion repo is at ~/.centurion/centurion-os\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `centurion memory reset` is run.
    """
...
class TestMemoryReset:
    """Tests for `centurion memory reset` subcommand."""
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's CENTURION_HOME."""
