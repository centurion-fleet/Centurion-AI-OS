"""Tests for Codex auth — tokens stored in Centurion auth store (~/.centurion/auth.json)."""
...
def _setup_centurion_auth(centurion_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Centurion auth store."""
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Centurion auth store, not ~/.codex/."""
    centurion_home = tmp_path / "centurion"
    codex_home = tmp_path / "codex-cli"
    centurion_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "centurion-at", "refresh_token": "centurion-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Centurion store
    assert not (codex_home / "auth.json").exists()

    # Centurion auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "centurion-at"
