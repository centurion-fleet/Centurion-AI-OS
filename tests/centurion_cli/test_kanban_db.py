...
def kanban_home(tmp_path, monkeypatch):
    """Isolated CENTURION_HOME with an empty kanban DB."""
...
def test_dispatch_skips_nonspawnable_into_separate_bucket(kanban_home, monkeypatch):
    ...
    from centurion_cli import profiles
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Centurion profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="centurion-task", assignee="daily")
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Centurion-managed scratch dir should be cleaned up"
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Centurion' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Centurion' DB/metadata/logs on task completion.
    """
...
# spawned with `centurion -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's CENTURION_HOME.
# ---------------------------------------------------------------------------
...
class TestSharedBoardPaths:
    ...
    def test_default_install_anchors_at_home_dot_centurion(
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # CENTURION_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `centurion -p coder`).
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: CENTURION_HOME=/opt/centurion/profiles/coder;
        # `get_default_centurion_root()` walks up to /opt/centurion because
    ...
    def test_explicit_override_via_centurion_kanban_home(
        ...
        # Explicit override: CENTURION_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".centurion"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CENTURION_HOME", str(profile_home))
        monkeypatch.setenv("CENTURION_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CENTURION_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # CENTURION_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile CENTURION_HOME and reads.
        monkeypatch.setenv("CENTURION_HOME", str(profile_home))
        with kb.connect() as conn:
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see centurion_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("centurion_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="centurion_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'centurion kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
def test_resolve_centurion_argv_prefers_path_shim(monkeypatch):
    ...
    monkeypatch.delenv("CENTURION_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/centurion")
    argv = kb._resolve_centurion_argv()
    assert argv == ["/usr/local/bin/centurion"]
...
def test_resolve_centurion_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CENTURION_BIN", ".\\centurion.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_centurion_argv() == [os.path.abspath(".\\centurion.exe")]
...
def test_resolve_centurion_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "centurion.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("CENTURION_BIN", raising=False)
...
def test_resolve_centurion_argv_honors_centurion_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like CENTURION_BIN lets service managers pin the executable."""
    import shutil
    import centurion_cli.kanban_db as kb
    ...
    monkeypatch.setenv("CENTURION_BIN", str(shim))
...
def test_resolve_centurion_argv_centurion_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare CENTURION_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import centurion_cli.kanban_db as kb
...
def test_resolve_centurion_argv_centurion_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare CENTURION_BIN does not accept current-directory shadow executables."""
    import sys
    import centurion_cli.kanban_db as kb

    (tmp_path / "centurion.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_centurion_argv_centurion_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved CENTURION_BIN batch shim is not used as worker argv[0]."""
    import sys
    import centurion_cli.kanban_db as kb
...
def test_resolve_centurion_argv_centurion_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved CENTURION_BIN command names do not delegate cwd search to Popen."""
    import sys
    import centurion_cli.kanban_db as kb
...
def test_resolve_centurion_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    ...
    `python -m centurion` which fails with `No module named centurion` on every
    invocation.
    """
    import shutil
    import sys
...
def test_resolve_centurion_argv_module_actually_runs():
    ...
    with mock.patch.dict(os.environ, {}, clear=False):
        os.environ.pop("CENTURION_BIN", None)
    ...
    assert "Centurion AI OS" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
