"""Tests for the Command Installation check in centurion doctor."""
...
import centurion_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal CENTURION_HOME + PROJECT_ROOT for doctor tests."""
    ...
    centurion_bin = venv_bin_dir / "centurion"
    centurion_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    centurion_bin.chmod(0o755)
    ...
    # Stub auth checks
    try:
        from centurion_cli import auth as _auth_mod
    ...
    return home, project, centurion_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, centurion_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "centurion"
        cmd_link.symlink_to(centurion_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "centurion doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "centurion"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == centurion_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "centurion"
        wrong_target = tmp_path / "wrong_centurion"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        centurion_bin = project / ".venv" / "bin" / "centurion"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "centurion"
        cmd_link.symlink_to(centurion_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/centurion" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/centurion is a regular file (not symlink), accept it."""
        home, project, centurion_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "centurion"
        cmd_link.write_text("#!/bin/sh\nexec python -m centurion_cli.main \"$@\"\n")
