...
class TestSlackNativeSlashes:
    ...
    def test_centurion_catchall_is_first(self):
        """``/centurion`` must be reserved as the first slot so the legacy
        ``/centurion <subcommand>`` form keeps working after we add new
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /centurion <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `centurion slack manifest`)."""
...
from centurion_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/centurion metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",
