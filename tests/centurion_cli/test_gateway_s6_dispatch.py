...
`centurion gateway start/stop/restart` invocation should be routed to
...
def test_dispatch_renders_gateway_not_registered_friendly(
    ...
    """`centurion -p typo gateway start` should print a clear message and
    exit 1 — not dump a traceback at the user."""
    from centurion_cli import gateway as gw
    from centurion_cli.service_manager import GatewayNotRegisteredError
    ...
    assert "centurion profile create typo" in out
...
def test_dispatch_renders_s6_command_error_friendly(
    ...
    from centurion_cli import gateway as gw
    from centurion_cli.service_manager import S6CommandError
