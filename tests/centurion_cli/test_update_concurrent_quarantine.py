"""Tests for issue #26670 — concurrent centurion.exe detection and improved
quarantine retry / reboot-deferred fallback during `centurion update` on Windows.
...
from centurion_cli import main as cli_main


# Tests in this module either exercise the REAL _detect_concurrent_centurion_instances
# helper (and need the autouse stub in tests/centurion_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_centurion_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "centurion.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "centurion.exe").write_bytes(b"")
    (scripts_dir / "centurion-gateway.exe").write_bytes(b"")
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "centurion.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_centurion_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "centurion.exe"
    shim.write_bytes(b"")
    ...
    procs = [
        _make_proc(other_pid, str(shim), "centurion.exe"),
    ...
    assert result == [(other_pid, "centurion.exe")]
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_centurion_instances(tmp_path) == []
...
def test_detect_concurrent_excludes_parent_chain(_winp, tmp_path):
    ...
    Simulates the real Windows topology: centurion.exe launcher (PID L) spawns
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "centurion.exe"),
    ]
...
def test_detect_concurrent_still_finds_unrelated_other_centurion(_winp, tmp_path):
    ...
    sibling_pid = me + 200  # an UNRELATED centurion.exe (must still be reported)

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "centurion.exe"),
        _make_proc(sibling_pid, str(shim), "centurion.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
        parent_chain=[launcher_pid],
        proc_iter_rows=rows,
    ...
    assert result == [(sibling_pid, "centurion.exe")]
...
def test_detect_concurrent_parent_chain_walks_deep(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(parent_pid, str(shim), "centurion.exe"),
        _make_proc(grandparent_pid, str(shim), "centurion.exe"),
        _make_proc(greatgrandparent_pid, str(shim), "centurion.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
...
def test_detect_concurrent_parent_walk_handles_stub_without_process(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "centurion.exe"),
        _make_proc(other_pid, str(shim), "centurion.exe"),
    ]
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    matches = [(1234, "centurion.exe"), (5678, "centurion.exe")]
    msg = cli_main._format_concurrent_instances_message(matches, tmp_path)

    assert "1234" in msg
    assert "5678" in msg
    assert "centurion.exe" in msg
    assert "Centurion Desktop" in msg
    assert "--force" in msg
    # Mentions the file that would have been overwritten
    assert str(tmp_path / "centurion.exe") in msg


# ---------------------------------------------------------------------------
# _quarantine_running_centurion_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "centurion.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_centurion_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("centurion.exe.old.")
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_centurion_exe_shims", lambda d: [shim])
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Centurion Desktop" in captured or "gateway" in captured.lower()
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another centurion.exe is running, the update bails out before
    ...
    with patch.object(
        ...
        cli_main,
        "_detect_concurrent_centurion_instances",
        return_value=[(4242, "centurion.exe")],
    ), patch.object(
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "centurion.exe")])
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_centurion_instances", detect
    ), patch.object(
