"""Tests for IRC gateway configuration via `centurion setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Centurion install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Centurion install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#centurion")
            monkeypatch.setenv("IRC_NICKNAME", "centurion-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `centurion setup gateway` experience with IRC present."""
