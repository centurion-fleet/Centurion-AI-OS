"""Tests for centurion backup and import commands."""
...
def _make_centurion_tree(root: Path) -> None:
    ...
    (root / "centurion_state.db").write_bytes(b"fake-state")
...
class TestBackup:
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/centurion-backup-*.zip."""
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        ...
        zips = list(tmp_path.glob("centurion-backup-*.zip"))
        assert len(zips) == 1
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Centurion backup."""
        from centurion_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only centurion_state.db (old wrong name) is rejected."""
        from centurion_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["centurion_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into centurion home."""
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        monkeypatch.setenv("CENTURION_HOME", str(centurion_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_strips_centurion_prefix(self, tmp_path, monkeypatch):
        """Import strips .centurion/ prefix if all entries share it."""
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        monkeypatch.setenv("CENTURION_HOME", str(centurion_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".centurion/config.yaml": "model: test\n",
            ".centurion/skills/a/SKILL.md": "# A\n",
        })
    ...
    def test_rejects_non_centurion_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a centurion backup."""
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        monkeypatch.setenv("CENTURION_HOME", str(centurion_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        # traversal file should NOT exist outside centurion home
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from centurion_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".centurion"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("CENTURION_HOME", str(dst_home))
...
class TestFormatSize:
    def test_bytes(self):
        from centurion_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without centurion markers fails validation."""
        import io
        from centurion_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_hermes(self):
        ...
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".centurion/config.yaml", "test")
            zf.writestr(".centurion/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".centurion/"
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        with zipfile.ZipFile(buf, "w") as zf:
            ...
            zf.writestr(".centurion/", "")
            zf.writestr(".centurion/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ""
...
class TestBackupEdgeCases:
    def test_nonexistent_centurion_home(self, tmp_path, monkeypatch):
        """Backup exits when centurion home doesn't exist."""
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("centurion-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_centurion_home(self, tmp_path, monkeypatch):
        """Backup handles empty centurion home (no files to back up)."""
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert "centurion -p coder" in coder_wrapper
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        import centurion_cli.backup as backup_mod
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from centurion_cli.backup import _safe_copy_db
...
class TestPreUpdateBackup:
    ...
    def test_backup_contents_match_full_backup(self, centurion_home):
        ...
        ``centurion backup`` would, and should exclude the same directories."""
        from centurion_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(centurion_home=centurion_home)
        assert out is not None
...
class TestRunPreUpdateBackup:
    ...
    def centurion_home(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for centurion_cli.config + centurion_constants so they pick up CENTURION_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("centurion_cli.config") or mod == "centurion_constants":
    ...
    def test_backup_flag_creates_backup(self, centurion_home, capsys):
        ...
        assert "centurion import" in out
...
# ---------------------------------------------------------------------------
# Pre-migration backup (centurion claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``centurion claw migrate`` runs before mutating ~/.centurion/."""
    ...
    def test_creates_backup_under_backups_dir(self, centurion_home):
        ...
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `centurion
    ...
    def test_backup_uses_shared_exclusion_rules(self, centurion_home):
        ...
        ``centurion backup`` / ``create_pre_update_backup`` — no drift."""
        from centurion_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(centurion_home=centurion_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_centurion_import(self, centurion_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Centurion
        backup — `centurion import` should accept it."""
