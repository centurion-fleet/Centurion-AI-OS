...
def centurion_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves CENTURION_HOME.
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from centurion_cli.goals import _parse_judge_response
...
class TestJudgeGoal:
    def test_empty_goal_skipped(self):
        from centurion_cli.goals import judge_goal
...
class TestGoalManager:
    ...
    def test_continuation_prompt_shape(self, centurion_home):
        ...
        mgr.set("port goal command to centurion")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to centurion" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from centurion_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from centurion_cli import goals
        from centurion_cli.goals import GoalManager, load_goal
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        import json
        from centurion_cli.goals import GoalState
