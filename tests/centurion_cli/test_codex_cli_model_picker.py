...
Covers:
 - Normal path (tokens already in Centurion auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Centurion
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `centurion auth openai-codex`. The old
...
@pytest.fixture()
def centurion_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Centurion auth store (no Codex CLI needed)."""
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Centurion
    auth store."""
    centurion_home = tmp_path / ".centurion"
    centurion_home.mkdir()
