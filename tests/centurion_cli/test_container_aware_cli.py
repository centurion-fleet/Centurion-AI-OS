...
def container_env(tmp_path, monkeypatch):
    ...
    container_mode.write_text(
        ...
        "exec_user=centurion\n"
        "centurion_bin=/data/current-package/bin/centurion\n"
    )
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    assert info["centurion_bin"] == "/data/current-package/bin/centurion"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("centurion_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=centurion-custom\n"
        "exec_user=myuser\n"
        "centurion_bin=/opt/centurion/bin/centurion\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "centurion-custom"
    assert info["exec_user"] == "myuser"
    assert info["centurion_bin"] == "/opt/centurion/bin/centurion"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from centurion_cli.main import _exec_in_container
    ...
    assert "/data/current-package/bin/centurion" in cmd
    assert "chat" in cmd
