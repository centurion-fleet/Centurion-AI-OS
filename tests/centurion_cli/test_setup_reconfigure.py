...
- Bare `centurion setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `centurion setup --quick` runs the narrower "fill in missing items" flow.
- `centurion setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `centurion setup` on an existing install = full reconfigure wizard."""
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`centurion setup --reconfigure` behaves the same as bare `centurion setup`."""
