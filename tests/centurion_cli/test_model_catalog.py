...
def isolated_home(tmp_path, monkeypatch):
    """Isolate CENTURION_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from centurion_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "nous": {
                "metadata": {"display_name": "Centurion Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from centurion_cli.model_catalog import _validate_manifest
...
class TestIntegrationWithModelsModule:
    ...
    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        ...
        from centurion_cli import model_catalog
        from centurion_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from centurion_cli import model_catalog
        from centurion_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_manifest(self, tmp_path, monkeypatch):
        ...
        the slash-command picker until the next Centurion release.
        """
        ...
        # ``CENTURION_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` CENTURION_HOME directly instead.
        import importlib
        from centurion_cli import model_catalog
        importlib.reload(model_catalog)
        try:
            from centurion_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["CENTURION_HOME"])
