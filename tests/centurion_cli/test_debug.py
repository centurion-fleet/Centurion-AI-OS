"""Tests for ``centurion debug`` CLI command and debug utilities."""
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from centurion_cli.debug import _upload_dpaste_com
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of CENTURION_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    def centurion_home_with_secret(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("CENTURION_REDACT_SECRETS", raising=False)
    ...
    def test_force_true_works_when_redaction_disabled(
        ...
        input unchanged when CENTURION_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("CENTURION_REDACT_SECRETS", "false")

        from centurion_cli.debug import _capture_log_snapshot

        assert os.environ.get("CENTURION_REDACT_SECRETS", "") == "false"
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, centurion_home):
        ...
        assert "--- centurion dump ---" in report
...
class TestRunDebugShare:
    ...
    def test_share_uploads_three_pastes(self, centurion_home, capsys):
        ...
        with patch("centurion_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- centurion dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- centurion dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- centurion dump ---" in gateway_paste
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from centurion_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from centurion_cli.debug import delete_paste
...
class TestScheduleAutoDelete:
    ...
    handle the DELETE requests synchronously on the next ``centurion debug``
    invocation.
    """
    ...
    def test_does_not_spawn_subprocess(self, centurion_home):
        ...
        import ast
        import inspect
        from centurion_cli.debug import _schedule_auto_delete
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_survives_sweep_failure(self, centurion_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: centurion debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from centurion_cli.debug import run_debug_delete
