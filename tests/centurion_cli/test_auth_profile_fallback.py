...
When ``CENTURION_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.centurion/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.centurion            (has its own auth.json fixture)
    * Profile     -> tmp_path/.centurion/profiles/coder   (active, CENTURION_HOME points here)
    ...
    global_root = tmp_path / ".centurion"
    global_root.mkdir()
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (CENTURION_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.centurion as the "real home" — which is NOT equal
    # to the CENTURION_HOME we set (tmp_path/classic), so the guard passes.
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from centurion_cli.auth import read_credential_pool, write_credential_pool
