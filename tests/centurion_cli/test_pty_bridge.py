"""Unit tests for centurion_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf centurion-ok"])
        try:
            output = _read_until(bridge, b"centurion-ok")
            assert b"centurion-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$CENTURION_PTY_TEST\""],
            env={**os.environ, "CENTURION_PTY_TEST": "pty-env-works"},
        )
        try:
