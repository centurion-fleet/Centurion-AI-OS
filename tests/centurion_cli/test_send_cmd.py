"""Tests for the ``centurion send`` CLI subcommand.
...
from centurion_cli import send_cmd
...
def test_load_centurion_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``centurion send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``centurion config set``) lives
