"""Tests for the `centurion proxy` subcommand and its upstream adapters."""
...
from centurion_cli.proxy.adapters import ADAPTERS, get_adapter
from centurion_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from centurion_cli.proxy.adapters.nous_portal import NousPortalAdapter
from centurion_cli.proxy.adapters.xai import XAIGrokAdapter
...
def test_nous_adapter_metadata():
    ...
    assert adapter.display_name == "Centurion Portal"
...
def test_nous_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # CENTURION_HOME is already set by conftest, but make doubly sure
...
def test_nous_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("CENTURION_HOME", str(tmp_path))
    adapter = NousPortalAdapter()
    with pytest.raises(RuntimeError, match="centurion login nous"):
...
from centurion_cli.proxy.server import create_app  # noqa: E402
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "Centurion-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("CENTURION_HOME", str(tmp_path))
    from centurion_cli.proxy.cli import cmd_proxy_status
    ...
    assert "nous" in out
    assert "Centurion Portal" in out
    assert "not logged in" in out


def test_cmd_proxy_providers_runs(capsys):
    from centurion_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from centurion_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "centurion login nous" in err
