...
The root ``package.json`` is installed by ``centurion update`` on every user,
...
* ``@askjo/camofox-browser`` is NOT eager. It is an explicit opt-in
  ...
  ``centurion tools`` → Browser Automation → Camofox, and only used at
  runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
  Firefox-fork binary, which silently blocked ``centurion update`` for
...
guidance in the ``centurion-os-dev`` skill before changing the
expectations.
"""
...
def test_camofox_is_not_in_root_dependencies() -> None:
    ...
    assert "@askjo/camofox-browser" not in deps, (
        ...
        "Camofox post_setup handler in centurion_cli/tools_config.py so it "
        "only installs when the user explicitly selects Camofox via "
        "`centurion tools` → Browser Automation → Camofox."
    )
