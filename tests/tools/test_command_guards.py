...
def _clean_state():
    ...
    saved = {}
    for k in ("CENTURION_INTERACTIVE", "CENTURION_GATEWAY_SESSION", "CENTURION_EXEC_ASK", "CENTURION_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["CENTURION_INTERACTIVE"] = "1"
        session_key = os.getenv("CENTURION_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No CENTURION_INTERACTIVE or CENTURION_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
