...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_hermes_bin(self):
        """After _INSTALL_FAILED, manual install in CENTURION_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
...
class TestHermesHomeIsolation:
    def test_hermes_bin_dir_respects_hermes_home(self):
        """_hermes_bin_dir must use CENTURION_HOME, not hardcoded ~/.hermes."""
        from tools.tirith_security import _hermes_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"CENTURION_HOME": tmpdir}):
            result = _hermes_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_hermes_home(self):
        """_failure_marker_path must use CENTURION_HOME, not hardcoded ~/.hermes."""
    ...
    def test_conftest_isolation_prevents_real_home_writes(self):
        ...
        hermes_home = os.getenv("CENTURION_HOME")
        assert hermes_home is not None, "CENTURION_HOME should be set by conftest"
    ...
    def test_get_hermes_home_fallback(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            # Remove CENTURION_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".centurion")
