...
def isolated_session(monkeypatch, tmp_path):
    ...
    CENTURION_HOME so the real user's command_allowlist doesn't leak in."""
    import tools.approval as _am

    session_key = "test:session:approval_hooks"
    token = set_current_session_key(session_key)
    monkeypatch.setenv("CENTURION_SESSION_KEY", session_key)
...
class TestCliPathFiresHooks:
    """CLI-interactive approval path: CENTURION_INTERACTIVE is set, the
...
class TestGatewayPathFiresHooks:
    """Async gateway approval path: CENTURION_GATEWAY_SESSION is set and a
