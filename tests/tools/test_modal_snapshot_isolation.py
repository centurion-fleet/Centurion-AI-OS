...
def _restore_tool_modules():
    original_centurion_home = os.environ.get("CENTURION_HOME")
    original_modules = {
        name: module
        for name, module in sys.modules.items()
        if name == "tools"
        or name.startswith("tools.")
        or name == "centurion_cli"
        or name.startswith("centurion_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_centurion_home is None:
            os.environ.pop("CENTURION_HOME", None)
        else:
            os.environ["CENTURION_HOME"] = original_centurion_home
        _reset_modules(("tools", "centurion_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="centurion-os", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": centurion_home / "modal_snapshots.json",
