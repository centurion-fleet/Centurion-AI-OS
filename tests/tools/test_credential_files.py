...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        (centurion_home / "token.json").write_text("{}")
    ...
    def test_dict_with_name_key_fallback(self, tmp_path):
        ...
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        (centurion_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        (centurion_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        (centurion_home / "real.json").write_text("{}")
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape CENTURION_HOME."""
        monkeypatch.setenv("CENTURION_HOME", str(tmp_path / ".centurion"))
        (tmp_path / ".centurion").mkdir()

        # Create a sensitive file one level above centurion_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside centurion_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside CENTURION_HOME must still be registered."""
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        monkeypatch.setenv("CENTURION_HOME", str(centurion_home))
        (centurion_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_centurion_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of CENTURION_HOME must be allowed."""
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        subdir = centurion_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside CENTURION_HOME pointing outside must be rejected."""
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        monkeypatch.setenv("CENTURION_HOME", str(centurion_home))

        # Create a sensitive file outside centurion_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside centurion_home pointing outside
        symlink = centurion_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes CENTURION_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, centurion_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape CENTURION_HOME."""
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        monkeypatch.setenv("CENTURION_HOME", str(centurion_home))
        ...
        sensitive.write_text("{}")
        self._write_config(centurion_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(centurion_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside CENTURION_HOME via config must still mount."""
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        monkeypatch.setenv("CENTURION_HOME", str(centurion_home))

        (centurion_home / "oauth.json").write_text("{}")
        self._write_config(centurion_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        (centurion_home / "cache" / "documents").mkdir(parents=True)
        (centurion_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("CENTURION_HOME", str(centurion_home))
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        centurion_home = tmp_path / ".centurion"
        centurion_home.mkdir()
        # Use legacy dir name — get_centurion_dir prefers old if it exists
        (centurion_home / "document_cache").mkdir()
        (centurion_home / "image_cache").mkdir()
        monkeypatch.setenv("CENTURION_HOME", str(centurion_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(centurion_home / "document_cache") in host_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        centurion_home = tmp_path / ".centurion"
        doc_dir = centurion_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        centurion_home = tmp_path / ".centurion"
        ss_dir = centurion_home / "cache" / "screenshots"
