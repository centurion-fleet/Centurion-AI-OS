...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_centurion_home_override(self, tmp_path):
        """Auth lookup should read from CENTURION_HOME/auth.json, not ~/.centurion/auth.json."""
        ...
        with patch.dict(os.environ, {
            "HOME": str(real_home),
            "CENTURION_HOME": str(centurion_home),
        }, clear=False):
            import tools.web_tools
...
class TestBackendSelection:
    ...
    ``centurion tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
