...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("centurion/centurion-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned centurion/* and pr-* branches."""

    def test_prunes_orphaned_centurion_branch(self, git_repo):
        """centurion/centurion-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "centurion/centurion-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "centurion/centurion-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "centurion/centurion-deadbeef" in result.stdout
        ...
        assert "centurion/centurion-deadbeef" in orphaned
        ...
        assert "centurion/centurion-deadbeef" not in result.stdout
