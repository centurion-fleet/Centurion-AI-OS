...
def session_db(tmp_path):
    ...
    os.environ["CENTURION_HOME"] = str(tmp_path / ".centurion")
    os.makedirs(tmp_path / ".centurion", exist_ok=True)
    from centurion_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".centurion" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal CenturionCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        CenturionCLI._handle_branch_command(cli_instance, "/branch refactor approach")
