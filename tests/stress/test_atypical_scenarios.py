...
normal tests assume away:
  ...
  - Filesystem: CENTURION_HOME with spaces / unicode / symlinks.
...
def scenario(name):
    """Decorator: run `fn` in its own CENTURION_HOME, collect failures.
    ...
    def wrap(fn):
        def run():
            home = tempfile.mkdtemp(prefix=f"centurion_atyp_{name}_")
...
def _(home, kb):
    ...
    env = {**os.environ, "PYTHONPATH": str(WT), "CENTURION_HOME": home, "HOME": home}
    bad_metas = [
        "not-json",
...
def _(home, kb):
    ...
    try:
        ...
        # allow escape.
        try:
            from centurion_cli.kanban_db import resolve_workspace
            ...
            home_abs = str(Path(os.environ["CENTURION_HOME"]).resolve())
...
def _(home, kb):
    ...
    try:
        ...
        print(f"  CENTURION_HOME with spaces: OK at {weird}")
    finally:
        conn.close()
...
def _(home, kb):
    ...
    weird = f"/tmp/centurion_héllo_émöji_{os.getpid()}"
    ...
    try:
        ...
        print(f"  CENTURION_HOME with unicode path: OK at {weird}")
    finally:
        conn.close()
        shutil.rmtree(weird, ignore_errors=True)
...
def _(home, kb):
    ...
    try:
        os.environ["CENTURION_HOME"] = link1
        ...
        os.environ["CENTURION_HOME"] = link2
        ...
        conn2.close()
        print("  symlinks to same CENTURION_HOME share DB correctly")
    finally:
...
def _(home, kb):
    ...
    try:
        from centurion_cli import web_server as ws  # noqa
    except Exception:
        pass
