...
def server():
    with patch.dict(
        "sys.modules",
        {
            "centurion_constants": MagicMock(
                get_centurion_home=MagicMock(return_value="/tmp/centurion_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'centurion-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'centurion-release' patched"
    }
