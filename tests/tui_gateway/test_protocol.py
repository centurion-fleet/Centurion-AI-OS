...
def server():
    with patch.dict("sys.modules", {
        "centurion_constants": MagicMock(get_centurion_home=MagicMock(return_value="/tmp/centurion_test")),
        "centurion_cli.env_loader": MagicMock(),
        "centurion_cli.banner": MagicMock(),
        "centurion_state": MagicMock(),
    }):
        import importlib
...
def test_disable_flush_env_var_actually_wires_to_module_constant(monkeypatch):
    """End-to-end: setting `CENTURION_TUI_GATEWAY_NO_FLUSH=1` and importing
    ...
    monkeypatch.setenv("CENTURION_TUI_GATEWAY_NO_FLUSH", "1")
    ...
    try:
        ...
        monkeypatch.delenv("CENTURION_TUI_GATEWAY_NO_FLUSH", raising=False)
...
def test_config_load_missing(server, tmp_path):
    server._centurion_home = tmp_path
...
def test_slash_exec_rejects_skill_commands(server):
    ...
    fake_skills = {"/centurion-os-dev": {"name": "centurion-os-dev", "description": "Dev workflow"}}

    with patch("agent.skill_commands.get_skill_commands", return_value=fake_skills):
        resp = server.handle_request({
            "id": "r1",
            "method": "slash.exec",
            "params": {"command": "centurion-os-dev", "session_id": sid},
        })
...
def test_slash_exec_handles_plugin_commands_in_live_gateway(server):
    ...
    with patch(
        "centurion_cli.plugins.get_plugin_command_handler",
...
def test_command_dispatch_returns_skill_payload(server):
    ...
    with patch("agent.skill_commands.scan_skill_commands", return_value=fake_skills), \
        ...
        resp = server.handle_request({
            "id": "r2",
            "method": "command.dispatch",
            "params": {"name": "centurion-os-dev", "session_id": sid},
        })
    ...
    assert result["name"] == "centurion-os-dev"
