...
$CENTURION_HOME/profiles/<name>/gateway_state.json on every boot and
...
def restart_container(request, built_image: str):
    ...
    preserves $CENTURION_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"centurion-restart-{safe}"
    volume = f"centurion-restart-vol-{safe}"
    _docker("rm", "-f", name)
