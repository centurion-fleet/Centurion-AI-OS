...
def _run_gateway_import(centurion_home: Path, initial_env: dict[str, str]) -> dict[str, str]:
...
@pytest.fixture
def centurion_home(tmp_path: Path) -> Path:
...
def test_config_gateway_timeout_wins_over_stale_env(centurion_home: Path) -> None:
    """Every agent.* bridge key must be config-authoritative, not .env-authoritative."""
    _write_config(centurion_home, agent_cfg={
