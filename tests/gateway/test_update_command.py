...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        assert "brew upgrade centurion-os" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _centurion_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._centurion_home", tmp_path), \
        ...
        with patch("gateway.run._centurion_home", tmp_path):
            ...
            # project_root will be the real centurion-os dir (which HAS .git).
    ...
    async def test_no_centurion_binary(self, tmp_path):
        ...
        assert "centurion update" in result
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        ...
        assert "centurion_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_centurion_bin_prefers_which(self, tmp_path):
    ...
    async def test_resolve_centurion_bin_fallback(self):
        """_resolve_centurion_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_centurion_bin
        ...
        assert result == [sys.executable, "-m", "centurion_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_centurion_bin_returns_none_when_both_fail(self):
        """_resolve_centurion_bin returns None when both strategies fail."""
        from gateway.run import _resolve_centurion_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        assert not (centurion_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((centurion_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (centurion_home / ".update_pending.json").exists()
        assert not (centurion_home / ".update_exit_code").exists()
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (centurion_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = centurion_home / ".update_pending.claimed.json"
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (centurion_home / ".update_pending.json").write_text(json.dumps(pending))
        (centurion_home / ".update_output.txt").write_text("x" * 5000)
        (centurion_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (centurion_home / ".update_pending.json").write_text(json.dumps(pending))
        (centurion_home / ".update_output.txt").write_text("Traceback: boom")
