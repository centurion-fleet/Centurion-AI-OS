...
``$CENTURION_HOME/plugins/``, runs ``PluginManager().discover_and_load()``,
and asserts the registration result.

Mirrors the structure of
``tests/centurion_cli/test_plugin_scanner_recursion.py::TestRegisterImageGenProvider``.
"""
...
class TestRegisterTTSProvider:
    ...
    def test_accepts_valid_provider(self):
        from centurion_cli.plugins import PluginManager

        from agent import tts_registry
        tts_registry._reset_for_tests()

        centurion_home = Path(os.environ["CENTURION_HOME"])
        _write_plugin(
        ...
        _enable(centurion_home, "my-tts-plugin")
    ...
    def test_rejects_non_provider(self, caplog):
        ...
        _enable(centurion_home, "bad-tts-plugin")
    ...
    def test_rejects_builtin_shadow(self, caplog):
        ...
        _enable(centurion_home, "shadow-tts-plugin")
