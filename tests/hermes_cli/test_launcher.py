...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./hermes` should use `centurion_cli.main`, not the legacy Fire wrapper."""
    ...
    def fake_main():
        called.append("centurion_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "centurion_cli.main", fake_main_module)
    ...
    assert called == ["centurion_cli.main"]
