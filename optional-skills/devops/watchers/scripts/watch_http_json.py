...
Usage (via cron with --no-agent):
    ...
    centurion cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CENTURION_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Centurion-Watcher/1.0"})
