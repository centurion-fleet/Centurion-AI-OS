...
Maps each role archetype to the Centurion skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public centurion-ai-os
...
## Centurion skills relevant to video production

### Visual / rendering skills (`Centurion-AI-OS/skills/creative/`)
...
### Design / pre-production skills (`Centurion-AI-OS/skills/creative/`)
...
### Audio / media skills (`Centurion-AI-OS/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`Centurion-AI-OS/skills/devops/`)
...
These are **not** Centurion skills but external CLIs / APIs that profiles invoke.
...
## Built-in Centurion tools for media review

These are native Centurion tools — not invoked via terminal but through their own
...
key is present in `~/.centurion/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Centurion profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.centurion/.env` → environment variable.
...
When a new Centurion-public video skill ships:
