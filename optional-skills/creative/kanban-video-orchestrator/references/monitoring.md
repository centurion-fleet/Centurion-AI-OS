...
centurion kanban watch --tenant <project-slug>

# Snapshot of the board
centurion kanban list --tenant <project-slug>
centurion kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
centurion kanban stats --tenant <project-slug>

# Visual dashboard (browser)
centurion dashboard

# Inspect a specific task (includes comments + events)
centurion kanban show <task-id>

# Follow a single task's event stream
centurion kanban tail <task-id>
```

Verify available subcommands with `centurion kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `centurion kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `centurion kanban block <id>` then `centurion kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `centurion kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `centurion kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `centurion profile list` (profile exists?), `centurion status` (gateway/dispatcher up?), and verify tenant. |
...
centurion kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
centurion kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(centurion kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
centurion kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
centurion kanban block <task-id>
centurion kanban archive <task-id>

# Diagnose what happened
centurion kanban show <task-id>      # task body, comments, recent events
centurion kanban tail <task-id>      # follow the live event stream
centurion kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `centurion kanban create` for the director
...
while true; do
    clear
    centurion kanban list --tenant <slug>
    echo "---"
    centurion kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `centurion kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `centurion profile list` and start
