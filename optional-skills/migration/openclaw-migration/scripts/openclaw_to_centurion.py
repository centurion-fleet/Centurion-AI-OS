#!/usr/bin/env python3
"""OpenClaw -> Centurion migration helper.

This script migrates the parts of an OpenClaw user footprint that map cleanly
into Centurion AI OS, archives selected unmapped docs for manual review, and
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the OpenClaw persona file into Centurion.",
    },
    "workspace-agents": {
    ...
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Centurion memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Centurion memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Centurion-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Centurion-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge OpenClaw exec approval patterns into Centurion command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy OpenClaw skills into ~/.centurion/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.centurion/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Centurion .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Centurion .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Centurion .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Centurion .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Centurion .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Centurion config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Centurion config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared OpenClaw skills from ~/.openclaw/skills/ into Centurion.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Centurion MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from OpenClaw into Centurion config.yaml.",
    },
    "plugins-config": {
    ...
    "cron-jobs": {
        ...
        "description": "Import cron job definitions. Archive for manual recreation via 'centurion cron'.",
    },
    "hooks-config": {
    ...
    "agent-config": {
        ...
        "description": "Import agent defaults (compaction, context, thinking) into Centurion config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Centurion session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Centurion custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Centurion config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Centurion config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Centurion config.yaml approvals section.",
    },
    "memory-backend": {
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Centurion config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace OpenClaw brand names with Centurion in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``OpenClaw`` → ``Centurion`` (prose), but lowercase matches
# like ``openclaw`` → ``centurion`` (so filesystem paths like ``~/.openclaw``
# become ``~/.centurion`` — the real Centurion home — not the broken ``~/.Centurion``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Centurion'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Centurion'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Centurion'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``OpenClaw`` → ``Centurion`` but maps ``openclaw`` → ``centurion`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.centurion/config.yaml`` (the real Centurion home) instead of the broken
...
def rebrand_text(text: str) -> str:
    """Replace OpenClaw / ClawdBot / MoltBot brand names with Centurion.
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# OpenClaw -> Centurion Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # OpenClaw's extensions/migrate-centurion/apply.ts "blocked by earlier
    ...
    # Option ids that mutate the Centurion config.yaml file.  Once any one of
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on OpenClaw's extensions/migrate-centurion/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Centurion session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Centurion config.yaml does not exist yet")
            return
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # that path will be stale after migration and would cause the Centurion
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Centurion-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Centurion-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: centurion config set",
                        )
                    continue
        ...
        for oc_key, centurion_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and centurion_key not in secret_additions:
                secret_additions[config_key] = val
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, centurion_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and centurion_key not in secret_additions:
                        secret_additions[config_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        centurion_config = load_yaml_file(destination)
        current_model = centurion_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = centurion_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                centurion_config["model"] = {"default": model_str}
            dump_yaml_file(destination, centurion_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # OpenClaw renamed "edge" to "microsoft"; Centurion still uses "edge"
        ...
        centurion_config = load_yaml_file(destination)
        existing_tts = centurion_config.get("tts", {})
        ...
        if self.execute:
            ...
            centurion_config["tts"] = merged_tts
            dump_yaml_file(destination, centurion_config)
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Centurion destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Centurion-compatible values were extracted; raw OpenClaw config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Centurion-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        centurion_cfg_path = self.target_root / "config.yaml"
        centurion_cfg = load_yaml_file(centurion_cfg_path)
        existing_mcp = centurion_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Centurion config")
                continue

            centurion_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                centurion_srv["command"] = srv["command"]
                if srv.get("args"):
                    centurion_srv["args"] = srv["args"]
                if srv.get("env"):
                    centurion_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    centurion_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                centurion_srv["url"] = srv["url"]
                if srv.get("headers"):
                    centurion_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    centurion_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                centurion_srv["enabled"] = False
            if srv.get("timeout"):
                centurion_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                centurion_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                centurion_srv["tools"] = {}
                if tools_cfg.get("include"):
                    centurion_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    centurion_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                centurion_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = centurion_srv
            added += 1
        ...
        if added > 0 and self.execute:
            ...
            centurion_cfg["mcp_servers"] = existing_mcp
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'centurion cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = centurion_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map OpenClaw thinking -> Centurion reasoning_effort
        ...
        if compaction:
            compression = centurion_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = centurion_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            centurion_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = centurion_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'centurion gateway' to configure.")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        centurion_cfg_path = self.target_root / "config.yaml"
        centurion_cfg = load_yaml_file(centurion_cfg_path)
        sr = centurion_cfg.get("session_reset") or {}
        changes = False
        ...
        if changes:
            centurion_cfg["session_reset"] = sr
            if self.execute:
                self.maybe_backup(centurion_cfg_path)
                dump_yaml_file(centurion_cfg_path, centurion_cfg)
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        centurion_cfg_path = self.target_root / "config.yaml"
        centurion_cfg = load_yaml_file(centurion_cfg_path)
        custom_providers = centurion_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(centurion_cfg_path)
            centurion_cfg["custom_providers"] = custom_providers
            dump_yaml_file(centurion_cfg_path, centurion_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Centurion config
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        centurion_cfg_path = self.target_root / "config.yaml"
        centurion_cfg = load_yaml_file(centurion_cfg_path)
        browser_centurion = centurion_cfg.get("browser") or {}
        changed = False
        ...
        if changed:
            centurion_cfg["browser"] = browser_centurion
            if self.execute:
                self.maybe_backup(centurion_cfg_path)
                dump_yaml_file(centurion_cfg_path, centurion_cfg)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if mode:
            ...
            centurion_mode = mode_map.get(mode, "manual")
            centurion_cfg.setdefault("approvals", {})["mode"] = centurion_mode
            if self.execute:
                self.maybe_backup(centurion_cfg_path)
                dump_yaml_file(centurion_cfg_path, centurion_cfg)
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# OpenClaw -> Centurion Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "direct 1:1 mapping in Centurion. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Centurion Config Not Overwritten)",
                "",
                "These items already existed in your Centurion config. Re-run with",
        ...
        notes.extend([
            ...
            "state files (todo.json, sessions, logs). If the Centurion agent discovers these",
            "directories, it may read/write to them instead of the Centurion state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `centurion claw cleanup` to rename the OpenClaw",
            ...
            "or todo lists, run `centurion claw cleanup` to fix it.",
            "",
            "## Centurion-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `centurion claw cleanup` to archive the OpenClaw directory (prevents state confusion)",
            "- Run `centurion setup` to configure any remaining settings",
            "- Run `centurion mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `centurion cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `centurion cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `centurion gateway install` if you need the gateway service",
            "- Review `~/.centurion/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate OpenClaw user state into Centurion AI OS.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="OpenClaw home directory")
    parser.add_argument("--target", default=os.environ.get("CENTURION_HOME") or str(Path.home() / ".centurion"), help="Centurion home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Centurion targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Centurion-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print(f"  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   OpenClaw -> Centurion Migration   [{mode_label:>8s}]   ║")
    print(f"  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.centurion/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.centurion/config.yaml")
        print("    2. Run: centurion mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: centurion cron")
