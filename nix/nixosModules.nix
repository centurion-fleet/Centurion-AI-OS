# nix/nixosModules.nix — NixOS module for centurion-ai-os
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: centurion runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $CENTURION_HOME/.env
# and read by centurion at startup — no container recreation needed for env changes.
#
# Tool resolution: the centurion wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.centurion-ai-os;
    effectivePackage =
    ...
    centurion-ai-os = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-centurion-ai-os)
    deepConfigType = lib.types.mkOptionType {
      name = "centurion-config-attrs";
      description = "Centurion YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "centurion-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "centurion-documents" { } (
      ''
        mkdir -p $out
    ...
    containerName = "centurion-ai-os";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/centurion";
    ...
    # centurion user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "centurion-container-entrypoint" ''
      set -eu

      CENTURION_UID="''${CENTURION_UID:?CENTURION_UID must be set}"
      CENTURION_GID="''${CENTURION_GID:?CENTURION_GID must be set}"

      # ── Group: ensure a group with GID=$CENTURION_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$CENTURION_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="centurion"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$CENTURION_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$CENTURION_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$CENTURION_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$CENTURION_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="centurion"
        TARGET_HOME="/home/centurion"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$CENTURION_UID" -g "$CENTURION_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$CENTURION_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$CENTURION_UID:$CENTURION_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure CENTURION_HOME is owned by the target user.
      ...
      if [ -n "''${CENTURION_HOME:-}" ] && [ -d "$CENTURION_HOME" ]; then
        find "$CENTURION_HOME" \! -user "$CENTURION_UID" -exec chown "$CENTURION_UID:$CENTURION_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/centurion-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/centurion-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/centurion ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/centurion
        chmod 0440 /etc/sudoers.d/centurion
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$CENTURION_UID" --regid="$CENTURION_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $CENTURION_HOME/.env.
    ...
    # Default: /var/lib/centurion/workspace → /data/workspace.
  ...
  in {
    options.services.centurion-ai-os = with lib; {
      enable = mkEnableOption "Centurion AI OS gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = centurion-ai-os;
        description = "The centurion-ai-os package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/centurion";
        description = "State directory. Contains .centurion/ subdir (CENTURION_HOME).";
      };
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $CENTURION_HOME/.env at activation time.
          Centurion reads this file on every startup via load_centurion_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $CENTURION_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $CENTURION_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `centurion gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the centurion user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the centurion plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Centurion discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          centurion_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as centurion (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in centurion-ai-os's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the centurion CLI to environment.systemPackages and export
          CENTURION_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.centurion symlink to the service
            stateDir. These users are automatically added to the centurion group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.centurion-ai-os.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the centurion CLI to system PATH and export CENTURION_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.centurion/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.CENTURION_HOME = "${cfg.stateDir}/.centurion";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.centurion-ai-os.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the centurion user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.centurion-ai-os: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed centurion
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure centurion is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.centurion        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.centurion/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.centurion/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.centurion/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.centurion/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.centurion/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."centurion-ai-os-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.centurion
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.centurion ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.centurion ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.centurion -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.centurion/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.centurion/$_subdir"
            chmod 2770 "${cfg.stateDir}/.centurion/$_subdir"
            find "${cfg.stateDir}/.centurion/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.centurion/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.centurion/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.centurion/config.yaml
            chmod 0640 ${cfg.stateDir}/.centurion/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.centurion/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.centurion/.managed
          chmod 0644 ${cfg.stateDir}/.centurion/.managed
    ...
    HERMES_CONTAINER_MODE_EOF
          ...
          '' else ''
            rm -f ${cfg.stateDir}/.centurion/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.centurion";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.centurion" ]; then
                  rm -f "${symlinkPath}"
                  echo "centurion-ai-os: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.centurion -> stateDir/.centurion for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.centurion";
                target = "${cfg.stateDir}/.centurion";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "centurion-ai-os: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.centurion/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.centurion/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.centurion/auth.json
              fi
            ''}
          ''}
          ...
          # Centurion reads $CENTURION_HOME/.env at startup via load_centurion_dotenv(),
    ...
    HERMES_NIX_ENV_EOF
        ...
        find ${cfg.stateDir}/.centurion/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.centurion/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.centurion/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.centurion-ai-os = {
          description = "Centurion AI OS Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            CENTURION_HOME = "${cfg.stateDir}/.centurion";
            CENTURION_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $CENTURION_HOME/.env by the activation script. load_centurion_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/centurion"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the centurion group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.centurion-ai-os = {
          description = "Centurion AI OS Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              CENTURION_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              CENTURION_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env CENTURION_UID="$CENTURION_UID" \
                --env CENTURION_GID="$CENTURION_GID" \
                --env CENTURION_HOME=${containerDataDir}/.centurion \
                --env CENTURION_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/centurion gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
