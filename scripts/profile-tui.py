#!/usr/bin/env python3
"""Drive the Centurion TUI under CENTURION_DEV_PERF and summarize the pipeline.
...
Environment overrides:
  CENTURION_PERF_LOG     (default ~/.centurion/perf.log)
  CENTURION_PERF_NODE    (default node from $PATH)
...
or produced no perf data (suggests CENTURION_DEV_PERF wiring is broken).
"""
...
DEFAULT_LOG = Path(os.environ.get("CENTURION_PERF_LOG", str(get_centurion_home() / "perf.log")))
DEFAULT_STATE_DB = get_centurion_home() / "state.db"
...
def format_report(data: dict[str, Any]) -> str:
    ...
    if not react:
        out.append("  (no react events — CENTURION_DEV_PERF wired? threshold too high?)")
    else:
...
def run_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env["CENTURION_DEV_PERF"] = "1"
    env["CENTURION_DEV_PERF_MS"] = str(args.threshold_ms)
    env["CENTURION_DEV_PERF_LOG"] = str(log)
    ...
    node = os.environ.get("CENTURION_PERF_NODE", "node")
...
def main() -> int:
    ...
    p.add_argument("--threshold-ms", type=float, default=0.0, help="CENTURION_DEV_PERF_MS (0 = capture all)")
