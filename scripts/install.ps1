# ============================================================================
# Centurion AI OS Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$CenturionHome = "$env:LOCALAPPDATA\centurion",
    [string]$InstallDir = "$env:LOCALAPPDATA\centurion\centurion-ai-os",
...
$RepoUrlSsh = "git@github.com:centurion-fleet/Centurion-AI-OS.git"
$RepoUrlHttps = "https://github.com/centurion-fleet/Centurion-AI-OS.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * Centurion AI OS Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  Sovereign AI agent — runs on your own hardware.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $CenturionHome)) {
        New-Item -ItemType Directory -Force -Path $CenturionHome | Out-Null
    }
    $envFile = Join-Path $CenturionHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $CenturionHome "node"
...
function Install-Git {
    ...
    which Centurion AI OS uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\centurion\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Centurion AI OS needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Centurion copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\centurion\git``
    ...
    # Download PortableGit into $CenturionHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $CenturionHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Centurion AI OS features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Centurion AI OS needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    #
    # Layouts:
    #   PortableGit (our default): $CenturionHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $CenturionHome\git\usr\bin\bash.exe
    $candidates += "$CenturionHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$CenturionHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
...
function Test-Node {
    ...
    $managedNode = "$CenturionHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$CenturionHome\node;$env:Path"
        Write-Success "Node.js $version found (Centurion-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # The portable zip path drops node.exe + npm into $CenturionHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $CenturionHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\centurion-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$CenturionHome\node") { Remove-Item -Recurse -Force "$CenturionHome\node" }
                Move-Item $extractedDir.FullName "$CenturionHome\node"
                ...
                $version = & "$CenturionHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $CenturionHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running centurion processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/centurion-fleet/Centurion-AI-OS/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/centurion-fleet/Centurion-AI-OS/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/centurion-fleet/Centurion-AI-OS/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\centurion-ai-os-$zipLabel.zip"
                $extractPath = "$env:TEMP\centurion-ai-os-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `centurion.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install centurion-ai-os package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and centurion.exe broken with
    ...
    # here, not 30 seconds later when the user runs `centurion`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `centurion dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `centurion dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `centurion dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up centurion command..."
    
    if ($NoVenv) {
        $centurionBin = "$InstallDir"
    } else {
        $centurionBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so centurion is globally available
    # On Windows, the centurion.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$centurionBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$centurionBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $centurionBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set CENTURION_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\centurion instead
    # of the Unix default ~/.centurion
    $currentCenturionHome = [Environment]::GetEnvironmentVariable("CENTURION_HOME", "User")
    if (-not $currentCenturionHome -or $currentCenturionHome -ne $CenturionHome) {
        [Environment]::SetEnvironmentVariable("CENTURION_HOME", $CenturionHome, "User")
        Write-Success "Set CENTURION_HOME=$CenturionHome"
    }
    $env:CENTURION_HOME = $CenturionHome
    
    # Update current session
    $env:Path = "$centurionBin;$env:Path"
    
    Write-Success "centurion command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.centurion directory structure
    New-Item -ItemType Directory -Force -Path "$CenturionHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CenturionHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CenturionHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CenturionHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CenturionHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CenturionHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CenturionHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CenturionHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$CenturionHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$CenturionHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.centurion/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.centurion/.env"
        }
    } else {
        Write-Info "~/.centurion/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$CenturionHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.centurion/config.yaml from template"
        }
    } else {
        Write-Info "~/.centurion/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Centurion's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$CenturionHome\SOUL.md"
...
"@
        ...
        Write-Success "Created ~/.centurion/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.centurion/"
    
    # Seed bundled skills into ~/.centurion/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.centurion/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.centurion/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$CenturionHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.centurion/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'centurion setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\centurion-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\centurion-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\centurion-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.centurion/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'centurion setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m centurion_cli.main setup
    } else {
        python -m centurion_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $centurionCmd = "$InstallDir\venv\Scripts\centurion.exe"
    if (-not (Test-Path $centurionCmd)) {
        $centurionCmd = "centurion"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$CenturionHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'centurion whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: centurion gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$CenturionHome\logs\gateway.log"
            Start-Process -FilePath $centurionCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$CenturionHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: centurion gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: centurion gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$CenturionHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$CenturionHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$CenturionHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$CenturionHome\centurion-ai-os\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   centurion              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   centurion setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   centurion config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   centurion config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   centurion gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   centurion update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Centurion repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Centurion to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/centurion-fleet/Centurion-AI-OS/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
