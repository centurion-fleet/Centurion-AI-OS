...
# Bootstrap Open WebUI against Centurion AI OS's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.centurion/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-centurion.sh
...
#   OPEN_WEBUI_NAME='Johnny Centurion'
...
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Centurion AI OS WebUI}"
...
install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.centurion.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.centurion</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.centurion"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.centurion"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-centurion.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Centurion AI OS
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-centurion.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.centurion/logs/openwebui.log
StandardError=append:%h/.centurion/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-centurion.service
}
