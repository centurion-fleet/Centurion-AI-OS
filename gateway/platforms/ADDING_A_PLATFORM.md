...
There are two ways to add a platform to the Centurion gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.centurion/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Centurion code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `centurion gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `centurion_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Centurion core.
...
And add it to the `centurion-gateway` composite:

```python
"centurion-gateway": {
    "includes": [..., "centurion-your-platform"]
}
```
...
## 12. Status Display (`centurion_cli/status.py`)
...
## 13. Gateway Setup Wizard (`centurion_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ centurion_cli/ toolsets.py \
  --include="*.py" -l | sort -u
