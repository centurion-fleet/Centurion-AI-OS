    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if centurion_home_str:
            takeover_path = Path(centurion_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(centurion_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded centurion-os but never re-ran ``centurion setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../centurion-gateway.service"
    ...
    # that's the common case for centurion.
    ...
    # constant in centurion_cli/gateway.py.
    headroom = 30.0
