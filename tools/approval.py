...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("CENTURION_YOLO_MODE", ""))
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("CENTURION_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("CENTURION_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("CENTURION_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set CENTURION_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind CENTURION_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds CENTURION_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("CENTURION_CRON_SESSION"):
        return False
    if env_var_enabled("CENTURION_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $CENTURION_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_CENTURION_ENV_PATH = (
    r'(?:~\/\.centurion/|'
    r'(?:\$home|\$\{home\})/\.centurion/|'
    r'(?:\$centurion_home|\$\{centurion_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bcenturion\s+gateway\s+(stop|restart)\b', "stop/restart centurion gateway (kills running agents)"),
    (r'\bcenturion\s+update\b', "centurion update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart centurion-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart centurion-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(centurion|gateway|cli\.py)\b', "kill centurion/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill centurion` but not
    # `kill -9 $(pgrep -f centurion)` because the substitution is opaque
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("CENTURION_INTERACTIVE")
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s — set CENTURION_INTERACTIVE or CENTURION_GATEWAY_SESSION to require approval.",
            description, command[:200],
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("CENTURION_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = env_var_enabled("CENTURION_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("CENTURION_EXEC_ASK")
