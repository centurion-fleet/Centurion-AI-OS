...
# Centurion-internal env vars that should NOT leak into terminal subprocesses.
_CENTURION_PROVIDER_ENV_FORCE_PREFIX = "_CENTURION_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from centurion_cli.config import OPTIONAL_ENV_VARS
...
_CENTURION_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _inject_context_centurion_home(env: dict) -> None:
    """Bridge the context-local Centurion home override into subprocess env."""
    try:
        from centurion_constants import get_centurion_home_override

        value = get_centurion_home_override()
        if value:
            env["CENTURION_HOME"] = value
    except Exception:
        pass


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Centurion-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_CENTURION_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _CENTURION_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_CENTURION_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_CENTURION_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _CENTURION_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_centurion_home(sanitized)
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("CENTURION_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\centurion\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\centurion\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _centurion_portable_git = os.path.join(_local_appdata, "centurion", "git") if _local_appdata else ""
    if _centurion_portable_git:
        for candidate in (
            os.path.join(_centurion_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_centurion_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Centurion AI OS requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set CENTURION_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_CENTURION_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_CENTURION_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _CENTURION_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # Centurion is driving via _find_bash (Git Bash), and Git Bash itself
    ...
    _inject_context_centurion_home(run_env)

    # Per-profile HOME isolation: redirect system tool configs (git, ssh, gh,
    # npm …) into {CENTURION_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Centurion trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``CENTURION_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under CENTURION_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_centurion_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "centurion_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._centurion_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_centurion_pgid", None)
                    if pgid is None:
                        raise
