...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``centurion tools`` repaint, tool-registration scans,
    ...
    2. ``~/.centurion/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
    ...
    try:
        ...
        auth_path = get_centurion_home() / "auth.json"
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.centurion/.env`` first, then ``os.environ``.

    Wraps :func:`centurion_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from centurion_cli.config import get_env_value as _centurion_get_env_value

        value = _centurion_get_env_value(name)
