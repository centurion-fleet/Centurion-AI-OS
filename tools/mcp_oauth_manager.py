...
`mcp_tool.py`, and `centurion_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# CenturionMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_centurion_provider_class() -> Optional[type]:
    ...
    class CenturionMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._centurion_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`CenturionTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, CenturionTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._centurion_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._centurion_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import CenturionTokenStorage
                        if isinstance(storage, CenturionTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._centurion_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import CenturionTokenStorage
            if not isinstance(storage, CenturionTokenStorage):
                return
    ...
    return CenturionMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_CENTURION_PROVIDER_CLS: Optional[type] = _make_centurion_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`CenturionMCPOAuthProvider` directly using the helpers
        ...
        storage = CenturionTokenStorage(server_name)
        ...
        return _CENTURION_PROVIDER_CLS(
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``centurion mcp remove <name>`` and (indirectly) by
        ``centurion mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
