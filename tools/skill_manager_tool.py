...
~/.centurion/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `centurion config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.centurion/skills/ (single source of truth)
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`centurion curator unpin {name}` if they want to delete it. "
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.centurion/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Centurion profiles.
    ...
    try:
        root = get_default_centurion_root()
    except Exception:
        return matches
    ...
    # Default profile (~/.centurion/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.centurion/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`centurion -p "
            ...
            base += (
                ...
                f"Switch profiles (`centurion -p <name>`) to edit there, or "
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_centurion_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `centurion curator unpin <name>`. "
