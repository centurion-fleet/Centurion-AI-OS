...
This module provides the glue:
    - ``CenturionTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Centurion AI OS"
"""
...
from centurion_constants import secure_parent_dir
...
def _get_token_dir() -> Path:
    ...
    Uses CENTURION_HOME so each profile gets its own OAuth tokens.
...
# ---------------------------------------------------------------------------
# CenturionTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class CenturionTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        CENTURION_HOME/mcp-tokens/<server_name>.json         -- tokens
        CENTURION_HOME/mcp-tokens/<server_name>.client.json   -- client info
        CENTURION_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Centurion records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Centurion.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  See: https://github.com/centurion-fleet/Centurion-AI-OS/tree/main/website/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `centurion mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Centurion AI OS")
    scope = cfg.get("scope")
