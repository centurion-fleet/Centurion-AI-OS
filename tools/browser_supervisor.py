...
One ``CDPSupervisor`` runs per Centurion ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "centurion-dialog-bridge.invalid"
...
(() => {
  if (window.__centurionDialogBridgeInstalled) return;
  window.__centurionDialogBridgeInstalled = true;
  const ENDPOINT = "http://centurion-dialog-bridge.invalid/";
