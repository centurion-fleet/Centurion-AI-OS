# Centurion AI OS Security Policy

This document describes Centurion AI OS's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/centurion-fleet/Centurion-AI-OS/security/advisories/new)
or **security@centurion-fleet.com**. Do not open public issues for
security vulnerabilities. **Centurion AI OS does not operate a bug
bounty program.**
...
- Environment details (`centurion version`, commit SHA, OS, Python
  version).
...
Centurion AI OS is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Centurion AI OS,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Centurion AI OS access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Centurion AI OS's documentation
...
Centurion AI OS supports two OS-level isolation postures. They address
...
Centurion AI OS supports this in two ways:

- **Centurion AI OS's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Centurion AI OS's in-process heuristics
...
Centurion AI OS filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Centurion AI OS
itself. Bugs in Centurion AI OS's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Centurion AI OS does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Centurion AI OS's own documentation, or reasonable
  operator expectations would predict — including cases where
  Centurion AI OS has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  centurion-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Centurion AI OS's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Centurion AI OS. Bugs in Centurion AI OS's
  skill-install or plugin-install path that prevent the
...
- Centurion AI OS includes supply-chain guards for MCP server
